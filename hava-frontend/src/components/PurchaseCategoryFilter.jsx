...
function PurchaseCategoryFilter({
  ...
  const DropdownInternal = ({
    ...
    return (
      <div className={`relative ${internalClassName}`} ref={dropdownRef}>
        ...
        >
          <span className="flex-grow truncate overflow-hidden whitespace-nowrap">
            {selectedOptionLabel}
          </span>
          <motion.div
  ...
  if (renderInlineLabel) {
    return (
      <div
        className={`flex items-center space-x-3 ${variant === "home" ? "mb-0" : "mb-6"} ${className}`}
      >
        {/* Added dark mode class for label text */}
        <label className="text-green-700 dark:text-green-400 font-semibold whitespace-nowrap">
          Show:
        </label>
        <DropdownInternal

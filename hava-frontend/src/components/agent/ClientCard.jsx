...
const ClientCard = ({
  ...
  return (
    <Card
      ...
      onClick={() => {
        !isPendingRequestCard && onViewProfile(client.user_id);
      }}
    >
      <div className="flex flex-row-reverse items-start gap-4 mb-4">
        <div className="flex-shrink-0 flex flex-col items-center relative">
          ...
          <div className="w-28 text-center text-xs font-medium mt-4">
            {" "}
            {/* Increased mt-4 for more space */}
            <span
              className={
                isPendingRequestCard
                  ...
                  : client.client_status === "vip"
                    ...
                    : darkMode
                      ? "text-gray-500"
                      : "text-gray-500" // Regular color for both light/dark
              }
            >
              {isPendingRequestCard
                ? "Pending"
                : formatStatus(client.client_status)}
            </span>
          </div>
        </div>
        ...
        <div className="flex-grow text-left min-w-0 break-words">
          ...
          <div className="text-xs mb-1 text-gray-600 dark:text-gray-300 break-words">
            {" "}
          ...
          <div className="text-xs mb-1 text-gray-600 dark:text-gray-300 break-words">
            ...
            {/* Reduced N/A font size */}
          </div>
          {client.date_joined && (
            <div className="text-xs text-gray-500 dark:text-gray-400">
              {isPendingRequestCard
                ? `Requested: ${formatDate(client.date_joined)}`
                : `Joined: ${formatDate(client.date_joined)}`}
            </div>
          )}
          {isAgencyAdmin &&
            client.agent_name && ( // Display assigned agent for agency admins
          ...
          <div className="w-full mt-4">
            {isPendingRequestCard ? (
              <div className="text-sm text-gray-500 dark:text-gray-400">
                <span
                  className={`font-medium ${darkMode ? "text-gray-300" : "text-gray-700"} block mb-1`}
                >
                  Message:
                </span>
              ...
              <div
                className="text-sm text-gray-500 dark:text-gray-400 relative break-words min-w-0"
                onClick={(e) => e.stopPropagation()}
              >
                <div className="flex items-center gap-1 mb-1">
                  <span
                  ...
                  {!isAgencyAdmin && ( // Only show edit button if not agency admin
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        onEditNote(client.user_id, client.notes);
                      }}
                  ...
                  {isAgencyAdmin && ( // Tooltip for agency admin
                    <span
                      ...
                      title="Only assigned agent can edit notes."
                    ></span>
                  )}
                </div>
          ...
          {isPendingRequestCard ? (
            ...
            <div className="flex gap-1 flex-nowrap overflow-x-auto">
              ...
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  onToggleStatus(client.user_id, client.client_status);
                }}
                ...
                title={
                  isAgencyAdmin
                    ? "Only assigned agent can change client status"
                    : client.client_status === "vip"
                      ? "Make Regular"
                      : "Make VIP"
                }
              ...
              <button
                ...
                title={
                  isAgencyAdmin
                    ? "Only assigned agent can remove client"
                    : "Disconnect client"
                }
                disabled={isAgencyAdmin}
              >

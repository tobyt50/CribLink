...
const ListingCard = ({ listing, onAction }) => {
  ...
  const performAction = async (action) => {
    try {
      ...
      showMessage(
        `Listing ${listing.title} successfully ${action}d!`,
        "success",
      );
    } catch (err) {
  ...
  return (
    ...
    >
      <h3
        className={`text-lg font-semibold ${darkMode ? "text-green-400" : "text-gray-800"}`}
      >
        {listing.title}
      </h3>
      ...
      <p
        className={`mt-2 line-clamp-3 ${darkMode ? "text-gray-300" : "text-gray-700"}`}
      >
        {listing.description}
      </p>
      ...
      <div className="flex justify-between mt-4">
        ...
        <div className="flex gap-2">
          <button
            ...
            className={`text-sm hover:underline ${darkMode ? "text-green-400 hover:text-green-300" : "text-green-600 hover:text-green-800"}`}
          >
            Approve
          </button>
          <button
            onClick={() => performAction("reject")}
            className={`text-sm hover:underline ${darkMode ? "text-red-400 hover:text-red-300" : "text-red-600 hover:text-red-800"}`}
          >
            Reject
          </button>
          <button
            onClick={() => performAction("feature")}
            className={`text-sm hover:underline ${darkMode ? "text-blue-400 hover:text-blue-300" : "text-blue-600 hover:text-blue-800"}`}
          >
            Feature
          </button>
          <button

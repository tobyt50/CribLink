...
import { Hourglass, UserPlus, UserX, X } from "lucide-react"; // Import all necessary icons
 ...
 * @param {function} [props.onFavouriteToggle] - Function to call when favourite button is clicked.
 * @param {boolean} [props.isFavourited=false] - Indicates if the agency is currently favourited.
 */
function AgencyCard({
  agency,
  onClick,
  ...
  const currentAffiliation = agentMemberships.find(
    (m) => m.agency_id === agency.agency_id,
  );
  ...
  const isThisAgencyCurrentUsersAdminAgency =
    isCurrentUserAgencyAdmin && agency.agency_id === currentUserAgencyId;

  // Determine if the agent has reached the maximum number of affiliations (connected or pending)
  const hasReachedMaxAffiliations =
    agentMemberships.filter(
      (m) => m.request_status === "accepted" || m.request_status === "pending",
    ).length >= maxAgencyAffiliations;
  ...
  const isConnectDisabled =
    ...
    isConnected ||
    isPending ||
    hasReachedMaxAffiliations ||
    isThisAgencyCurrentUsersAdminAgency;

  // Determine if the "Disconnect" button should be disabled for an admin
  const isDisconnectDisabledForAdmin =
    isThisAgencyCurrentUsersAdminAgency && isLastAdminOfOwnAgency;

  const handleFavouriteClick = (e) => {
    e.stopPropagation(); // Prevent card click from triggering
    if (onFavouriteToggle) {
      // Ensure onFavouriteToggle is provided
      onFavouriteToggle(agency.agency_id, isFavourited);
    }
  };
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-row-reverse items-start gap-4 mb-2">
        {/* Right side: Logo */}
        <div className="flex-shrink-0 flex flex-col items-center mt-8 relative">
          {" "}
          {/* Increased pt- to lower logo further */}
          {agency.logo_url ? (
            <img
              src={agency.logo_url}
              ...
              onError={(e) => {
                ...
                e.target.src = `https://placehold.co/96x96/${darkMode ? "374151" : "E0F7FA"}/${darkMode ? "D1D5DB" : "004D40"}?text=${getAgencyInitial(agency.name)}`;
              }}
            />
          ) : (
            <div
              className={`w-24 h-24 rounded-full flex items-center justify-center text-3xl font-bold border-2 border-green-500
          ...
          <div className="space-y-1">
            ...
            <div className="text-sm mb-1 text-gray-600 dark:text-gray-300 overflow-hidden text-ellipsis whitespace-nowrap">
              <a
                ...
                className={`${darkMode ? "text-gray-300" : "text-gray-600"} hover:underline`}
      ...
      {agency.description && (
        <div className="w-full mb-2">
          <p
            className={`text-xs text-left italic overflow-y-auto max-h-10 ${darkMode ? "text-gray-500" : "text-gray-500"}`}
          >
            {agency.description}
          </p>
        </div>
            ...
            <>
              {isThisAgencyCurrentUsersAdminAgency ? ( // If the current user is an admin of THIS agency
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    onDisconnectClick(agency.agency_id);
                  }}
                  ...
                  title={
                    isDisconnectDisabledForAdmin
                      ? "You are the last admin of this agency. Please assign another admin before disconnecting."
                ...
                <div className="flex items-center space-x-2">
                  ...
                  {onCancelRequestClick && (
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        onCancelRequestClick(agency.agency_id, agency.name);
                      }}
                      className={`text-xs rounded-xl px-3 py-1 h-8 flex items-center justify-center
                ...
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    onConnectClick(agency.agency_id);
                  }}
                  ...
                  title={
                    isConnectDisabled
                      ? hasReachedMaxAffiliations
                        ? `You can only affiliate with ${maxAgencyAffiliations} agencies.`
                        : "Already connected or pending with this agency."
                      : "Connect to Agency"
                  }
                >

...
function Agencies() {
  ...
  // New state for favourite agencies
  const [favouriteAgenciesStatus, setFavouriteAgenciesStatus] = useState(
    new Set(),
  );
  ...
  const fetchAgentMemberships = useCallback(async () => {
    if (!user?.user_id || user?.role === "client") {
      // Clients don't have agency memberships here
      setAgentMemberships([]);
  ...
  const fetchAgencyAdminCount = useCallback(async () => {
    if (user?.role === "agency_admin" && user?.agency_id) {
      ...
      try {
        ...
        console.error(
          "Error fetching agency admin count:",
          error.response?.data || error.message,
        );
  ...
  const fetchFavouriteAgencies = useCallback(async () => {
    if (!user?.user_id) {
      setFavouriteAgenciesStatus(new Set());
      return;
    }
    try {
      const token = localStorage.getItem("token");
      ...
      const favouritedIds = new Set(
        response.data.favourites.map((fav) => fav.agency_id),
      );
      setFavouriteAgenciesStatus(favouritedIds);
    } catch (error) {
      console.error(
        "Error fetching favourite agencies:",
        error.response?.data || error.message,
      );
      // showMessage("Failed to load favourite agencies.", "error"); // Suppress for cleaner UX
      setFavouriteAgenciesStatus(new Set());
    }
  }, [user?.user_id]);
  ...
  useEffect(() => {
    fetchFavouriteAgencies();
  }, [fetchFavouriteAgencies, user?.user_id]); // Re-fetch favourites when user changes
  ...
  const handleConnectToAgency = async (targetAgencyId) => {
    ...
    const existingAffiliation = agentMemberships.find(
      (m) => m.agency_id === targetAgencyId,
    );
    if (existingAffiliation) {
      if (existingAffiliation.request_status === "pending") {
        showMessage(
          "Your request to join this agency is already pending.",
          "info",
        );
        ...
        showMessage(
          "Your request to this agency was rejected. You can re-send a request.",
          "info",
        );
      }
      return;
    ...
    if (agentMemberships.length >= MAX_AGENCY_AFFILIATIONS) {
      showMessage(
        `You can only be affiliated with a maximum of ${MAX_AGENCY_AFFILIATIONS} agencies (connected or pending requests). Please disconnect from an existing agency to join a new one.`,
        "info",
      );
      return;
    }
    ...
    showConfirm({
      ...
      onConfirm: async () => {
        try {
          ...
          const joinedAgency = agencies.find(
            (a) => a.agency_id === targetAgencyId,
          );
          if (joinedAgency) {
          ...
          console.error(
            "Error sending agency connection request:",
            error.response?.data || error.message,
          );
          showMessage(
            `Failed to send agency connection request: ${error.response?.data?.message || "Please try again."}`,
            "error",
          );
  ...
  const handleDisconnectFromAgency = async (agencyIdToDisconnect) => {
    ...
    const isDisconnectingFromOwnAdminAgency =
      user.role === "agency_admin" && user.agency_id === agencyIdToDisconnect;

    // Safeguard for last admin
    if (isDisconnectingFromOwnAdminAgency && isLastAdminOfOwnAgency) {
      showMessage(
        "You cannot disconnect from your agency as you are the last administrator. Please assign another admin first.",
        "error",
      );
      return;
    }
    ...
    const agencyToDisconnect =
      agentMemberships.find((m) => m.agency_id === agencyIdToDisconnect) ||
      agencies.find((a) => a.agency_id === agencyIdToDisconnect);
    const agencyName = agencyToDisconnect
      ? agencyToDisconnect.name || agencyToDisconnect.agency_name
      : "the agency";
    ...
    showConfirm({
      ...
      onConfirm: async () => {
        try {
          ...
          setAgentMemberships((prev) =>
            prev.filter((m) => m.agency_id !== agencyIdToDisconnect),
          );
      ...
      onConfirm: async () => {
        try {
          ...
          if (response.status === 200) {
            ...
            setAgentMemberships((prev) =>
              prev.filter((m) => m.agency_id !== agencyIdToCancel),
            );
  ...
  // Handle adding/removing agency from favourites
  const handleFavouriteToggle = useCallback(
    async (agencyId, isCurrentlyFavourited) => {
      if (!user?.user_id) {
        showMessage("Please log in to add agencies to favourites.", "info");
        return;
      }
      ...
      try {
        if (isCurrentlyFavourited) {
          await axiosInstance.delete(
            `${API_BASE_URL}/favourites/agencies/${agencyId}`,
            {
              headers: { Authorization: `Bearer ${token}` },
            },
          );
          setFavouriteAgenciesStatus((prev) => {
            const newState = new Set(prev);
            newState.delete(agencyId);
            return newState;
          });
          showMessage("Agency removed from favourites!", "success");
        } else {
          ...
          setFavouriteAgenciesStatus((prev) => new Set(prev).add(agencyId));
          showMessage("Agency added to favourites!", "success");
        }
      } catch (error) {
        console.error(
          "Error toggling favourite agency status:",
          error.response?.data || error.message,
        );
        showMessage(
        ...
        fetchFavouriteAgencies(); // Re-fetch to ensure UI consistency on error
      }
    },
    [user?.user_id, showMessage, fetchFavouriteAgencies],
  );
          ...
          {loading || loadingMemberships || loadingAdminCount ? (
            ...
            agencies.map((agency) => (
              <AgencyCard
                ...
                isFavourited={favouriteAgenciesStatus.has(agency.agency_id)} // Pass favourite status
              />
            ))
          ) : (
            <motion.div

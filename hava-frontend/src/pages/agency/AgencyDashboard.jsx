  ...
  <div className={`animate-pulse space-y-6`}>
    ...
    <div className="mt-10 grid grid-cols-1 md:grid-cols-2 gap-6">
      <Card>
        ...
        <div
          className={`mt-4 h-6 w-1/2 rounded mx-auto ${darkMode ? "bg-gray-700" : "bg-gray-200"}`}
        ></div>
      </Card>
      <Card>
        <div
      ...
      <Card>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {[...Array(3)].map((_, i) => (
            <div
              key={i}
              className={`h-24 rounded-xl ${darkMode ? "bg-gray-700" : "bg-gray-200"}`}
            ></div>
          ))}
        </div>
        <div
          className={`mt-4 h-4 w-3/4 rounded ${darkMode ? "bg-gray-600" : "bg-gray-300"}`}
        ></div>
      </Card>
    </div>
  </div>
...
const AgencyDashboard = () => {
  ...
  const [pendingAgentRequestsCount, setPendingAgentRequestsCount] =
    useState(null);
  ...
  useEffect(() => {
    if (
      !authLoading &&
      (!user || user.role !== "agency_admin" || !user.agency_id)
    ) {
      showMessage(
        "Access Denied: You must be an agency administrator to view this page.",
        "error",
      );
      navigate("/signin");
    } else if (user?.agency) {
  ...
  const goToPendingListings = () =>
    navigate("/agency/listings", { state: { statusFilter: "pending" } });
  const goToPendingAgentRequests = () =>
    navigate("/agency/members", { state: { showPendingRequests: true } });
  const goToUnderOfferListings = () =>
    navigate("/agency/listings", { state: { statusFilter: "under offer" } });
  const goToSoldListings = () =>
    navigate("/agency/listings", { state: { statusFilter: "sold" } });
  const goToMembersAdmins = () =>
    navigate("/agency/members", { state: { roleFilter: "agency_admin" } }); // Corrected role filter
  const goToMembersAgents = () =>
    navigate("/agency/members", { state: { roleFilter: "agent" } });
    ...
    const fetchAgencyStats = async () => {
      ...
      try {
        const [
          ...
          axiosInstance.get(`/agency-stats/${agencyId}/agents/count`, {
            headers,
          }),
          axiosInstance.get(`/agency-stats/${agencyId}/admins/count`, {
            headers,
          }),
          axiosInstance.get(`/agency-stats/${agencyId}/clients/count`, {
            headers,
          }),
          axiosInstance.get(`/agency-stats/${agencyId}/listings/count`, {
            headers,
          }),
          axiosInstance.get(
            `/agency-stats/${agencyId}/listings/pending-approvals`,
            { headers },
          ),
          axiosInstance.get(
            `/agency-stats/${agencyId}/pending-agent-requests/count`,
            { headers },
          ),
          axiosInstance.get(
            `/agency-stats/${agencyId}/listings/under-offer/count`,
            { headers },
          ),
          axiosInstance.get(`/agency-stats/${agencyId}/listings/sold/count`, {
            headers,
          }),
          axiosInstance.get(`/inquiries/agent/count/all-inquiries`, {
            headers,
          }), // Note: This might need to be agency-specific
          axiosInstance.get(`/inquiries/agent/count/agent-responses`, {
            headers,
          }), // Note: This might need to be agency-specific
          axiosInstance.get(`/agency-stats/${agencyId}/recent-activity`, {
            headers,
          }),
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-10">
              ...
              <Card className="lg:col-span-2">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 w-full">
                  ...
                  <StatCard
                    label="Inquiries"
                    value={clientInquiriesCount}
                    textCentered={true}
                    icon={<MessageSquare size={20} />}
                  />
                  <StatCard
                    label="Pending Agents"
                    value={pendingAgentRequestsCount}
                    onClick={goToPendingAgentRequests}
                    textCentered={true}
                    icon={<UserPlus size={20} />}
                  />
              ...
              <Card>
                ...
                {tier === "basic" && (
                  <div className="text-center p-4 rounded-lg bg-gray-100 dark:bg-gray-800">
                    ...
                    <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
                      You have access to basic listing view counts for the
                      agency.
                    </p>
                    <Link
                      to="/subscriptions"
                ...
                {tier === "pro" && (
                  <div className="text-center p-4 rounded-lg bg-purple-50 dark:bg-gray-800 border border-purple-200 dark:border-purple-700">
                    ...
                    <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
                      Track overall agency views, inquiries, and agent
                      performance.
                    </p>
                    <Link
                      to="/agency/analytics"
                ...
                {tier === "enterprise" && (
                  <div className="text-center p-4 rounded-lg bg-yellow-50 dark:bg-gray-800 border border-yellow-200 dark:border-yellow-700">
                    ...
                    <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
                      Access market heatmaps, lead tracking, and exportable
                      reports.
                    </p>
                    <Link
                      to="/agency/analytics"
              ...
              <Card>
                ...
                >
                  <li className="truncate">
                    <strong>Top Performing Agents:</strong> Identify agents with
                    most listings, sales, or inquiries.
                  </li>
                  <li className="truncate">
                    <strong>Listing Performance:</strong> Track views,
                    inquiries, and conversion rates for listings.
                  </li>
                  <li className="truncate">
                    <strong>Revenue Projections:</strong> Based on completed
                    sales (requires finance integration).
                  </li>
                  <li className="truncate">
                    <strong>Client Acquisition Trends:</strong> Analyze how new
                    clients are joining.
                  </li>
                </ul>
                <div className="mt-4 text-center">
            ...
            <div className="mt-10">
              <Card>
                ...
                >
                  *Financial data is illustrative. Full integration with

...
const AgentPerformanceSkeleton = ({ darkMode, limit }) => (
  <div
    className={`overflow-x-auto animate-pulse ${darkMode ? "text-gray-300" : "text-gray-700"}`}
  >
          ...
          >
            {[...Array(9)].map((_, colIndex) => (
              <td key={colIndex} className="py-2 px-2">
                <div
                  className={`h-4 rounded ${darkMode ? "bg-gray-600" : "bg-gray-300"} ${colIndex === 1 ? "w-3/4" : "w-1/2"}`}
                ></div>
              </td>
            ))}
          </tr>
    ...
    <div className="flex justify-center items-center space-x-4 mt-4">
      <div
        className={`h-8 w-20 rounded-lg ${darkMode ? "bg-gray-700" : "bg-gray-100"}`}
      ></div>
      <div
        className={`h-6 w-24 rounded ${darkMode ? "bg-gray-600" : "bg-gray-300"}`}
      ></div>
      <div
        className={`h-8 w-20 rounded-lg ${darkMode ? "bg-gray-700" : "bg-gray-100"}`}
      ></div>
...
const AgentPerformance = () => {
  ...
  const [isDesktopFilterModalOpen, setIsDesktopFilterModalOpen] =
    useState(false);
  ...
  const fetchAgentPerformance = useCallback(async () => {
    ...
    try {
      const response = await axiosInstance.get(
        `${API_BASE_URL}/agent-performance/performance?${params.toString()}`,
        { headers },
      );
      ...
      let errorMessage =
        "Failed to fetch agent performance data. Please try again.";
      if (
        error.response &&
        error.response.data &&
  ...
  const performDeletePerformance = async (userId) => {
    ...
    try {
      await axiosInstance.delete(
        `${API_BASE_URL}/agent-performance/performance/${userId}`,
        {
          headers: {
      ...
      let errorMessage =
        "Failed to delete agent performance entry. Please try again.";
      if (
        error.response &&
        error.response.data &&
        error.response.data.message
  ...
  const handleExportCsv = async (scope) => {
    const dataToExport =
      scope === "current" ? filteredPerformance : performanceData;

    if (dataToExport.length === 0) {
      showMessage(
        `No agent performance data found for ${scope} export.`,
        "info",
      );
      setIsExportDropdownOpen(false);
      return;
    ...
    try {
      const csvRows = dataToExport.map((p) =>
        [
          ...
          p.client_feedback
            ? `"${String(p.client_feedback).replace(/"/g, '""')}"`
            : "N/A",
          p.region,
          p.commission_earned,
      ...
      let errorMessage =
        "Failed to export agent performance data to CSV. Please try again.";
      if (
        error.response &&
        error.response.data &&
        error.response.data.message
              ...
              <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
                ...
                <div className="flex gap-2 items-center">
                  <button
                    ...
                    onClick={() =>
                      navigate("/agent-performance/performance/add")
                    }
                  >
                    +Add Entry
                  </button>
                ...
                >
                  ...
                  >
                    {filteredPerformance.map((entry) => (
                      ...
                      >
                        <td className="py-2 px-2 max-w-[80px] truncate">
                          {entry.user_id}
                        </td>
                        <td
                          className="py-2 px-2 max-w-[120px] truncate"
                          title={
                            entry.full_name && entry.full_name.length > 15
                              ? entry.full_name
                              : ""
                          }
                        >
                        ...
                        <td className="py-2 px-2 max-w-[100px] truncate">
                          {entry.deals_closed}
                        </td>
                        <td
                          className="py-2 px-2 max-w-[120px] truncate"
                          title={
                            entry.revenue ? formatCurrency(entry.revenue) : ""
                          }
                        >
                        ...
                        <td className="py-2 px-2 max-w-[120px] truncate">
                          {entry.properties_assigned}
                        </td>
                        <td
                          className="py-2 px-2 max-w-[90px] truncate"
                          title={
                            entry.region && entry.region.length > 10
                              ? entry.region
                              : ""
                          }
                        >
                        ...
                        <td className="py-2 px-2 space-x-2 max-w-[120px]">
                          <div className="flex items-center gap-2">
                            <button
                              ...
                              onClick={() =>
                                navigate(
                                  `/agent-performance/performance/edit/${entry.user_id}`,
                                )
                              }
                              title="Edit Entry"
                            >
                            ...
                            <button
                              ...
                              onClick={() =>
                                handleDeletePerformance(entry.user_id)
                              }
                              title="Delete Entry"
                            >
          ...
          >
            <div className="flex justify-between items-center mb-6">
              ...
              <button
                ...
                className={`p-2 rounded-xl h-10 w-10 flex items-center justify-center ${darkMode ? "bg-gray-700 text-gray-200" : "bg-gray-200"}`}
              >
                <X size={24} />
              </button>
            </div>

...
const DocumentCardSkeleton = ({ darkMode }) => (
  ...
  >
    <div
      className={`w-full h-24 rounded-lg ${darkMode ? "bg-gray-600" : "bg-gray-300"} mb-3`}
    ></div>
    <div
...
const LegalDocuments = () => {
  ...
  const fetchDocuments = useCallback(async () => {
    ...
    if (
      ...
      statusFilter.toLowerCase() !== "all statuses"
    ) {
    ...
    try {
      const response = await axiosInstance.get(
        `${API_BASE_URL}/docs?${params.toString()}`,
        { headers },
      );
  ...
  const handleDeleteDocument = (documentId, publicId) => {
    showConfirm({
      ...
      message:
        "Are you sure you want to permanently delete this legal document? This action cannot be undone.",
  ...
  const handleExportCsv = async (scope) => {
    ...
    const csvRows = dataToExport.map((d) =>
      [
        ...
        d.completion_date
          ? new Date(d.completion_date).toLocaleDateString()
          : "N/A",
        d.document_url || "N/A",
        d.agent_name || "N/A",
        d.agency_name || "N/A",
                    ...
                    <AnimatePresence>
                      {showSearchBarFilters && (
                        ...
                        >
                          <div>
                            <label
                              htmlFor="document-type-filter-mobile"
                    ...
                    <AnimatePresence>
                      {showSearchBarFilters && (
                        ...
                        >
                          <div>
                            <label
                              htmlFor="document-type-filter-desktop"
                              className={`block text-sm font-medium mb-2 ${darkMode ? "text-gray-300" : "text-gray-700"}`}
                            >
                              Document Type
                            </label>
              ...
              viewMode === "graphical" ? (
                <div className="grid grid-cols-2 gap-6 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
                  {[...Array(currentLimit)].map((_, i) => (
                    <DocumentCardSkeleton key={i} darkMode={darkMode} />
                  ))}
                </div>
              ) : (
                ...
                >
                  ...
                  >
                    {filteredDocuments.map((doc) => (
                      ...
                      >
                        <td
                          className="py-1 px-1 sm:py-2 sm:px-2 max-w-[90px] truncate"
                          title={doc.document_id || ""}
                        >
                          {doc.document_id}
                        </td>
                        <td
                          className="py-1 px-1 sm:py-2 sm:px-2 max-w-[150px] truncate"
                          title={doc.title || ""}
                        >
                          {doc.title}
                        </td>
                        <td
                          className="py-1 px-1 sm:py-2 sm:px-2 max-w-[120px] truncate"
                        ...
                        <td
                          className={`py-1 px-1 sm:py-2 sm:px-2 max-w-[80px] truncate font-semibold ${
                          ...
                          title={capitalizeFirstLetter(doc.status)}
                        >
                        ...
                        >
                          {doc.completion_date
                            ? new Date(doc.completion_date).toLocaleDateString()
                            : "N/A"}
                        </td>
                        {user && user.role === "admin" && (
                          <td
                            className="py-1 px-1 sm:py-2 sm:px-2 max-w-[120px] truncate"
                            title={doc.agency_name || ""}
                          >
                            {doc.agency_name || "N/A"}
                          </td>
                        )}

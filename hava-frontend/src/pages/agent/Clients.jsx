...
const ClientCardSkeleton = ({ darkMode }) => (
  ...
  >
    <div className="flex items-center mb-4">
      <div
        className={`w-12 h-12 rounded-full ${darkMode ? "bg-gray-600" : "bg-gray-300"}`}
      ></div>
    ...
    <div
      className={`h-3 w-full rounded ${darkMode ? "bg-gray-600" : "bg-gray-300"} mb-2`}
    ></div>
    <div
      className={`h-3 w-5/6 rounded ${darkMode ? "bg-gray-600" : "bg-gray-300"} mb-4`}
    ></div>
...
const ClientTableRowSkeleton = ({ darkMode, userRole }) => (
  ...
  >
    ...
    <td className="px-1 py-2">
      <div
        className={`h-4 w-2/3 rounded ${darkMode ? "bg-gray-600" : "bg-gray-300"}`}
      ></div>
    </td>
...
const Clients = () => {
  ...
  const fetchClientsAndRequests = useCallback(async () => {
    ...
    try {
      ...
      if (userRole === "agency_admin" && agencyId) {
        ...
        clientsRes = await axios.get(
          `${API_BASE_URL}/clients/agency/${agencyId}/clients`,
          {
            headers: { Authorization: `Bearer ${token}` },
          },
        );
        ...
        clientsRes = await axios.get(
          `${API_BASE_URL}/clients/agent/${currentUserId}/clients`,
          {
            headers: { Authorization: `Bearer ${token}` },
          },
        );
      ...
      if (userRole === "agent") {
        const requestsRes = await axios.get(
          `${API_BASE_URL}/agents/${currentUserId}/connection-requests/incoming`,
          {
            headers: { Authorization: `Bearer ${token}` },
          },
        );
      ...
      showMessage(
        "Failed to fetch clients or requests. Please try again.",
        "error",
      );
    } finally {
      setLoading(false); // End loading
  ...
  const fetchConversationForClient = useCallback(
    async (clientIdToFetch, agentIdForConversation) => {
      ...
      try {
        ...
        const res = await fetch(
          `${API_BASE_URL}/inquiries/agent/${agentIdForConversation}/client/${clientIdToFetch}/conversation`,
          {
            headers: { Authorization: `Bearer ${token}` },
          },
        );
        if (res.ok) {
          ...
          if (data && data.conversation) {
            ...
            return {
              ...
              clientName:
                clients.find((c) => c.user_id === conv.client_id)?.full_name ||
                conv.client_full_name,
              clientEmail:
                clients.find((c) => c.user_id === conv.client_id)?.email ||
                conv.client_email,
              clientPhone:
                clients.find((c) => c.user_id === conv.client_id)?.phone ||
                conv.client_phone,
    ...
    [clients, showMessage],
  ); // Removed agentId from dependencies, now passed as param

  // Effect to filter and sort clients
  useEffect(() => {
    let currentData =
      activeTab === "your_clients" ? [...clients] : [...pendingRequests];
    ...
    currentData.sort((a, b) => {
      const aValue =
        activeTab === "your_clients"
          ? a[sortKey]
          : a[`client_${sortKey}`] || a[sortKey]; // Handle client_name, client_email for pending
      const bValue =
        activeTab === "your_clients"
          ? b[sortKey]
          : b[`client_${sortKey}`] || b[sortKey];
  ...
  const handleOpenChat = useCallback(
    async (clientToChat) => {
      if (!clientToChat || !clientToChat.user_id) {
        showMessage(
          "Client data is missing for chat. Please wait or refresh.",
          "info",
        );
        return;
      }
      ...
      const agentIdForConversation =
        userRole === "agency_admin" ? clientToChat.agent_id : currentUserId;

      if (!agentIdForConversation) {
        showMessage(
          "Agent ID not available for conversation. Please try again later.",
          "info",
        );
        return;
      }
      ...
      if (!conversationToOpen) {
        try {
          ...
          if (createRes.ok) {
            ...
            if (!conversation) {
              showMessage(
                "No conversation data returned from backend.",
                "error",
              );
              return;
            }
            ...
            conversationToOpen = {
              ...
              clientName:
                clients.find((c) => c.user_id === conversation.client_id)
                  ?.full_name || conversation.client_full_name, // Corrected to use conversation.client_id
              clientEmail:
                clients.find((c) => c.user_id === conversation.client_id)
                  ?.email || conversation.client_email, // Corrected to use conversation.client_id
              clientPhone:
                clients.find((c) => c.user_id === conversation.client_id)
                  ?.phone || conversation.client_phone, // Corrected to use conversation.client_id
              propertyTitle: conversation.property_title || "General Inquiry",
              messages: Array.isArray(conversation.messages)
                ? conversation.messages.map((msg) => ({
                    inquiry_id: msg.inquiry_id,
                    sender:
                      msg.sender_id === agentIdForConversation
                        ? "Agent"
                        : "Client", // Use agentIdForConversation here
      ...
      if (conversationToOpen.unreadCount > 0) {
        ...
        try {
          ...
          socket.emit("message_read", {
            ...
            role: userRole,
          }); // Use currentUserId and userRole
    ...
    [currentUserId, userRole, fetchConversationForClient, showMessage, clients],
  ); // Added userRole to dependencies
  ...
  const handleRemoveClient = async (clientId) => {
    ...
    showConfirm({
      ...
      message:
        "Are you sure you want to archive this client? You can restore them later from 'Archived Clients'.",
      onConfirm: async () => {
        const removed = clients.find((c) => c.user_id === clientId);
        setClients((prev) => prev.filter((c) => c.user_id !== clientId));
        setFilteredClients((prev) =>
          prev.filter((c) => c.user_id !== clientId),
        );

        try {
          const token = localStorage.getItem("token");
          await axios.delete(
            `${API_BASE_URL}/clients/agent/${currentUserId}/clients/${clientId}`,
            {
              // Use currentUserId
              headers: { Authorization: `Bearer ${token}` },
            },
  ...
  const handleToggleStatus = async (clientId, currentStatus) => {
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.put(
        `${API_BASE_URL}/clients/agent/${currentUserId}/clients/${clientId}/vip`,
        { status: newStatus },
        {
          // Use currentUserId
          headers: { Authorization: `Bearer ${token}` },
      ...
      setClients((prev) =>
        prev.map((c) =>
          c.user_id === clientId ? { ...c, client_status: newStatus } : c,
        ),
      );
      setFilteredClients((prev) =>
        prev.map((c) =>
  ...
  const contentShift = isMobile
    ? 0
    : isCollapsed
      ? 80
      : userRole === "agency_admin"
        ? 256
        : 256; // Assuming agency sidebar is also 256px when expanded

  const handleEditNote = useCallback(
    (clientId, currentNote) => {
  ...
  const handleSaveNote = useCallback(
    async (clientId) => {
      ...
      try {
        const token = localStorage.getItem("token");
        await axios.put(
          `${API_BASE_URL}/clients/agent/${currentUserId}/clients/${clientId}/note`,
          {
            // Use currentUserId
        ...
        setClients((prev) =>
          prev.map((c) =>
            c.user_id === clientId ? { ...c, notes: editedNoteContent } : c,
          ),
        );
    ...
    showConfirm({
      ...
      onConfirm: async () => {
        try {
          const token = localStorage.getItem("token");
          await axios.post(
            `${API_BASE_URL}/agents/${currentUserId}/connection-requests/${requestId}/accept-from-client`,
            {},
            {
              // Use currentUserId
              headers: { Authorization: `Bearer ${token}` },
          ...
          showMessage(
            `Failed to accept request: ${err.response?.data?.message || "Please try again."}`,
            "error",
          );
        }
      },
    ...
    showConfirm({
      ...
      onConfirm: async () => {
        try {
          const token = localStorage.getItem("token");
          await axios.post(
            `${API_BASE_URL}/agents/${currentUserId}/connection-requests/${requestId}/reject-from-client`,
            {},
            {
              // Use currentUserId
              headers: { Authorization: `Bearer ${token}` },
          ...
          showMessage(
            `Failed to reject request: ${err.response?.data?.message || "Please try again."}`,
            "error",
          );
        }
      },
        ...
        >
          ...
          <div className="hidden md:grid grid-cols-3 items-center gap-4 mb-6 max-w-[1344px] mx-auto">
            ...
            <input
              ...
              placeholder={
                userRole === "agency_admin"
                  ? "Search clients by name, email, phone, or agent..."
                  ...
                  <thead>
                    ...
                    >
                      <th
                        ...
                        style={{
                          width: userRole === "agency_admin" ? "15%" : "15%",
                        }}
                      >
                        Name
                      </th>
                      ...
                      <th
                        className="text-left py-2 px-1 whitespace-nowrap"
                        style={{
                          width: userRole === "agency_admin" ? "25%" : "30%",
                        }}
                      >
                        Actions
                      </th>
              ...
              <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                {paginatedData.map((request) => (
                  <ClientCard
                    ...
                    onToggleStatus={() =>
                      showMessage(
                        "Cannot toggle status for pending requests.",
                        "info",
                      )
                    }
                    onRemoveClient={() =>
                    ...
                    onEditNote={() =>
                      showMessage(
                        "Cannot edit notes for pending requests.",
                        "info",
                      )
                    }
                    onSaveNote={() => {}}
                    onCancelEdit={() => {}}
                    acceptAction={() =>
                      handleAcceptRequest(request.request_id, request.client_id)
                    }
                ...
                >
                  <thead>
                    ...
                    >
                      ...
                      <th
                        onClick={() => handleSortClick("client_email")}
                        className="cursor-pointer text-left py-2 px-1 whitespace-nowrap"
                        style={{ width: "20%" }}
                      >
                        Email {renderSortIcon("client_email")}
                      </th>
                      <th
                        className="text-left py-2 px-1 whitespace-nowrap"
                        style={{ width: "15%" }}
                      ...
                      <th
                        ...
                        style={{ width: "15%" }}
                      >
                        Requested At {renderSortIcon("created_at")}
                      </th>
                      <th
                        className="text-left py-2 px-1 whitespace-nowrap"
                        style={{ width: "25%" }}
                  ...
                  >
                    {paginatedData.map((request) => (
                      ...
                      >
                        <td className="px-1 py-2" title={request.client_name}>
                          {request.client_name}
                        </td>
                        <td className="px-1 py-2" title={request.client_email}>
                          {request.client_email}
                        </td>
                        <td className="px-1 py-2" title={request.client_phone}>
                        ...
                        >
                          <button
                            className="text-green-600 hover:border-green-700 p-1 border border-transparent"
                            onClick={() =>
                              handleAcceptRequest(
                                request.request_id,
                                request.client_id,
                          ...
                          <button
                            className="text-red-600 hover:border-red-700 p-1 border border-transparent"
                            onClick={() =>
                              handleRejectRequest(request.request_id)
                            }
                          ...
                          {request.client_phone && (
                            <button
                              onClick={() =>
                                handleCallClient(request.client_phone)
                              }
              ...
              >
                <thead>
                  ...
                  >
                    ...
                    <th
                      onClick={() => handleSortClick("client_status")}
                      className="cursor-pointer text-left py-2 px-1 whitespace-nowrap"
                      style={{ width: "10%" }}
                    >
                      Status {renderSortIcon("client_status")}
                    </th>
                    <th
                      className="text-left py-2 px-1 whitespace-nowrap"
                      style={{
                ...
                >
                  {paginatedData.map((client) => (
                    ...
                    >
                      <td className="px-1 py-2" title={client.full_name}>
                        {client.full_name}
                      </td>
                      <td className="px-1 py-2" title={client.email}>
                        {client.email}
                      </td>
                      <td className="px-1 py-2" title={client.phone}>
                        {client.phone || "N/A"}
                      </td>
                      {userRole === "agency_admin" && (
                        <td className="px-1 py-2" title={client.agent_name}>
                      ...
                      <td
                        ...
                        title={client.client_status || "regular"}
                      >
                      ...
                      >
                        ...
                        {userRole === "agent" && (
                          <>
                            ...
                            <button
                              onClick={() => handleRemoveClient(client.user_id)}
                              title="Remove client"

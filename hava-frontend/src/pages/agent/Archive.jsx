...
const ClientTableRowSkeleton = ({ darkMode }) => (
  ...
  >
    ...
    <td className="px-2 py-2 flex gap-2">
      <div
        className={`h-8 w-16 rounded ${darkMode ? "bg-gray-600" : "bg-gray-300"}`}
      ></div>
      <div
...
const InquiryCardSkeleton = ({ darkMode }) => (
  <div
    className={`p-4 rounded-xl shadow-md animate-pulse ${darkMode ? "bg-gray-700" : "bg-gray-100"}`}
  >
    <div className="flex items-center justify-between mb-2">
      ...
      <div
        className={`h-6 w-20 rounded-full ${darkMode ? "bg-gray-600" : "bg-gray-300"}`}
      ></div>
    </div>
    <div
      className={`h-4 w-3/4 rounded ${darkMode ? "bg-gray-600" : "bg-gray-300"} mb-1`}
    ></div>
    <div
      className={`h-4 w-2/3 rounded ${darkMode ? "bg-gray-600" : "bg-gray-300"} mb-1`}
    ></div>
    <div
      className={`h-4 w-full rounded ${darkMode ? "bg-gray-600" : "bg-gray-300"} mb-1`}
    ></div>
    <div
...
const InquiryTableRowSkeleton = ({ darkMode }) => (
  ...
  >
    <td className="py-2 px-2">
      <div className="flex items-center">
        <div
          className={`w-8 h-8 rounded-full mr-3 ${darkMode ? "bg-gray-600" : "bg-gray-300"}`}
        ></div>
        <div
          className={`h-4 w-24 rounded ${darkMode ? "bg-gray-600" : "bg-gray-300"}`}
        ></div>
      </div>
    </td>
    <td className="py-2 px-2">
      <div
        className={`h-4 w-32 rounded ${darkMode ? "bg-gray-600" : "bg-gray-300"}`}
      ></div>
    </td>
    <td className="py-2 px-2">
      <div
        className={`h-4 w-40 rounded ${darkMode ? "bg-gray-600" : "bg-gray-300"}`}
      ></div>
    </td>
    <td className="py-2 px-2">
      <div
    ...
    <td className="py-2 px-2">
      <div
        className={`h-4 w-20 rounded ${darkMode ? "bg-gray-600" : "bg-gray-300"}`}
      ></div>
    </td>
...
const Archive = () => {
  ...
  const [sortKeyInquiries, setSortKeyInquiries] = useState(
  ...
  const fetchArchivedClients = useCallback(async () => {
    ...
    try {
      const { data } = await axios.get(
        `${API_BASE_URL}/clients/agent/${agentId}/archived-clients`,
        {
          headers: { Authorization: `Bearer ${token}` },
        },
      );
  ...
  const fetchArchivedInquiries = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(
        `${API_BASE_URL}/inquiries/agent/archived?sort=${sortKeyInquiries}&direction=${sortDirectionInquiries}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        },
      );
  ...
  const handleRestoreClient = async (clientId) => {
    ...
    showConfirm({
      ...
      onConfirm: async () => {
        try {
          await axios.post(
            `${API_BASE_URL}/clients/agent/${agentId}/archived-clients/${clientId}/restore`,
            {},
            {
              headers: { Authorization: `Bearer ${token}` },
            },
  ...
  const handleDeleteClientPermanently = async (clientId) => {
    showConfirm({
      ...
      message:
        "Are you sure you want to permanently delete this archived client? This action cannot be undone.",
      onConfirm: async () => {
        const token = localStorage.getItem("token");
        if (!token) {
          showMessage(
        ...
        try {
          await axios.delete(
            `${API_BASE_URL}/clients/agent/${agentId}/archived-clients/${clientId}`,
            {
              headers: { Authorization: `Bearer ${token}` },
            },
          );
  ...
  const handleRestoreInquiry = async (conversationId) => {
    ...
    showConfirm({
      ...
      message:
        "Are you sure you want to restore this inquiry to your active list?",
      onConfirm: async () => {
        try {
          // Assuming an endpoint to unarchive an inquiry for the current agent
          await axios.put(
            `${API_BASE_URL}/inquiries/${conversationId}/restore-for-agent`,
            {},
            {
              headers: { Authorization: `Bearer ${token}` },
            },
          ...
          setInquiries((prev) =>
            prev.filter((inq) => inq.id !== conversationId),
          );
  ...
  const handleDeleteInquiry = async (conversationId, isReassignedFromMe) => {
    showConfirm({
      ...
      message: isReassignedFromMe
        ? "Are you sure you want to remove this reassigned chat from your archive? It will be permanently deleted if the client also deletes it."
        : "Are you sure you want to delete this inquiry from your archive? It will be permanently deleted if the client also deletes it.",
      onConfirm: async () => {
        const token = localStorage.getItem("token");
        if (!token) {
          showMessage(
        ...
        try {
          ...
          await axios.delete(
            `${API_BASE_URL}/inquiries/agent/delete-conversation/${conversationId}`,
            {
              headers: { Authorization: `Bearer ${token}` },
            },
          );
  ...
  const handleSendMessageToConversation = async (
    ...
    try {
      ...
      showMessage(
        "Failed to send message or unarchive chat. Please try again.",
        "error",
      );
    }
  };
  ...
  const handleSortClick = (key, type) => {
    if (type === "clients") {
      ...
      if (sortKeyClients === key) {
        setSortDirectionClients(
          sortDirectionClients === "asc" ? "desc" : "asc",
        );
      } else {
      ...
      if (sortKeyInquiries === key) {
        setSortDirectionInquiries(
          sortDirectionInquiries === "asc" ? "desc" : "asc",
        );
      } else {
  ...
  const renderSortIcon = (key, type) => {
    const currentSortKey =
      type === "clients" ? sortKeyClients : sortKeyInquiries;
    const currentSortDirection =
      type === "clients" ? sortDirectionClients : sortDirectionInquiries;
  ...
  const sortedClients = [...clients].sort((a, b) => {
    ...
    if (typeA === "string" && typeB === "string") {
      return sortDirectionClients === "asc"
  ...
  const sortedInquiries = [...inquiries].sort((a, b) => {
    ...
    if (typeof aValue === "string" || typeof bValue === "string") {
      return sortDirectionInquiries === "asc"
            ...
            loadingClients ? (
              <div className="overflow-x-auto">
                ...
                >
                  ...
                  >
                    {[...Array(5)].map((_, i) => (
                      <ClientTableRowSkeleton key={i} darkMode={darkMode} />
                    ))}
                  </tbody>
                </table>
              </div>
                ...
                >
                  ...
                  >
                    {sortedClients.map((client) => (
                      ...
                      >
                        <td
                          className="px-2 py-2 whitespace-nowrap max-w-[150px] truncate"
                          ...
                          {client.user_id ? (
                            <span
                              ...
                              onClick={(e) => {
                                e.stopPropagation();
                                navigate(
                                  `/agent/client-profile/${client.user_id}`,
                                );
                              }}
                            >
                              {client.full_name}
                        ...
                        <td
                          className="px-2 py-2 whitespace-nowrap max-w-[200px] truncate"
                          title={client.email}
                        >
                          {client.email}
                        </td>
                        ...
                        <td className="px-2 py-2 whitespace-nowrap flex items-center gap-2">
                          ...
                          <button
                            onClick={() =>
                              handleDeleteClientPermanently(client.user_id)
                            }
          ...
          loadingInquiries ? (
            isMobile ? (
              <div className="space-y-4">
                {[...Array(5)].map((_, i) => (
                  <InquiryCardSkeleton key={i} darkMode={darkMode} />
                ))}
              </div>
            ) : (
              <div className="overflow-x-auto">
                ...
                >
                  ...
                  >
                    {[...Array(5)].map((_, i) => (
                      <InquiryTableRowSkeleton key={i} darkMode={darkMode} />
                    ))}
                  </tbody>
                </table>
              </div>
                        ...
                        >
                          <div className="flex items-center justify-between mb-2">
                            <div className="flex items-center">
                              <img
                                src={
                                  inq.clientProfilePictureUrl ||
                                  `https://placehold.co/40x40/${darkMode ? "374151" : "E0F7FA"}/${darkMode ? "D1D5DB" : "004D40"}?text=${getInitial(inq.clientName)}`
                                }
                                alt="Client Profile"
                                className="w-12 h-12 rounded-full mr-4 object-cover cursor-pointer"
                                onError={(e) => {
                                  e.target.onerror = null;
                                  e.target.src = `https://placehold.co/40x40/${darkMode ? "374151" : "E0F7FA"}/${darkMode ? "D1D5DB" : "004D40"}?text=${getInitial(inq.clientName)}`;
                                }}
                                onClick={(e) => {
                                  e.stopPropagation();
                                ...
                                {inq.client_id ? (
                                  <span
                                    ...
                                    onClick={(e) => {
                                      e.stopPropagation();
                                      navigate(
                                        `/agent/client-profile/${inq.client_id}`,
                                      );
                                    }}
                                  >
                                    {inq.clientName}
                          ...
                          <div>
                            ...
                            >
                              ...
                              {inq.property_id && (
                                <button
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    navigate(`/listings/${inq.property_id}`);
                                  }}
                            ...
                            >
                              ...
                              <span
                                ...
                                onClick={(e) => {
                                  e.stopPropagation();
                                  if (inq.agent_id)
                                    navigate(`/agent-profile/${inq.agent_id}`);
                                }}
                              >
                                {inq.agent_name || "N/A"}
                              </span>
                            ...
                            {isReassignedFromMe &&
                              ...
                              inq.reassigned_at && (
                                <p
                                  className={`text-xs mb-1 ${darkMode ? "text-yellow-300" : "text-yellow-700"}`}
                                >
                                  <Tag
                                    size={12}
                                    className="inline-block mr-1"
                                  />
                                  Reassigned to {inq.agent_name} by{" "}
                                  ...
                                  {inq.reassigned_by_admin_id ? (
                                    <span
                                      ...
                                      onClick={(e) => {
                                        e.stopPropagation();
                                        navigate(
                                          `/agency-admin-profile/${inq.reassigned_by_admin_id}`,
                                        );
                                      }}
                                    >
                                      {inq.reassigned_by_admin_name}
                    ...
                    <thead>
                      ...
                      >
                        {[
                          ...
                          { key: "agent_name", label: "Archived By" },
                          { key: "reassigned_at", label: "Reassigned At" },
                          ...
                          <th
                            ...
                            onClick={
                              c.key !== "Actions"
                                ? () => handleSortClick(c.key, "inquiries")
                                : undefined
                            }
                    ...
                    >
                      {sortedInquiries.map((inq) => {
                        ...
                        return (
                          <tr
                            key={inq.id}
                            className={`border-t cursor-pointer max-w-full break-words ${darkMode ? "border-gray-700 hover:bg-gray-700" : "border-gray-200 hover:bg-gray-50"} ${isReassignedFromMe ? "opacity-60 bg-yellow-100 dark:bg-yellow-900" : ""}`}
                            onClick={() => handleViewConversation(inq)}
                          >
                            <td
                            ...
                            >
                              ...
                              {/* Closing div for flex items-center */}
                            </td>
                            <td
                              className="px-2 py-2 truncate"
                            ...
                            <td
                              className="px-2 py-2 truncate"
                              title={inq.lastMessage}
                            >
                              {inq.lastMessage || "No messages"}
                            </td>
                            ...
                            >
                              ...
                              {isReassignedFromMe &&
                                ...
                                inq.reassigned_at && (
                                  <p
                                    className={`text-xs ${darkMode ? "text-yellow-300" : "text-yellow-700"}`}
                                  >
                                    (from {inq.original_agent_name} by{" "}
                                    {/* Admin Name clickable */}
                                    {inq.reassigned_by_admin_id ? (
                                      <span
                                        className="cursor-pointer hover:underline"
          ...
          <AgentInquiryModal
            ...
            onDelete={() =>
              handleDeleteInquiry(
                ...
                selectedConversation.isReassignedFromMe,
              )
            }
            ...
            isReassignedForCurrentAgent={
              selectedConversation.isReassignedFromMe || false
            }
          />
        )}

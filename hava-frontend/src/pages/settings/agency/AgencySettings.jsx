...
const AgencySettingsSkeleton = ({ darkMode }) => (
  <div className="animate-pulse space-y-8">
    ...
    <div className="pb-6 mb-6 border-b border-gray-200 dark:border-gray-700">
      <div
        className={`h-8 w-1/3 rounded ${darkMode ? "bg-gray-700" : "bg-gray-300"} mb-5`}
      ></div>
      <div className="flex flex-col items-center mb-6">
        <div
          className={`w-32 h-32 rounded-full ${darkMode ? "bg-gray-600" : "bg-gray-300"}`}
        ></div>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {[...Array(6)].map((_, i) => (
          <div key={i} className="space-y-2">
            <div
              className={`h-4 w-1/4 rounded ${darkMode ? "bg-gray-600" : "bg-gray-300"}`}
            ></div>
            <div
              className={`h-10 w-full rounded-xl ${darkMode ? "bg-gray-600" : "bg-gray-300"}`}
            ></div>
          </div>
        ))}
      </div>
    ...
    <div className="pb-6 mb-6 border-b border-gray-200 dark:border-gray-700">
      ...
      <div className="space-y-4">
        {[...Array(2)].map((_, i) => (
          <div
            key={i}
            className={`flex items-center justify-between p-4 rounded-xl h-20 ${darkMode ? "bg-gray-700" : "bg-gray-100"}`}
          ></div>
        ))}
      </div>
    </div>
    ...
    <div className="pb-6 mb-6 border-b border-gray-200 dark:border-gray-700">
      ...
      ></div>
      <div
        className={`p-4 rounded-xl h-24 ${darkMode ? "bg-gray-700" : "bg-gray-100"}`}
      ></div>
    </div>
  </div>
);
...
function AgencySettings() {
  ...
  const fetchAgencyMembers = useCallback(async () => {
    ...
    try {
      const response = await axiosInstance.get(
        `/agencies/${user.agency_id}/agents`,
      );
  ...
  const fetchPendingRequests = useCallback(async () => {
    ...
    try {
      const response = await axiosInstance.get(
        `/agencies/${user.agency_id}/pending-requests`,
      );
  ...
  const handleUpdateAgencyInfo = async () => {
    ...
    try {
      ...
      showMessage(
        error.response?.data?.message || "Failed to update agency info.",
        "error",
      );
    } finally {
      ...
      onConfirm: async () => {
        try {
          await axiosInstance.put(
            `/agencies/approve-join-request/${requestId}`,
          );
      ...
      onConfirm: async () => {
        try {
          await axiosInstance.delete(
            `/agencies/${user.agency_id}/members/${agentId}`,
          );
  ...
  const handleDeleteAgency = () => {
    showConfirm({
      ...
      message:
        "This action is irreversible and will permanently delete all agency data, disconnect all agents, and revert your role. Are you absolutely sure?",
      onConfirm: () => {
        setTimeout(() => {
          showConfirm({
            ...
            message: (
              <div>
                ...
                <input
                  ...
                  className={`w-full p-2 mt-2 border rounded ${darkMode ? "bg-gray-700 border-gray-600" : ""}`}
                />
              </div>
            ),
            onConfirm: async () => {
              ...
              try {
                await axiosInstance.delete(
                  `/agencies/${user.agency_id}/admin-delete`,
                );
                showMessage(
                  "Agency deleted successfully. Please log in again.",
                  "success",
                );
                logout();
              } catch (error) {
  ...
  if (!user || (user.role !== "agency_admin" && user.role !== "admin"))
    return <p>You do not have permission to view these settings.</p>;
  if (!agencyInfo)
    return (
      <p>
        Could not load agency information. You may not be part of an agency.
      </p>
    );
    ...
    >
      ...
      <div className="pb-8 mb-8 border-b border-gray-200 dark:border-gray-700">
        <div className="flex items-center justify-between flex-wrap gap-4 mb-6">
          ...
          >
            <Landmark className="mr-3" /> Agency Overview
          </h3>
            ...
            {editingAgencyInfo ? (
              <>
                <button
                  ...
                  className={`px-4 py-1.5 rounded-full font-medium text-sm ${darkMode ? "bg-gray-700" : "bg-gray-200"}`}
                >
                  Cancel
                </button>
                <button
                  ...
                  className="px-4 py-1.5 font-medium rounded-full flex items-center text-sm bg-green-600 text-white disabled:opacity-50"
                >
                  {updatingAgency ? (
              ...
              <button
                ...
                className="px-4 py-1.5 rounded-full font-medium flex items-center text-sm bg-green-600 text-white"
              >
          ...
          {editingAgencyInfo ? (
            <div className="md:col-span-2 space-y-4">
              <input
                ...
                className={`block w-full text-sm ${darkMode ? "text-gray-300" : ""}`}
              />
              <input
                name="name"
            ...
            <>
              ...
              <InfoCard
                icon={<Globe size={18} />}
                label="Website"
                value={agencyInfo.website || "N/A"}
              />
              <InfoCard
                icon={<MapPin size={18} />}
                label="Address"
                value={agencyInfo.address || "N/A"}
              />
              <InfoCard
                icon={<FileText size={18} />}
                label="Description"
                value={agencyInfo.description || "N/A"}
              />
            </>
          )}
        </div>
      ...
      <div className="pb-6 mb-6 border-b border-gray-200 dark:border-gray-700">
        <h3
          className={`text-lg md:text-xl font-bold mb-4 flex items-center ${darkMode ? "text-gray-200" : "text-gray-800"}`}
        >
          <UserRoundCheck className="mr-3" /> Agency Members
        </h3>
        <div className="space-y-4">
          {agencyMembers.length > 0 ? (
            agencyMembers.map((member) => (
              <div
                key={member.user_id}
                className={`flex items-center justify-between p-4 rounded-xl border ${darkMode ? "border-gray-700 bg-gray-700" : "border-gray-200 bg-gray-50"}`}
              >
                <div>
                  <p className="font-semibold">
                    {member.full_name} ({member.agency_role})
                  </p>
                ...
                {member.user_id !== user.user_id && (
                  <button
                    onClick={() =>
                      handleRemoveAgent(member.user_id, member.full_name)
                    }
                    className={`px-3 py-1 rounded-full text-sm font-semibold ${darkMode ? "bg-red-600" : "bg-red-100 text-red-600"}`}
                  >
                    Remove
                  </button>
                )}
            ...
            <p className={darkMode ? "text-gray-400" : "text-gray-600"}>
              No other agents in your agency.
            </p>
          )}
        </div>
      </div>
      ...
      <div className="pb-6 mb-6 border-b border-gray-200 dark:border-gray-700">
        <h3
          className={`text-lg md:text-xl font-bold mb-4 flex items-center ${darkMode ? "text-gray-200" : "text-gray-800"}`}
        >
          <Hourglass className="mr-3" /> Pending Join Requests
        </h3>
        <div className="space-y-4">
      ...
      <div>
        <h3
          className={`text-lg md:text-xl font-bold mb-4 flex items-center text-red-500`}
        >
          <ShieldAlert className="mr-3" /> Danger Zone
        </h3>
        <div className="p-6 rounded-xl border-2 border-red-500 bg-red-50 dark:bg-red-950/20">
          <p className="mb-4 text-red-700 dark:text-red-300">
            Permanently delete your agency and all associated data. This action
            cannot be undone.
          </p>
          <button
            onClick={handleDeleteAgency}
            className="px-6 py-3 bg-red-600 text-white font-semibold rounded-xl flex items-center"
          >
...
const InfoCard = ({ icon, label, value }) => {
  ...
  return (
    <div
      className={`flex flex-col p-4 rounded-xl shadow-sm border ${darkMode ? "bg-gray-800 border-gray-700" : "bg-white border-gray-200"}`}
    >
      <label
        className={`text-sm font-medium mb-2 flex items-center space-x-2 ${darkMode ? "text-gray-300" : "text-gray-600"}`}
      >
        {icon} <span>{label}</span>
      </label>
      <p
        className={`text-base break-words ${darkMode ? "text-gray-100" : "text-gray-900"}`}
      >
        {value}
      </p>
    </div>

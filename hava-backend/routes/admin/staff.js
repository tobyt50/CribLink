...
router.get("/staff", async (req, res) => {
  ...
  if (search) {
    ...
    whereClaases.push(
      `(LOWER(sd.full_name) LIKE $${values.length} OR LOWER(sd.email) LIKE $${values.length})`,
    );
  }
  ...
  const orderByColumn = validSortColumns.includes(sort)
    ? `sd.${sort}`
    : "sd.start_date"; // Prefix with sd.
...
router.get("/staff/:id", async (req, res) => {
  ...
  try {
    ...
    res
      .status(501)
      .json({ message: `GET staff member ${staffId} not implemented` });
  } catch (err) {
...
router.put("/staff/:id", async (req, res) => {
  ...
  try {
    ...
    res
      .status(501)
      .json({ message: `UPDATE staff member ${staffId} not implemented` });
  } catch (err) {
...
router.delete("/staff/:id", async (req, res) => {
  ...
  try {
    ...
    res
      .status(501)
      .json({ message: `DELETE staff member ${staffId} not implemented` });
  } catch (err) {
...
router.put("/staff/:id/status", async (req, res) => {
  ...
  try {
    ...
    res
      .status(501)
      .json({
        message: `UPDATE staff member ${staffId} status not implemented`,
      });
  } catch (err) {
...
router.post("/staff/:id/reset-password", async (req, res) => {
  ...
  try {
    ...
    res
      .status(501)
      .json({
        message: `Reset password for staff member ${staffId} not implemented`,
      });
  } catch (err) {

...
router.post(
  ...
  inquiriesController.createGeneralInquiry,
);
...
router.post(
  "/message",
  authenticateToken,
  inquiriesController.sendMessageInquiry,
);
...
router.get(
  ...
  authenticateToken,
  authorizeRoles("agent", "admin", "agency_admin"),
  inquiriesController.getAllInquiriesForAgent,
);
...
router.get(
  ...
  authenticateToken,
  authorizeRoles("agent", "admin", "agency_admin"),
  inquiriesController.getArchivedInquiriesForAgent,
);

// NEW: PUT /inquiries/:conversationId/archive-for-agent - Archive an inquiry for the current agent
router.put(
  "/:conversationId/archive-for-agent",
  authenticateToken,
  authorizeRoles("agent", "admin", "agency_admin"),
  inquiriesController.archiveInquiryForAgent,
);

// NEW: PUT /inquiries/:conversationId/restore-for-agent - Restore an inquiry for the current agent
router.put(
  "/:conversationId/restore-for-agent",
  authenticateToken,
  authorizeRoles("agent", "admin", "agency_admin"),
  inquiriesController.restoreInquiryForAgent,
);

// NEW: DELETE /inquiries/:conversationId/permanently-delete - Permanently delete an inquiry (admin/agency_admin only)
router.delete(
  "/:conversationId/permanently-delete",
  authenticateToken,
  authorizeRoles("admin", "agency_admin"),
  inquiriesController.permanentlyDeleteInquiry,
);
...
router.get(
  "/agent/:agentId/client/:clientId/conversation",
  authenticateToken,
  authorizeRoles(["agent", "admin", "client", "agency_admin"]),
  inquiriesController.getAgentClientConversation,
);

// PUT /inquiries/agent/assign/:inquiryId — Assign an inquiry to an agent
router.put(
  "/agent/assign/:inquiryId",
  authenticateToken,
  authorizeRoles("agent", "admin", "agency_admin"),
...
router.put(
  "/agent/mark-read/:conversationId",
  authenticateToken,
  authorizeRoles("agent", "admin", "agency_admin"),
  inquiriesController.markMessagesAsRead,
);
...
router.put(
  "/agent/mark-opened/:conversationId",
  authenticateToken,
  authorizeRoles("agent", "admin", "agency_admin"),
  inquiriesController.markConversationOpened,
);

// PUT /inquiries/agent/mark-responded/:conversationId — Mark conversation as responded by agent
router.put(
  "/agent/mark-responded/:conversationId",
  authenticateToken,
  authorizeRoles("agent", "admin", "agency_admin"),
  inquiriesController.markConversationResponded,
);
...
router.delete(
  "/agent/delete-conversation/:conversationId",
  authenticateToken,
  authorizeRoles("agent", "admin", "agency_admin"),
  inquiriesController.deleteConversation,
);
...
router.get(
  "/agent/count/agent-responses",
  authenticateToken,
  authorizeRoles("agent", "admin", "agency_admin"),
  inquiriesController.countAgentResponses,
);
...
router.put(
  "/agency-admin/reassign/:conversationId",
  authenticateToken,
  authorizeRoles("agency_admin", "admin"),
  inquiriesController.reassignInquiry,
);

...
exports.getAgentProfile = async (req, res) => {
  ...
  console.log(
    "[agentController.getAgentProfile] Entering function. req.user:",
    req.user,
  );
  ...
  try {
    ...
    if (userResult.rows.length === 0) {
      console.warn(
        "[agentController.getAgentProfile] Agent/Agency Admin not found or user is not an agent/agency admin for ID:",
        agentId,
      );
      return res
        .status(404)
        .json({ message: "Agent or Agency Administrator not found." });
    }
    ...
    const agentPerformance =
      performanceResult.rows.length > 0 ? performanceResult.rows[0] : {};
    ...
    console.error(
      "[agentController.getAgentProfile] Error fetching agent profile details:",
      err,
    );
    res
      .status(500)
      .json({
        message: "Failed to fetch agent profile details.",
        error: err.message,
      });
  }
};
...
exports.sendConnectionRequestToClient = async (req, res) => {
  ...
  console.log(
    "[agentController.sendConnectionRequestToClient] Entering function. req.user:",
    req.user,
  );
  ...
  if (req.user.role !== "agent" && req.user.role !== "agency_admin") {
    ...
    console.warn(
      "[agentController.sendConnectionRequestToClient] Forbidden: User role is not agent or agency_admin:",
      req.user.role,
    );
    return res
      .status(403)
      .json({
        message:
          "Forbidden: Only agents or agency administrators can send connection requests to clients.",
      });
  }
  if (parseInt(clientId) === agentId) {
    console.warn(
      "[agentController.sendConnectionRequestToClient] Cannot send request to self.",
    );
    return res
      .status(400)
  ...
  try {
    ...
    const clientCheck = await db.query(
      `SELECT user_id FROM users WHERE user_id = $1 AND role = 'client'`,
      [clientId],
    );
    if (clientCheck.rows.length === 0) {
      console.warn(
        "[agentController.sendConnectionRequestToClient] Recipient is not a valid client:",
        clientId,
      );
      return res
        .status(404)
        .json({ message: "The specified recipient is not a valid client." });
    }
    ...
    if (existingAgentClientRelationship.rows.length > 0) {
      console.log(
        "[agentController.sendConnectionRequestToClient] Already connected with client:",
        clientId,
      );
      return res
        .status(200)
        .json({
          message: "You are already connected with this client.",
          status: "connected",
        });
    }
    ...
    if (existingRequest.rows.length > 0) {
      ...
      console.log(
        "[agentController.sendConnectionRequestToClient] Existing request found. Status:",
        reqStatus,
      );
      if (reqStatus === "pending") {
        if (existingRequest.rows[0].sender_id === agentId) {
          ...
          return res
            ...
            .json({
              message:
                "This client has already sent you a connection request. Check your incoming requests.",
    ...
    console.log(
      "[agentController.sendConnectionRequestToClient] Connection request sent successfully to client:",
      clientId,
    );
    res
      .status(201)
      .json({
        message: "Connection request sent successfully to client.",
        status: "pending_sent",
      });
  } catch (err) {
    ...
    res
      .status(500)
      .json({
        error: "Failed to send connection request to client.",
        details: err.message,
      });
  }
};
...
exports.getAgentIncomingRequests = async (req, res) => {
  ...
  console.log(
    "[agentController.getAgentIncomingRequests] Entering function. req.user:",
    req.user,
  );

  if (agentRole !== "agent" && agentRole !== "agency_admin") {
    // NEW: Allow agency_admin
    console.warn(
      "[agentController.getAgentIncomingRequests] Forbidden: User role is not agent or agency_admin:",
      agentRole,
    );
    return res
      .status(403)
      .json({
        message:
          "Forbidden: This endpoint is for agents and agency administrators only.",
      });
  }
  ...
  try {
    ...
    console.log(
      "[agentController.getAgentIncomingRequests] Fetched incoming requests count:",
      result.rows.length,
    );
...
exports.getAgentOutgoingRequests = async (req, res) => {
  ...
  const agentRole = req.user.role;
  console.log(
    "[agentController.getAgentOutgoingRequests] Entering function. req.user:",
    req.user,
  );
  ...
  if (agentRole !== "agent" && agentRole !== "agency_admin") {
    // NEW: Allow agency_admin
    console.warn(
      "[agentController.getAgentOutgoingRequests] Forbidden: User role is not agent or agency_admin:",
      agentRole,
    );
    return res
      .status(403)
  ...
  try {
    ...
    console.log(
      "[agentController.getAgentOutgoingRequests] Fetched outgoing requests count:",
      result.rows.length,
    );
    res.status(200).json(result.rows);
  } catch (err) {
...
exports.acceptConnectionRequestFromClient = async (req, res) => {
  ...
  const agentRole = req.user.role;
  console.log(
    "[agentController.acceptConnectionRequestFromClient] Entering function. req.user:",
    req.user,
  );
  ...
  if (agentRole !== "agent" && agentRole !== "agency_admin") {
    // NEW: Allow agency_admin
    console.warn(
      "[agentController.acceptConnectionRequestFromClient] Forbidden: User role is not agent or agency_admin:",
      agentRole,
    );
    return res
      .status(403)
      .json({
        message:
          "Forbidden: Only agents and agency administrators can accept connection requests.",
      });
  }
  ...
  try {
    ...
    if (requestUpdateResult.rows.length === 0) {
      ...
      console.warn(
        "[agentController.acceptConnectionRequestFromClient] Request not found, already accepted, or not pending:",
        requestId,
      );
      return res
        .status(404)
    ...
    const { sender_id: clientId, receiver_id: acceptedAgentId } =
      requestUpdateResult.rows[0];
    console.log(
      `[agentController.acceptConnectionRequestFromClient] Request ${requestId} updated to accepted. Client: ${clientId}, Agent: ${acceptedAgentId}`,
    );
    ...
    console.log(
      `[agentController.acceptConnectionRequestFromClient] Relationship added/updated in agent_clients for Agent: ${acceptedAgentId}, Client: ${clientId}`,
    );
...
exports.rejectConnectionRequestFromClient = async (req, res) => {
  ...
  const agentRole = req.user.role;
  console.log(
    "[agentController.rejectConnectionRequestFromClient] Entering function. req.user:",
    req.user,
  );
  ...
  if (agentRole !== "agent" && agentRole !== "agency_admin") {
    // NEW: Allow agency_admin
    console.warn(
      "[agentController.rejectConnectionRequestFromClient] Forbidden: User role is not agent or agency_admin:",
      agentRole,
    );
    return res
      .status(403)
      .json({
        message:
          "Forbidden: Only agents and agency administrators can reject connection requests.",
      });
  }
  ...
  try {
    ...
    if (result.rows.length === 0) {
      console.warn(
        "[agentController.rejectConnectionRequestFromClient] Request not found, already processed, or not pending:",
        requestId,
      );
      return res
        .status(404)
    ...
    console.log(
      "[agentController.rejectConnectionRequestFromClient] Connection request rejected successfully:",
      requestId,
    );
    res
      .status(200)

...
exports.addFavouriteProperty = async (req, res) => {
  ...
  if (!user_id) {
    return res
      .status(401)
      .json({
        message: "Authentication required to add a favourite property.",
      });
  }
  if (!property_id) {
  ...
  try {
    ...
    const propertyExists = await pool.query(
      "SELECT 1 FROM property_listings WHERE property_id = $1",
      [property_id],
    );
    ...
    if (existingFavourite.rows.length > 0) {
      return res
        .status(409)
        .json({ message: "Listing already in favourites." });
    }
    ...
    res
      .status(201)
      .json({ message: "Listing added to favourites successfully." });
  } catch (err) {
    console.error("Error adding favourite property:", err);
    res
      .status(500)
      .json({ error: "Internal server error adding favourite property." });
  }
};
...
exports.removeFavouriteProperty = async (req, res) => {
  ...
  if (!user_id) {
    return res
      .status(401)
      .json({
        message: "Authentication required to remove a favourite property.",
      });
  }
  ...
  try {
    ...
    if (deleteResult.rows.length === 0) {
      return res
        .status(404)
        .json({ message: "Favourite property not found or already removed." });
    }
    ...
    res
      .status(200)
      .json({ message: "Listing removed from favourites successfully." });
  } catch (err) {
    console.error("Error removing favourite property:", err);
    res
      .status(500)
      .json({ error: "Internal server error removing favourite property." });
  }
};
...
exports.getFavouriteProperties = async (req, res) => {
  ...
  if (requestedUserId && currentUserRole === "agent") {
    ...
    try {
      ...
      if (
        clientPrivacyRes.rows.length > 0 &&
        clientPrivacyRes.rows[0].share_favourites_with_agents
      ) {
        ...
        return res
          .status(403)
          .json({
            message: "Client has chosen not to share their favourite listings.",
          });
      }
    } catch (error) {
      console.error(
        "Error checking client privacy setting for properties:",
        error,
      );
      return res
        .status(500)
        .json({
          error: "Internal server error checking client privacy setting.",
        });
    }
  } else if (!currentUserId) {
    return res
      .status(401)
      .json({ message: "Authentication required to view favourites." });
  }
  ...
  try {
    ...
    const favouritesWithGallery = await Promise.all(
    ...
    res
      .status(500)
      .json({ error: "Internal server error fetching favourite properties." });
  }
};
...
exports.getFavouritePropertyStatus = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({
        error: "Internal server error checking favourite property status.",
      });
  }
};
...
exports.addFavouriteAgent = async (req, res) => {
  ...
  if (!user_id) {
    return res
      .status(401)
      .json({ message: "Authentication required to add a favourite agent." });
  }
  if (!agent_id) {
  ...
  try {
    const agentExists = await pool.query(
      "SELECT 1 FROM users WHERE user_id = $1 AND role = 'agent'",
      [agent_id],
    );
    ...
    res
      .status(201)
      .json({ message: "Agent added to favourites successfully." });
  } catch (err) {
    console.error("Error adding favourite agent:", err);
    res
      .status(500)
      .json({ error: "Internal server error adding favourite agent." });
  }
};
...
exports.removeFavouriteAgent = async (req, res) => {
  ...
  if (!user_id) {
    return res
      .status(401)
      .json({
        message: "Authentication required to remove a favourite agent.",
      });
  }
  ...
  try {
    ...
    if (deleteResult.rows.length === 0) {
      return res
        .status(404)
        .json({ message: "Favourite agent not found or already removed." });
    }
    ...
    res
      .status(200)
      .json({ message: "Agent removed from favourites successfully." });
  } catch (err) {
    console.error("Error removing favourite agent:", err);
    res
      .status(500)
      .json({ error: "Internal server error removing favourite agent." });
  }
};
...
exports.getFavouriteAgents = async (req, res) => {
  ...
  if (requestedUserId && currentUserRole === "agent") {
    // Agents can't view other agents' favourites
    return res
      .status(403)
      .json({
        message: "Forbidden: You cannot view other users' favourite agents.",
      });
  } else if (requestedUserId && currentUserRole === "admin") {
    // Admin can view any user's favourites
    targetUserId = requestedUserId;
  } else if (!currentUserId) {
    return res
      .status(401)
      .json({ message: "Authentication required to view favourite agents." });
  }
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "Internal server error fetching favourite agents." });
  }
};
...
exports.getFavouriteAgentStatus = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({
        error: "Internal server error checking favourite agent status.",
      });
  }
};
...
exports.addFavouriteClient = async (req, res) => {
  ...
  if (!user_id || (user_role !== "agent" && user_role !== "agency_admin")) {
    return res
      .status(403)
      .json({
        message: "Only agents and agency administrators can favourite clients.",
      });
  }
  if (!client_id) {
  ...
  try {
    const clientExists = await pool.query(
      "SELECT 1 FROM users WHERE user_id = $1 AND role = 'client'",
      [client_id],
    );
    ...
    res
      .status(201)
      .json({ message: "Client added to favourites successfully." });
  } catch (err) {
    console.error("Error adding favourite client:", err);
    res
      .status(500)
      .json({ error: "Internal server error adding favourite client." });
  }
};
...
exports.removeFavouriteClient = async (req, res) => {
  ...
  if (!user_id || (user_role !== "agent" && user_role !== "agency_admin")) {
    return res
      ...
      .json({
        message:
          "Only agents and agency administrators can remove favourite clients.",
      });
  }
  ...
  try {
    ...
    if (deleteResult.rows.length === 0) {
      return res
        .status(404)
        .json({ message: "Favourite client not found or already removed." });
    }
    ...
    res
      .status(200)
      .json({ message: "Client removed from favourites successfully." });
  } catch (err) {
    console.error("Error removing favourite client:", err);
    res
      .status(500)
      .json({ error: "Internal server error removing favourite client." });
  }
};
...
exports.getFavouriteClients = async (req, res) => {
  ...
  if (!user_id || (user_role !== "agent" && user_role !== "agency_admin")) {
    return res
      ...
      .json({
        message:
          "Only agents and agency administrators can view favourite clients.",
      });
  }
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "Internal server error fetching favourite clients." });
  }
};
...
exports.getFavouriteClientStatus = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({
        error: "Internal server error checking favourite client status.",
      });
  }
};
...
exports.addFavouriteAgency = async (req, res) => {
  ...
  if (!user_id) {
    return res
      .status(401)
      .json({ message: "Authentication required to add a favourite agency." });
  }
  if (!agency_id) {
  ...
  try {
    const agencyExists = await pool.query(
      "SELECT 1 FROM agencies WHERE agency_id = $1",
      [agency_id],
    );
    ...
    res
      .status(201)
      .json({ message: "Agency added to favourites successfully." });
  } catch (err) {
    console.error("Error adding favourite agency:", err);
    res
      .status(500)
      .json({ error: "Internal server error adding favourite agency." });
  }
};
...
exports.removeFavouriteAgency = async (req, res) => {
  ...
  if (!user_id) {
    return res
      .status(401)
      .json({
        message: "Authentication required to remove a favourite agency.",
      });
  }
  ...
  try {
    ...
    if (deleteResult.rows.length === 0) {
      return res
        .status(404)
        .json({ message: "Favourite agency not found or already removed." });
    }
    ...
    res
      .status(200)
      .json({ message: "Agency removed from favourites successfully." });
  } catch (err) {
    console.error("Error removing favourite agency:", err);
    res
      .status(500)
      .json({ error: "Internal server error removing favourite agency." });
  }
};
...
exports.getFavouriteAgencies = async (req, res) => {
  ...
  if (requestedUserId && currentUserRole === "agency_admin") {
    // Agency admins can't view other agency's favourites
    return res
      .status(403)
      .json({
        message: "Forbidden: You cannot view other users' favourite agencies.",
      });
  } else if (requestedUserId && currentUserRole === "admin") {
    // Admin can view any user's favourites
    targetUserId = requestedUserId;
    ...
    return res
      .status(401)
      .json({ message: "Authentication required to view favourite agencies." });
  }
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "Internal server error fetching favourite agencies." });
  }
};
...
exports.getFavouriteAgencyStatus = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({
        error: "Internal server error checking favourite agency status.",
      });
  }
};

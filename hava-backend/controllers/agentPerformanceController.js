...
const AgentPerformanceController = {
  ...
  addAgentPerformance: async (req, res) => {
    ...
    try {
      ...
      if (!user_id || !full_name) {
        return res
          .status(400)
          .json({ message: "User ID and full name are required." });
      }
      ...
      res
        .status(201)
        .json({
          message: "Agent performance entry added successfully",
          entry: result.rows[0],
        });
    } catch (error) {
      ...
      if (error.code === "23505") {
        ...
        return res
          .status(409)
          .json({
            message: "A performance entry for this user ID already exists.",
          });
      }
      res
        .status(500)
        .json({
          message: "Error adding agent performance entry",
          error: error.message,
        });
    }
  },
  ...
  updateAgentPerformance: async (req, res) => {
    ...
    try {
      ...
      if (result.rows.length === 0) {
        return res
          .status(404)
          .json({ message: "Agent performance entry not found." });
      }
      res
        .status(200)
        .json({
          message: "Agent performance entry updated successfully",
          entry: result.rows[0],
        });
    } catch (error) {
      ...
      res
        .status(500)
        .json({
          message: "Error updating agent performance entry",
          error: error.message,
        });
    }
  },
  ...
  deleteAgentPerformance: async (req, res) => {
    ...
    try {
      ...
      res
        .status(200)
        .json({ message: "Agent performance entry deleted successfully" });
    } catch (error) {
      console.error("Error deleting agent performance entry:", error);
      res
        .status(500)
        .json({
          message: "Error deleting agent performance entry",
          error: error.message,
        });
    }
  },

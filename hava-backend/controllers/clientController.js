...
const getClientsForAgent = async (req, res) => {
  ...
  if (
    ...
    return res
      .status(403)
      .json({
        message: "Forbidden: You are not authorized to view these clients.",
      });
  }
...
const getClientProfileDetails = async (req, res) => {
  ...
  try {
    ...
    if (agentClientResult.rows.length > 0) {
      ...
      if (requestingUserRole === "agent") {
        return res
          ...
          .json({
            message:
              "Forbidden: This client is not associated with your account.",
          });
      }
    }
    ...
    res
      .status(500)
      .json({
        error: "Internal error fetching client profile details.",
        details: err.message,
      });
  }
};
...
const updateClientPreferences = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({
        error: "Failed to update client preferences.",
        details: err.message,
      });
  }
};
...
const getRecommendedListings = async (req, res) => {
  ...
  if (
    ...
    (req.user.role === "agent" && parseInt(agentId) !== req.user.user_id)
  ) {
    return res
      .status(403)
      .json({
        message:
          "Forbidden: You are not authorized to view these recommendations.",
      });
  }
...
const getRecommendedListingsByAgentForClient = async (req, res) => {
  ...
  if (
    ...
    parseInt(clientId) !== requestingUserId
  ) {
    return res
      .status(403)
      .json({
  ...
  if (
    ...
    parseInt(agentId) !== requestingUserId
  ) {
    ...
    return res
      ...
      .json({
        message:
          "Forbidden: You can only view recommendations made by yourself.",
      });
  }
  if (requestingUserRole === "guest") {
    return res
      .status(401)
      .json({
        message: "Authentication required to view recommended listings.",
      });
  }
  ...
  try {
    ...
    console.error(
      "Error fetching recommended listings by agent for client:",
      err,
    );
    res
      .status(500)
      .json({
        error: "Failed to fetch recommended listings by agent for client.",
        details: err.message,
      });
  }
};
  ...
  try {
    ...
    if (result.rows.length === 0) {
      return res
        .status(200)
        .json({
          message: "Listing already recommended to this client by you.",
        });
    }
    res
      .status(201)
      .json({
        message: "Listing recommended successfully.",
    ...
    res
      .status(500)
      .json({
        error: "Failed to add recommended listing.",
        details: err.message,
      });
  }
};
...
const removeRecommendedListing = async (req, res) => {
  ...
  try {
    ...
    if (result.rows.length === 0) {
      return res
        .status(404)
        .json({ message: "Recommended listing not found." });
    }
    res
      .status(200)
      .json({
        message: "Recommended listing removed successfully.",
        removedPropertyId: result.rows[0].property_id,
      });
  } catch (err) {
    ...
    res
      .status(500)
      .json({
        error: "Failed to remove recommended listing.",
        details: err.message,
      });
  }
};
...
const sendConnectionRequestToAgent = async (req, res) => {
  ...
  if (req.user.role !== "client") {
    return res
      ...
      .json({
        message:
          "Forbidden: Only clients can send connection requests to agents.",
      });
  }
  ...
  try {
    ...
    const agentCheck = await db.query(
      `SELECT user_id FROM users WHERE user_id = $1 AND role = 'agent'`,
      [agentId],
    );
    if (agentCheck.rows.length === 0) {
      return res
        .status(404)
        .json({ message: "The specified recipient is not a valid agent." });
    }
      ...
      if (reqStatus === "pending") {
        if (request.sender_id === clientId) {
          ...
          return res
            ...
            .json({
              message:
                "This agent has already sent you a connection request. Check your incoming requests.",
    ...
    res
      .status(201)
      .json({
        message: "Connection request sent successfully to agent.",
        status: "pending_sent",
      });
  } catch (err) {
    ...
    res
      .status(500)
      .json({
        error: "Failed to send connection request to agent.",
        details: err.message,
      });
  }
};
...
const getConnectionStatus = async (req, res) => {
  ...
  if (
    ...
    return res
      ...
      .json({
        message:
          "Forbidden: You can only check connection status for your own account.",
      });
  }
  if (
    requestingUserRole === "agent" &&
    ...
    return res
      ...
      .json({
        message:
          "Forbidden: You can only check connection status for your own agent profile.",
      });
  }
  ...
  try {
    ...
    if (pendingRequest.rows.length > 0) {
      ...
      if (
        request.sender_id === parseInt(clientId) &&
        request.receiver_id === parseInt(agentId)
      ) {
        return res.status(200).json({ status: "pending_sent" });
      } else if (
        request.sender_id === parseInt(agentId) &&
        request.receiver_id === parseInt(clientId)
      ) {
...
const disconnectFromAgent = async (req, res) => {
  ...
  if (parseInt(clientId) !== requestingUserId || req.user.role !== "client") {
    return res
      .status(403)
      .json({
        message: "Forbidden: You can only disconnect yourself from an agent.",
      });
  }
  ...
  try {
    ...
    if (deleteAgentClientResult.rows.length === 0) {
      ...
      return res
        .status(404)
        .json({
          message: "Active connection not found between client and agent.",
        });
    }
    ...
    res
      .status(500)
      .json({
        error: "Failed to disconnect from agent.",
        details: err.message,
      });
  }
};
...
const getClientIncomingRequests = async (req, res) => {
  ...
  if (clientRole !== "client") {
    return res
      .status(403)
      .json({ message: "Forbidden: This endpoint is for clients only." });
  }
  ...
  try {
    ...
    res
      .status(500)
      .json({
        error: "Failed to fetch incoming connection requests.",
        details: err.message,
      });
  }
};
...
const getClientOutgoingRequests = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({
        error: "Failed to fetch outgoing connection requests.",
        details: err.message,
      });
  }
};
...
const acceptConnectionRequestFromAgent = async (req, res) => {
  ...
  if (clientRole !== "client") {
    return res
      .status(403)
      .json({
        message: "Forbidden: Only clients can accept connection requests.",
      });
  }
  ...
  try {
    ...
    if (requestUpdateResult.rows.length === 0) {
      ...
      return res
        ...
        .json({
          message:
            "Connection request not found, already accepted, or not pending.",
        });
    }

    const { sender_id: agentId, receiver_id: acceptedClientId } =
      requestUpdateResult.rows[0];
...
const rejectConnectionRequestFromAgent = async (req, res) => {
  ...
  if (clientRole !== "client") {
    return res
      .status(403)
      .json({
        message: "Forbidden: Only clients can reject connection requests.",
      });
  }
  ...
  try {
    ...
    if (result.rows.length === 0) {
      return res
        ...
        .json({
          message:
            "Connection request not found, already processed, or not pending.",
        });
    }
...
const sendEmailToClient = async (req, res) => {
  ...
  console.log(
    `Email from agent ${agentId} to client ${clientId}: Subject=${subject}, Message=${message}`,
  );
...
const addNoteToClient = async (req, res) => {
  ...
  if (parseInt(agentId) !== currentUserId || currentUserRole !== "agent") {
    return res
      ...
      .json({
        message:
          "Forbidden: You are not authorized to update this client's note.",
      });
  }
  ...
  try {
    ...
    if (result.rowCount === 0) {
      return res
        .status(404)
        .json({ message: "Client connection not found or not accepted." });
    }

    // Optional: Log activity
    await logActivity(
      `Agent ${agentId} updated note for client ${clientId}`,
      req.user,
      "client_note_update",
    );
...
const toggleVipFlag = async (req, res) => {
  ...
  if (parseInt(agentId) !== currentUserId || currentUserRole !== "agent") {
    return res
      ...
      .json({
        message:
          "Forbidden: You are not authorized to change this client's VIP status.",
      });
  }
  ...
  try {
    ...
    // Optional: Log activity
    await logActivity(
      `Agent ${agentId} changed client ${clientId} status to ${status}`,
      req.user,
...
const sendMessageToClient = async (req, res) => {
  ...
  try {
    console.log(
      `Simulating message from agent ${agentId} to client ${clientId}: ${message}`,
    );
...
const archiveClient = async (req, res) => {
  ...
  if (parseInt(agentId) !== currentUserId || currentUserRole !== "agent") {
    return res
      .status(403)
      .json({
        message: "Forbidden: You are not authorized to archive this client.",
      });
  }

  // Log incoming parameters for debugging
  console.log(
    `[archiveClient] Attempting to archive client: agentId=${agentId}, clientId=${clientId}`,
  );
  try {
    ...
    console.log(
      `[archiveClient] Insert into archived_clients result: rows=${insertArchiveResult.rows.length}`,
    );
    ...
    if (insertArchiveResult.rows.length === 0) {
      ...
      await db.query("ROLLBACK");
      console.warn(
        `[archiveClient] Client ${clientId} not found or not in an accepted relationship with agent ${agentId} for archiving.`,
      );
      return res
        .status(404)
        .json({
          message:
            "Client not found or not in an accepted relationship to be archived.",
        });
    }
    ...
    await db.query("COMMIT"); // Commit transaction
    console.log(
      `[archiveClient] Client ${clientId} archived and relationship disconnected successfully.`,
    );
    res
      .status(200)
      .json({
        message: "Client archived and relationship disconnected successfully.",
      });
  } catch (err) {
    await db.query("ROLLBACK"); // Rollback transaction on error
    ...
    res
      .status(500)
      .json({ error: "Failed to archive client.", details: err.message }); // Send error message to frontend
  }
};
...
const getArchivedClients = async (req, res) => {
  ...
  if (parseInt(agentId) !== currentUserId && currentUserRole !== "admin") {
    return res
      ...
      .json({
        message:
          "Forbidden: You are not authorized to view these archived clients.",
      });
  }
...
const restoreClient = async (req, res) => {
  ...
  if (parseInt(agentId) !== currentUserId || currentUserRole !== "agent") {
    return res
      .status(403)
      .json({
        message: "Forbidden: You are not authorized to restore this client.",
      });
  }
...
const deleteArchivedClient = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to permanently delete archived client" });
  }
};
...
const getClientsByAgencyId = async (req, res) => {
  ...
  try {
    ...
    if (performingUserRole === "agency_admin") {
      ...
      if (
        ...
        return res
          ...
          .json({
            message:
              "Forbidden: You are not authorized to view clients for this agency.",
          });
      }
    ...
    inquiryClientsResult.rows.forEach((client) => {
      if (!combinedClientsMap.has(client.user_id)) {
        // Only add if not already in connected clients
        combinedClientsMap.set(client.user_id, {
          ...client,
          hasUnreadMessagesFromClient: client.unread_messages_count > 0,
        });
    ...
    res
      .status(500)
      .json({
        message: "Server error fetching agency clients.",
        error: error.message,
      });
  }
};
...
const getClientPendingAgentRequests = async (req, res) => {
  ...
  if (
    ...
    return res
      .status(403)
      .json({
        message: "Forbidden: You can only view your own pending requests.",
      });
  }
  ...
  try {
    ...
    res
      .status(500)
      .json({
        error: "Failed to fetch pending connection requests.",
        details: err.message,
      });
  }
};

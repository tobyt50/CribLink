...
io.on("connection", (socket) => {
  ...
  socket.on("message_read", ({ conversationId, userId, role }) => {
    console.log(
      `👁️ User ${userId} (${role}) read messages in conversation ${conversationId}`,
    );
  ...
  socket.on("send_message", (messageData) => {
    console.log(
      `✉️ Broadcasting message to room: ${messageData.conversationId}`,
    );
...
// app.use(express.static(path.join(__dirname, '../../hava-frontend/build')));

// The "catchall" handler
// app.get('*', (req, res) => {
//   res.sendFile(path.join(__dirname, '../../hava-frontend/build/index.html'));
// });

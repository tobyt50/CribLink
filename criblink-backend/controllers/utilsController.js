// criblink-backend/controllers/utilsController.js
...
exports.reverseGeocode = async (req, res) => {
    ...
    if (!GEOCODING_API_KEY) {
        console.error('SERVER ERROR: OpenCage Geocoding API Key is not set on the backend.');
        // Do not expose the specific error to the client, just fail gracefully.
        return res.status(500).json({ message: 'Location service is currently unavailable.' });
    }
    ...
    try {
        ...
        if (response.data.results.length > 0) {
            ...
            res.status(404).json({ message: 'No location found for the provided coordinates.' });
        }
    } catch (error) {
        console.error('Error during OpenCage API call from backend utility:', error.message);

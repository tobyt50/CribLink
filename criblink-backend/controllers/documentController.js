...
exports.uploadLegalDocument = async (req, res) => {
  try {
    ...
    await db.query(
      ...
      [title, client_name, property_id, document_type, status, completion_date, documentUrl, publicId, agent_id, agency_id]
    );
  ...
  try {
    ...
    if (userRole === 'agency_admin') {
      if (!userAgencyId || documentAgencyId !== userAgencyId) {
        return res.status(403).json({ message: 'Forbidden: You can only delete documents associated with your agency.' });
      }
    } else if (userRole !== 'admin') {
        // This case should ideally not be hit if authorizeRoles is set correctly, but as a fallback
        return res.status(403).json({ message: 'Forbidden: You do not have permission to delete legal documents.' });
    }

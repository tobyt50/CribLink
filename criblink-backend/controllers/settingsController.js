...
const getAdminSettings = async (req, res) => {
    try {
        ...
        if (result.rows.length === 0) {
            ...
            const newResult = await pool.query('SELECT * FROM admin_settings WHERE id = 1');
...
const updateAdminSettings = async (req, res) => {
    const {
        default_list_view, sidebar_permanently_expanded, email_notifications,
        sms_notifications, in_app_notifications, sender_email, smtp_host,
        require_2fa, min_password_length, crm_integration_enabled, analytics_id,
        auto_approve_listings, enable_comments, maintenance_mode, database_backup_scheduled
    } = req.body;
    ...
    try {
        ...
        if (req.user) {
            await logActivity(`${req.user.full_name} updated admin settings.`, req.user, 'admin_settings_update');
        }
        res.status(200).json({ message: 'Admin settings updated successfully.', settings: result.rows[0] });
    } catch (error) {
        console.error('Error updating admin settings:', error);
        res.status(500).json({ message: 'Server error while updating admin settings.' });
    }
};
...
const clearApplicationCache = async (req, res) => {
    try {
        ...
        // Log activity only if req.user is available
        if (req.user) {
            await logActivity(`${req.user.full_name} cleared application cache.`, req.user, 'application_cache_clear');
        }
        res.status(200).json({ message: 'Application cache cleared successfully (simulated).' });
    } catch (error) {
...
const backupDatabase = async (req, res) => {
    try {
        ...
        // Log activity only if req.user is available
        if (req.user) {
            await logActivity(`${req.user.full_name} initiated a database backup.`, req.user, 'database_backup');
        }
        res.status(200).json({ message: 'Database backup initiated (simulated).' });
    } catch (error) {
        console.error('Error performing database backup:', error);
        res.status(500).json({ message: 'Server error while performing database backup.' });
    }
};
...
const viewErrorLogs = async (req, res) => {
    try {
        const simulatedLogs = [
            { timestamp: new Date(), level: 'ERROR', message: 'Failed to connect to external CRM service.', code: 'CRM-001' },
            { timestamp: new Date(Date.now() - 3600000), level: 'WARN', message: 'High load detected on API endpoint /listings.', code: 'PERF-002' },
            { timestamp: new Date(Date.now() - 7200000), level: 'ERROR', message: 'Database connection pool exhausted.', code: 'DB-003' },
        ];
        // Log activity only if req.user is available
        if (req.user) {
            await logActivity(`${req.user.full_name} viewed error logs.`, req.user, 'view_error_logs');
        }
        res.status(200).json({ message: 'Simulated error logs retrieved successfully.', logs: simulatedLogs });
    } catch (error) {
...
const getAgentSettings = async (req, res) => {
    ...
    try {
        const result = await pool.query('SELECT * FROM agent_settings WHERE user_id = $1', [userId]);
        ...
        res.status(500).json({ message: 'Server error while fetching agent settings.' });
    }
};
...
const updateAgentSettings = async (req, res) => {
    const userId = req.user.user_id;
    const {
        two_factor_enabled, email_notifications, in_app_notifications,
        new_inquiry_alert, ticket_update_alert, is_available,
        default_signature, auto_assign_inquiries, theme,
        default_list_view, sidebar_permanently_expanded, language
    } = req.body;
    ...
    try {
        ...
        if (result.rows.length === 0) {
            return res.status(500).json({ message: 'Failed to update or insert agent settings.' });
        }
        // Log activity only if req.user is available
        if (req.user) {
            await logActivity(`${req.user.full_name} updated their agent settings.`, req.user, 'agent_settings_update');
        }
        res.status(200).json({ message: 'Agent settings updated successfully.', settings: result.rows[0] });
    } catch (error) {
        console.error('Error updating/inserting agent settings:', error);
        res.status(500).json({ message: 'Server error while updating agent settings.' });
    }
};
...
const getClientSettings = async (req, res) => {
    const userId = req.user.user_id;
    try {
        const result = await pool.query('SELECT * FROM client_settings WHERE user_id = $1', [userId]);
        ...
        res.status(500).json({ message: 'Server error while fetching client settings.' });
    }
};
...
const updateClientSettings = async (req, res) => {
    const userId = req.user.user_id;
    const {
        email_notifications, in_app_notifications, new_listing_alert,
        price_drop_alert, favourite_update_alert, preferred_property_type,
        preferred_location, max_price_alert, theme, default_list_view,
        language, sidebar_permanently_expanded
    } = req.body;
    ...
    try {
        ...
        if (result.rows.length === 0) {
            return res.status(500).json({ message: 'Failed to update or insert client settings.' });
        }
        // Log activity only if req.user is available
        if (req.user) {
            await logActivity(`${req.user.full_name} updated their client settings.`, req.user, 'client_settings_update');
        }
        res.status(200).json({ message: 'Client settings updated successfully.', settings: result.rows[0] });
    } catch (error) {
        console.error('Error updating/inserting client settings:', error);
        res.status(500).json({ message: 'Server error while updating client settings.' });
    }
};

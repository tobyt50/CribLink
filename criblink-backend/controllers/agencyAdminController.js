...
exports.getAgencyAdminProfile = async (req, res) => {
    ...
    console.log('[agencyAdminController.getAgencyAdminProfile] Entering function. req.user:', req.user);
    ...
    if (!req.user) {
        console.warn('[agencyAdminController.getAgencyAdminProfile] Unauthorized: No user authenticated.');
    ...
    try {
        ...
        if (userResult.rows.length === 0) {
            console.warn('[agencyAdminController.getAgencyAdminProfile] Agency Admin not found or user is not an agency admin for ID:', adminId);
            return res.status(404).json({ message: 'Agency Administrator not found.' });
        }
        ...
        if (adminDetails.agency_id) {
            ...
            agentManagement.total_agents = parseInt(totalAgentsRes.rows[0].total_agents || 0);
            ...
            agentManagement.actions_performed = [
                { id: 1, description: 'Approved new member John Doe', timestamp: new Date().toISOString() },
                { id: 2, description: 'Published 3 new listings', timestamp: new Date(Date.now() - 86400000).toISOString() },
            ];
        }
        ...
        if (adminDetails.agency_id) {
            ...
            if (totalListingsRes.rows.length > 0) {
                listingsManagement.total_listings = parseInt(totalListingsRes.rows[0].total_listings || 0);
                listingsManagement.published_listings = parseInt(totalListingsRes.rows[0].published_listings || 0);
                listingsManagement.unpublished_listings = parseInt(totalListingsRes.rows[0].unpublished_listings || 0);
            }
            ...
            listingsManagement.top_performing_listings = [
                { property_id: 'top-1', title: 'Luxury Villa in Ikoyi', inquiries: 15, views: 500 },
                { property_id: 'top-2', title: 'Spacious Apartment VI', inquiries: 12, views: 450 },
            ];
        }
        ...
        console.error('[agencyAdminController.getAgencyAdminProfile] Error fetching agency admin profile details:', err);
        res.status(500).json({ message: 'Failed to fetch agency administrator profile details.', error: err.message });
    }
};

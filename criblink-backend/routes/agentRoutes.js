...
const {
  authenticateToken,
  authorizeRoles,
} = require("../middleware/authMiddleware"); // Ensure authenticateToken and authorizeRoles are imported
...
router.post(
  "/:agentId/connection-requests/send-to-client/:clientId",
  authenticateToken,
  authorizeRoles(["agent", "agency_admin"]),
  agentController.sendConnectionRequestToClient,
); // NEW: Added agency_admin
// Get incoming requests for an agent - only the agent or agency_admin can see their incoming requests
router.get(
  "/:agentId/connection-requests/incoming",
  authenticateToken,
  authorizeRoles(["agent", "agency_admin"]),
  agentController.getAgentIncomingRequests,
); // NEW: Added agency_admin
// Get outgoing requests from an agent - only the agent or agency_admin can see their outgoing requests
router.get(
  "/:agentId/connection-requests/outgoing",
  authenticateToken,
  authorizeRoles(["agent", "agency_admin"]),
  agentController.getAgentOutgoingRequests,
); // NEW: Added agency_admin
// Agent accepts a request from a client - only the agent or agency_admin can accept requests sent to them
router.post(
  "/:agentId/connection-requests/:requestId/accept-from-client",
  authenticateToken,
  authorizeRoles(["agent", "agency_admin"]),
  agentController.acceptConnectionRequestFromClient,
); // NEW: Added agency_admin
// Agent rejects a request from a client - only the agent or agency_admin can reject requests sent to them
router.post(
  "/:agentId/connection-requests/:requestId/reject-from-client",
  authenticateToken,
  authorizeRoles(["agent", "agency_admin"]),
  agentController.rejectConnectionRequestFromClient,
); // NEW: Added agency_admin

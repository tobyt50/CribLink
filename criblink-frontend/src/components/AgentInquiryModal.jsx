...
import {
  ...
  Mail,
  Tag,
} from "lucide-react"; // Added Tag icon
...
const AgentInquiryModal = ({
  ...
  useEffect(() => {
    if (isOpen && conversation?.id && !isReassignedForCurrentAgent) {
      // Only mark as read if not reassigned from current agent
      const roleToMarkAsRead =
        userRole === "agency_admin" ? "agency_admin" : "agent"; // Use 'agency_admin' role if applicable
      const token = localStorage.getItem("token");
      if (token) {
        // Only attempt to mark as read if a token exists
        fetch(
          `${API_BASE_URL}/inquiries/${roleToMarkAsRead}/mark-read/${conversation.id}`,
          {
            method: "PUT",
            headers: {
          ...
          .then((response) => {
            if (!response.ok) {
              console.error(
                "Failed to mark messages as read on modal open:",
                response.statusText,
              );
            } else {
              ...
              const currentUserId = JSON.parse(
                atob(token.split(".")[1]),
              ).userId; // Get current user's userId from token
              socket.emit("message_read", {
                conversationId: conversation.id,
                userId: currentUserId,
                role: roleToMarkAsRead,
              });
            }
          })
          .catch((error) =>
            console.error(
              "Error marking messages as read on modal open:",
              error,
            ),
          );
      }
  ...
  useEffect(() => {
      ...
      if (newMessage.conversationId === conversation.id) {
        setChatHistory((prev) => {
          ...
          const updatedHistory = prev.some(
            ...
            : [
                ...prev,
                {
                  ...newMessage,
                  sender:
                    newMessage.senderId === conversation.client_id
                      ? "Client"
                      : newMessage.senderId === conversation.agent_id
                        ? "Agent"
                        : "Agency Admin", // Distinguish senders
          ...
          if (
            ...
            // Only mark as read if not reassigned
            const roleToMarkAsRead =
              userRole === "agency_admin" ? "agency_admin" : "agent";
            const token = localStorage.getItem("token");
            if (token) {
              fetch(
                `${API_BASE_URL}/inquiries/${roleToMarkAsRead}/mark-read/${conversation.id}`,
                ...
                .then((response) => {
                  if (!response.ok)
                    console.error(
                      "Failed to mark messages as read on new message receipt:",
                      response.statusText,
                    );
                  else {
                ...
                .catch((error) =>
                  console.error(
                    "Error marking messages as read on new message receipt:",
                    error,
                  ),
                );
            }
    ...
    const handleReadAck = ({ conversationId, role }) => {
      ...
      if (conversationId === conversation.id && role === "client") {
        setChatHistory((prev) =>
          prev.map((msg) =>
            msg.sender === "Agent" || msg.sender === "Agency Admin"
              ? { ...msg, read: true }
              : msg,
          ),
        );
    ...
    isReassignedForCurrentAgent,
    userRole,
  ]); // Re-run effect if modal open state or conversation changes

  const handleSendMessage = async (e) => {
    e.preventDefault();
    if (!agentMessage.trim())
      return showMessage("Please enter a message.", "error");
    if (!canSendMessage)
      return showMessage(
        "You do not have permission to reply to this inquiry.",
        "warning",
      ); // Prevent sending message if not allowed
    ...
    const optimisticMessage = {
      ...
      sender_id:
        userRole === "agency_admin"
          ? conversation.agency_admin_id
          : conversation.agent_id, // Assuming the conversation object has agency_admin_id or agent_id
    };
  ...
  const handleViewClientProfile = () => {
    if (conversation?.client_id) {
      if (userRole === "agent")
        navigate(`/agent/client-profile/${conversation.client_id}`);
      else if (userRole === "agency_admin")
        navigate(`/agency/client-profile/${conversation.client_id}`);
    } else {
  ...
  const bubbleStyle = (sender) => {
    if (sender === "Agent" || sender === "Agency Admin") {
      // Both agent and agency admin messages appear on the right
      return darkMode
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black bg-opacity-50">
      <motion.div
        ...
        exit={{ opacity: 0, y: -20 }}
        className={`relative rounded-3xl p-6 shadow-xl w-full max-w-lg max-h-[90vh] flex flex-col overflow-hidden ${darkMode ? "bg-gray-800 text-gray-200" : "bg-white text-gray-900"}`}
      >
          ...
          <div className="relative" ref={optionsMenuRef}>
            ...
            {showOptionsMenu && (
              <motion.div
                ...
                animate={{ opacity: 1, y: 0 }}
                className={`absolute right-0 mt-2 w-48 rounded-md shadow-lg py-1 ${darkMode ? "bg-gray-700" : "bg-white"} ring-1 ring-black ring-opacity-5`}
              >
                <button
                  onClick={() => {
                    onDelete();
                  ...
                  className={`flex items-center w-full text-left px-4 py-2 text-sm ${darkMode ? "text-red-300 hover:bg-gray-600" : "text-red-700 hover:bg-gray-100"}`}
                  disabled={isReassignedForCurrentAgent}
                >
                  {" "}
                  {/* Disable delete if reassigned */}
        ...
        <div className="flex-shrink-0">
          <h2
            className={`text-2xl font-bold mb-2 ${darkMode ? "text-green-400" : "text-green-700"}`}
          >
            Chat with{" "}
              ...
              <span
                ...
                onClick={(e) => {
                  ...
                  if (userRole === "agent")
                    ...
                    navigate(
                      `/agency/client-profile/${conversation.client_id}`,
                    );
                }}
              >
          ...
          <div className="text-sm mb-2 space-y-2">
              ...
              {conversation.client_id ? (
                <span
                  ...
                  onClick={(e) => {
                    ...
                    if (userRole === "agent")
                      navigate(
                        `/agent/client-profile/${conversation.client_id}`,
                      );
                    else if (userRole === "agency_admin")
                      navigate(
                        `/agency/client-profile/${conversation.client_id}`,
            ...
            {conversation.clientEmail && (
              <p className="flex items-center">
                ...
                <a
                  ...
                  className={`hover:underline ${darkMode ? "text-gray-200" : "text-gray-900"}`}
                >
                  {conversation.clientEmail}
                </a>
                {isGuestConversation && (
                  <>
                    <button
                      onClick={() =>
                        handleCopyToClipboard(conversation.clientEmail)
                      }
                    ...
                    <a
                      href={`mailto:${conversation.clientEmail}`}
                      className={`ml-2 p-1 rounded-full ${darkMode ? "text-gray-200 hover:bg-gray-700" : "text-gray-900 hover:bg-gray-100"}`}
                    >
                      <Mail size={16} />
                    </a>
                  </>
            ...
            {conversation.clientPhone && (
              <p className="flex items-center">
                <strong>Phone:</strong>&nbsp;
                <a
                  href={`tel:${conversation.clientPhone}`}
                  className={`hover:underline ${darkMode ? "text-gray-200" : "text-gray-900"}`}
                >
                  {conversation.clientPhone}
                </a>
                {isGuestConversation && (
                  <>
                    <button
                      onClick={() =>
                        handleCopyToClipboard(conversation.clientPhone)
                      }
                      className={`ml-2 p-1 rounded-full ${darkMode ? "hover:bg-gray-700" : "hover:bg-gray-100"}`}
                    >
                      <Copy size={16} />
            ...
            {conversation.property_id && (
              <p className="flex items-center">
                ...
                <span
                  className="truncate max-w-[calc(100%-120px)]"
                ...
                <button
                  ...
                  className="ml-2 py-1 px-3 bg-blue-500 text-white rounded-xl text-xs"
                >
                  View
                </button>
              </p>
          ...
          {isReassignedForCurrentAgent &&
            ...
            conversation.reassigned_at && (
              <div
                className={`mt-2 p-2 rounded-xl ${darkMode ? "bg-yellow-900 text-yellow-300" : "bg-yellow-100 text-yellow-800"} flex items-center text-sm`}
              >
                ...
                <span>
                  ...
                  {conversation.reassigned_by_admin_id ? (
                    <span
                      ...
                      onClick={(e) => {
                        e.stopPropagation();
                        navigate(
                          `/agency-admin-profile/${conversation.reassigned_by_admin_id}`,
                        );
                      }}
                    >
                      {conversation.reassigned_by_admin_name}
                  ...
                  on {new Date(conversation.reassigned_at).toLocaleDateString()}
          ...
          {Object.entries(groupedChatHistory).map(([date, messagesForDate]) => (
            <div key={date}>
              ...
              {messagesForDate.map((entry, index) => (
                <div
                  key={entry.inquiry_id || index}
                  className={`flex items-end max-w-[80%] my-2 ${entry.sender === "Agent" || entry.sender === "Agency Admin" ? "self-end justify-end ml-auto" : "self-start justify-start mr-auto"}`}
                >
                  <div
                    className={`flex flex-col px-3 py-2 rounded-2xl shadow-md ${bubbleStyle(entry.sender)}`}
                  >
                    ...
                    <div className="flex justify-end items-center text-xs mt-1 opacity-70">
                      ...
                      {(entry.sender === "Agent" ||
                        entry.sender === "Agency Admin") && (
                        <span
                          className={`ml-1 ${entry.read ? "text-green-400" : "text-gray-400"}`}
                        >
                          {entry.read ? "✓✓" : "✓"}
                        </span>
                      )}
        ...
        {!isGuestConversation && (
          <form
            ...
            className="flex-shrink-0 flex items-center mt-2"
          >
            <textarea
              value={agentMessage}

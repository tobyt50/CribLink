...
const ReassignAgentModal = ({ isOpen, onClose, darkMode, conversationId, currentAssignedAgentId, onReassign }) => {
  ...
  useEffect(() => {
    ...
    const fetchAgents = async () => {
      ...
      try {
        ...
        const res = await fetch(`${API_BASE_URL}/agencies/${agencyId}/agents?role=agent`, {
        ...
        const currentAgent = data.find(agent => agent.user_id === currentAssignedAgentId);
        if (currentAgent) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        {loadingAgents ? (
          <p className={`${darkMode ? 'text-gray-400' : 'text-gray-600'} text-center`}>Loading agents...</p>
        ) : filteredAgents.length === 0 ? (
          <p className={`${darkMode ? 'text-gray-400' : 'text-gray-600'} text-center`}>
            {searchTerm ? 'No agents found matching your search.' : 'No agents available in your agency.'}
          </p>
        ) : (
          <div className="space-y-3 max-h-60 overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-gray-400 scrollbar-track-gray-200">
            {filteredAgents.map((agent) => {
              const isSelected = selectedAgent && selectedAgent.user_id === agent.user_id;
              return (
                <div
                  key={agent.user_id}
                ...
                >
                  ...
                  <span className={`ml-2 text-sm ${darkMode ? 'text-gray-300' : 'text-gray-500'}`}>{agent.email}</span>
                </div>
              );
            })}
          </div>

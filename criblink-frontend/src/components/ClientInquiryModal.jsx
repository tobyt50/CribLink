...
const ClientInquiryModal = ({
  ...
  useEffect(() => {
    if (isOpen && conversation?.id) {
      ...
      if (token) {
        ...
        fetch(
          `${API_BASE_URL}/inquiries/${clientRole}/mark-read/${conversation.id}`,
          {
            method: "PUT",
            headers: {
  ...
  useEffect(() => {
    ...
    const handleReadAck = ({ conversationId, role }) => {
      ...
      if (conversationId === conversation.id && role === "agent") {
        setChatHistory((prev) =>
          prev.map((msg) =>
            msg.sender === "Client" ? { ...msg, read: true } : msg,
          ),
        );
      }
    };
  ...
  const handleSendMessage = async (e) => {
    ...
    if (isGuest && (!guestName.trim() || !guestEmail.trim()))
      return showMessage("Please provide your name and email.", "error");
    ...
    if (isGuest)
      await onSendMessage(conversation?.id, clientMessage, {
        name: guestName,
        email: guestEmail,
        phone: guestPhone,
      });
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black bg-opacity-50">
      <motion.div
        ...
        exit={{ opacity: 0, y: -20 }}
        className={`relative rounded-3xl p-6 shadow-xl max-w-lg w-full h-auto max-h-[90vh] flex flex-col ${darkMode ? "bg-gray-800 text-gray-200" : "bg-white text-gray-900"}`}
      >
        ...
        <div className="flex-shrink-0">
          ...
          >
            {isGuest
              ? "New Inquiry"
              : `Chat with ${conversation?.agentName || "Agent"}`}
          </h2>
          ...
          {!isGuest && conversation?.agentEmail && (
            <p className="text-sm mb-2">
              <strong>Email:</strong>{" "}
              <a
                href={`mailto:${conversation.agentEmail}`}
          ...
          {!isGuest && conversation?.agentPhone && (
            <p className="text-sm mb-2">
              <strong>Phone:</strong>{" "}
              <a
                href={`tel:${conversation.agentPhone}`}
                className={`hover:underline ${darkMode ? "text-gray-200" : "text-gray-900"}`}
              >
          ...
          {isGuest && (
            <div
              className={`mb-4 p-4 rounded-xl shadow-inner ${darkMode ? "bg-gray-900 border-gray-700" : "bg-gray-50 border-gray-200"}`}
            >
              <p className="font-semibold mb-3">Please provide your details:</p>
              <input
                type="text"
              ...
              <input
                ...
                className={inputStyle.replace("mb-3", "mb-0")}
              />
            </div>
          )}
        </div>
        ...
        {!isGuest && (
          <div className="flex-grow overflow-y-auto border border-gray-300 dark:border-gray-700 rounded-xl p-4 mb-4 space-y-4 bg-opacity-30 backdrop-blur-sm h-[200px] scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-100 dark:scrollbar-thumb-gray-600 dark:scrollbar-track-gray-800">
            {Object.entries(groupedChatHistory).map(
                ...
                <div key={date}>
                  ...
                  {messagesForDate.map((entry, index) => (
                    <div
                      ...
                      className={`flex items-end max-w-[80%] my-2 ${entry.sender === "Client" ? "self-end justify-end ml-auto" : "self-start justify-start mr-auto"}`}
                    >
                      <div
        ...
        >
          <textarea
            ...
            className={`flex-grow p-3 border rounded-full resize-none focus:outline-none focus:ring-1 mr-2 ${darkMode ? "bg-gray-700 border-gray-600 focus:ring-green-400" : "bg-white border-gray-300 focus:ring-green-600"}`}
            style={{ minHeight: "48px", maxHeight: "120px" }}
          />
          <button
            type="submit"
            className="flex-shrink-0 bg-green-500 text-white rounded-full p-3 hover:bg-green-600 transition shadow-lg"
          >
            <Send size={20} />
          </button>
        </form>

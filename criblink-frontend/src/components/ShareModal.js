...
import { X, UserCheck, Share2, Copy, Facebook, Twitter, Search } from 'lucide-react';
import axiosInstance from './../api/axiosInstance';
import API_BASE_URL from './../config';
import { useMessage } from './../context/MessageContext';

const ShareModal = ({ isOpen, onClose, clients, listing, darkMode, currentAgentId, userRole }) => {
  const { showMessage } = useMessage();
  const [selectedClients, setSelectedClients] = useState([]);
  const [activeTab, setActiveTab] = useState(userRole === 'agent' ? 'recommend' : 'share');
  ...
  const handleRecommendListing = async () => {
    if (!listing || !listing.property_id || !currentAgentId || selectedClients.length === 0) {
    ...
    for (const client of selectedClients) {
      try {
        ...
        if (response.status === 201 || response.status === 200) { // Backend might return 200 if already recommended
          successCount++;
        } else {
          failCount++;
        ...
        console.error(`Error recommending to ${client.full_name}:`, error.response?.data || error.message);
        failCount++;
        failedClients.push(client.full_name);
      }
    }
    ...
    if (successCount > 0 && failCount === 0) {
      showMessage('Listing recommended to all selected clients successfully!', 'success');
    } else if (successCount > 0 && failCount > 0) {
      showMessage(`Recommended to ${successCount} clients. Failed for: ${failedClients.join(', ')}.`, 'warning', 7000);
    } else {
      showMessage('Failed to recommend listing to any selected client. Please try again.', 'error');
    }
    onClose(); // Close modal after attempting recommendations
  };

  const currentListingUrl = listing ? `${window.location.origin}/listings/${listing.property_id}` : window.location.href;
  const shareTitle = listing ? `Check out this amazing property: ${listing.title}!` : 'Check out this amazing property!';

  const handleCopyLink = () => {
    if (navigator.clipboard && navigator.clipboard.writeText) {
      navigator.clipboard.writeText(currentListingUrl).then(() => {
  ...
  const handleRowClick = (client) => {
    ...
    if (selectedClients.some((c) => c.user_id === client.user_id) && selectedClients.length === 1) {
      setSelectedClients([]);
    } else {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black bg-opacity-50">
      ...
      >
        ...
        {/* Tabs */}
        <div className={`flex mb-4 border-b ${darkMode ? 'border-gray-700' : 'border-gray-200'}`}>
          {userRole === 'agent' && (
            <button
              className={`flex-1 py-2 text-center font-semibold transition-colors duration-200 ${
                activeTab === 'recommend'
                  ? (darkMode ? 'text-green-400 border-b-2 border-green-400' : 'text-green-700 border-b-2 border-green-700')
            ...
            >
              <UserCheck size={18} className="inline-block mr-2" /> Recommend to Client
            </button>
          )}
          <button
            className={`flex-1 py-2 text-center font-semibold transition-colors duration-200 ${
        ...
        {activeTab === 'recommend' && userRole === 'agent' && (
          <div>
            {/* Search Input */}
            <div className={`relative mb-4 flex items-center gap-2 ${darkMode ? 'text-gray-300' : 'text-gray-600'}`}>
              <div className="relative flex-grow">
                ...
                <input
                  ...
                  className={`w-full pl-10 pr-4 py-2 rounded-xl focus:outline-none focus:ring-2 ${
                    darkMode ? 'bg-gray-700 focus:ring-green-500' : 'bg-gray-100 focus:ring-green-600'
                  }`}
                  value={searchTerm}
            ...
            {filteredClients.length === 0 ? (
              <p className={`${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>
                {searchTerm ? 'No clients found matching your search.' : 'You have no active clients to recommend this listing to.'}
              </p>
            ) : (
              <div className="space-y-3 max-h-60 overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-gray-400 scrollbar-track-gray-200">
                {filteredClients.map((client) => {
                  const isSelected = selectedClients.some((c) => c.user_id === client.user_id);
                  return (
                    <div
                      key={client.user_id}
                    ...
                    >
                      ...
                      <span className={`ml-2 text-sm ${darkMode ? 'text-gray-300' : 'text-gray-500'}`}>{client.email}</span>
                    </div>
                  );
                })}
              </div>
          ...
          <div className="space-y-4">
            ...
            >
             ...
             <p className={`text-sm text-center ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>
                (Direct sharing to Instagram posts is not supported via web links.)
            </p>
          </div>
        )}
      </motion.div>

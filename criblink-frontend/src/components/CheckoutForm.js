...
export default function CheckoutForm({ selectedPlan, currentUserPlan }) {
    ...
    const handleSubmit = async (e) => {
        ...
        if (error) {
            ...
            try {
                const updatePayload = { subscription_type: selectedPlan.name.toLowerCase() };
                const endpoint = user.role === 'agency_admin'
                ...
                updateUser(data.user || { ...user, subscription_type: selectedPlan.name.toLowerCase() });

                showMessage(successMessage, 'success');
                navigate('/agent/dashboard');
            } catch (dbError) {
                showMessage('Payment was successful, but we failed to update your account. Please contact support.', 'error');
            } finally {
                setIsLoading(false);
            }
        }

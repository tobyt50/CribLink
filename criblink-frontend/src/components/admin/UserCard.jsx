    ...
    >
      <div>
          ...
          <div className="flex-shrink-0 flex flex-col items-center">
            <img
              src={
                ...
                `https://placehold.co/112x112/${darkMode ? "374151" : "E0F7FA"}/${darkMode ? "D1D5DB" : "004D40"}?text=${user.full_name.charAt(0).toUpperCase()}`
              }
              alt="Profile"
              className="w-28 h-28 rounded-full object-cover border-2 border-green-500"
              onError={(e) => {
                e.target.onerror = null;
                e.target.src = `https://placehold.co/112x112/${darkMode ? "374151" : "E0F7FA"}/${darkMode ? "D1D5DB" : "004D40"}?text=${user.full_name.charAt(0).toUpperCase()}`;
              }}
            />
          </div>
            ...
            {user.subscription_type && (
              <div className="text-xs text-gray-500 dark:text-gray-400 mt-1 flex items-center">
                ...
                <span className="inline-flex items-center px-2 py-0.5 text-xs rounded-full bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-200 normal-case">
                  <StarIcon
                    className={`h-3 w-3 mr-1 ${getStarColor(user.subscription_type)}`}
                  />
        ...
        <div className="flex items-center justify-between w-full mb-2">
          <div
            className={`text-[10px] uppercase tracking-wider font-semibold ${darkMode ? "text-gray-400" : "text-gray-500"} flex items-center`}
          >
            {userRole
              ?.replace("_", " ")
      ...
      <div
        className="flex flex-nowrap justify-center w-full border-t border-gray-200 dark:border-gray-700 overflow-x-auto whitespace-nowrap px-0 items-center mt-auto pt-2"
        ...
        {actionOptions.map((option, index, arr) => (
          <React.Fragment key={option.value}>
            <button
              ...
              className={`flex-shrink-0 flex items-center justify-center h-8 w-8 rounded-full transition-colors duration-200 ${darkMode ? `hover:bg-gray-700 text-gray-300` : `hover:bg-gray-100 text-gray-600`} border border-transparent hover:border-current`}
              title={option.title}
            >

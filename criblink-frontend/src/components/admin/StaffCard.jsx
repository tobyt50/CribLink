...
const StaffCard = ({
  ...
  const getActionOptions = (currentStaff) => {
    ...
    options.push({
      value: currentStaff.status === "active" ? "suspend" : "activate",
      label: currentStaff.status === "active" ? "Suspend" : "Activate",
    });
  ...
  const profilePictureUrl =
    ...
    `https://placehold.co/112x112/${darkMode ? "374151" : "E0F7FA"}/${darkMode ? "D1D5DB" : "004D40"}?text=${staff.full_name.charAt(0).toUpperCase()}`;
    ...
    <Card className="px-4 pt-4 pb-2 flex flex-col justify-between min-h-[200px] max-w-md">
      ...
      <div className="flex flex-row-reverse items-start gap-4 mb-4">
        ...
        <div className="flex-shrink-0 flex flex-col items-center">
          <img
            ...
            onError={(e) => {
              ...
              e.target.src = `https://placehold.co/112x112/${darkMode ? "374151" : "E0F7FA"}/${darkMode ? "D1D5DB" : "004D40"}?text=${staff.full_name.charAt(0).toUpperCase()}`;
            }}
          />
        </div>
        ...
        <div className="flex-grow text-left min-w-0">
          ...
          <div className="text-xs text-gray-500 dark:text-gray-400 overflow-hidden text-ellipsis whitespace-nowrap">
            {" "}
            {/* Added text truncation classes */}
      ...
      <div className="flex items-center justify-between w-full mb-2">
        {/* Role on the left */}
        <div
          className={`text-[10px] uppercase tracking-wider font-semibold ${darkMode ? "text-gray-400" : "text-gray-500"}`}
        >
        ...
        <div className="w-28 text-center text-xs font-medium">
          <span
            className={
              staff.status === "active" ? "text-green-600" : "text-yellow-600" // Assuming 'suspended' or other non-active states would be yellow
            }
          >
            {(staff.status || "active").charAt(0).toUpperCase() +
              (staff.status || "active").slice(1)}
          </span>
        </div>
      </div>

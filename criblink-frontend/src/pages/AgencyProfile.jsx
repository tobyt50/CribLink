...
const AgencyProfile = () => {
  ...
  const [isCurrentUserAgencyAdmin, setIsCurrentUserAgencyAdmin] =
    useState(false);
  const [isCurrentUserAffiliatedAgent, setIsCurrentUserAffiliatedAgent] =
    useState(false);
  ...
  const [loadingCurrentUserMemberships, setLoadingCurrentUserMemberships] =
    useState(true);
  const [
    isCurrentUserLastAdminOfThisAgency,
    setIsCurrentUserLastAdminOfThisAgency,
  ] = useState(false);
  ...
  useEffect(() => {
    if (user && agency) {
      setIsCurrentUserAgencyAdmin(
        user.role === "agency_admin" && user.agency_id === parseInt(id),
      );
    } else {
  ...
  const fetchAgencyMembers = useCallback(async () => {
    ...
    try {
      ...
      const { data } = await axiosInstance.get(
        `${API_BASE_URL}/agencies/${id}/agents`,
        {
  ...
  const fetchPendingRequests = useCallback(async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const { data } = await axiosInstance.get(
        `${API_BASE_URL}/agencies/${id}/pending-requests`,
        {
          headers: { Authorization: `Bearer ${token}` },
        },
      );
  ...
  const fetchAgencyListings = useCallback(async () => {
    ...
    try {
      const { data } = await axiosInstance.get(
        `${API_BASE_URL}/agencies/${id}/listings`,
      );
      setAgencyListings(data);
    } catch (error) {
      console.error(
        "Error fetching agency listings:",
        error.response?.data || error.message,
      );
  ...
  const fetchCurrentUserAgencyMemberships = useCallback(async () => {
    ...
    try {
      ...
      const response = await axiosInstance.get(
        `${API_BASE_URL}/agencies/${user.user_id}/agency-memberships`,
        {
          headers: { Authorization: `Bearer ${token}` },
        },
      );
      ...
      if (user?.role === "agency_admin" && user?.agency_id === parseInt(id)) {
        try {
          ...
          console.error(
            "Error checking admin count for disconnect button:",
            error,
          );
          setIsCurrentUserLastAdminOfThisAgency(false);
        }
      ...
      console.error(
        "Error fetching current user agency memberships:",
        error.response?.data || error.message,
      );
  ...
  const checkFavoriteAgencyStatus = useCallback(async () => {
    if (user?.user_id && id && user?.role === "client") {
      ...
      try {
        const response = await axiosInstance.get(
          `${API_BASE_URL}/favourites/agencies/status/${id}`,
          {
            headers: { Authorization: `Bearer ${token}` },
          },
        );
      ...
      onConfirm: async () => {
        try {
          ...
          await axiosInstance.put(
            `${API_BASE_URL}/agencies/${id}/members/${memberId}/promote-to-admin`,
            {},
            {
              headers: { Authorization: `Bearer ${token}` },
            },
          ...
          showMessage(
            `${memberName} promoted to administrator successfully!`,
            "success",
          );
          ...
          showMessage(
            `Failed to promote ${memberName}: ${error.response?.data?.message || "Please try again."}`,
            "error",
          );
        }
      },
    ...
    showConfirm({
      ...
      onConfirm: async () => {
        try {
          const token = localStorage.getItem("token");
          await axiosInstance.put(
            `${API_BASE_URL}/agencies/${id}/members/${memberId}/demote-to-agent`,
            {},
            {
              headers: { Authorization: `Bearer ${token}` },
            },
          ...
          showMessage(
            `${memberName} demoted to agent successfully!`,
            "success",
          );
          fetchAgencyMembers(); // Refresh members list
          setShowMemberOptionsMenu(null);
          ...
          showMessage(
            `Failed to demote ${memberName}: ${error.response?.data?.message || "Please try again."}`,
            "error",
          );
        }
      },
    ...
    showConfirm({
      ...
      onConfirm: async () => {
        try {
          const token = localStorage.getItem("token");
          await axiosInstance.put(
            `${API_BASE_URL}/agencies/${id}/members/${memberId}/status`,
            { status },
            {
              headers: { Authorization: `Bearer ${token}` },
            },
          ...
          showMessage(
            `${memberName}'s status updated to ${status} successfully!`,
            "success",
          );
          fetchAgencyMembers(); // Refresh members list
          setShowMemberOptionsMenu(null);
          ...
          showMessage(
            `Failed to update ${memberName}'s status: ${error.response?.data?.message || "Please try again."}`,
            "error",
          );
        }
      },
    ...
    showConfirm({
      ...
      onConfirm: async () => {
        try {
          const token = localStorage.getItem("token");
          await axiosInstance.delete(
            `${API_BASE_URL}/agencies/${id}/members/${memberId}`,
            {
              headers: { Authorization: `Bearer ${token}` },
            },
          );
          showMessage(
            `${memberName} removed from agency successfully.`,
            "success",
          );
          fetchAgencyMembers(); // Refresh members list
          setShowMemberOptionsMenu(null);
          ...
          showMessage(
            `Failed to remove ${memberName}: ${error.response?.data?.message || "Please try again."}`,
            "error",
          );
        }
      },
    ...
    showConfirm({
      ...
      onConfirm: async () => {
        try {
          const token = localStorage.getItem("token");
          await axiosInstance.put(
            `${API_BASE_URL}/agencies/approve-join-request/${requestId}`,
            {},
            {
              headers: { Authorization: `Bearer ${token}` },
            },
          ...
          showMessage(
            `Failed to approve ${agentName}'s request: ${error.response?.data?.message || "Please try again."}`,
            "error",
          );
        }
      },
    ...
    showConfirm({
      ...
      onConfirm: async () => {
        try {
          const token = localStorage.getItem("token");
          await axiosInstance.put(
            `${API_BASE_URL}/agencies/reject-join-request/${requestId}`,
            {},
            {
              headers: { Authorization: `Bearer ${token}` },
            },
          ...
          showMessage(
            `Failed to reject ${agentName}'s request: ${error.response?.data?.message || "Please try again."}`,
            "error",
          );
        }
      },
  ...
  const handleRevertToAgent = async () => {
    ...
    if (user?.role === "agency_admin" && user?.agency_id === parseInt(id)) {
      try {
        ...
        if (adminCount <= 1) {
          showMessage(
            "You cannot step down as you are the last agency administrator. Please promote another agent to admin first.",
            "error",
          );
    ...
    try {
      ...
      console.error(
        "Error reverting role:",
        error.response?.data || error.message,
      );
      showMessage(
        `Failed to step down as admin: ${error.response?.data?.message || "Please try again."}`,
        "error",
      );
  ...
  const handleDisconnectFromThisAgency = async () => {
    if (!user?.user_id || !id) {
      showMessage(
        "You are not authorized to disconnect from this agency at this time.",
        "info",
      );
      return;
    }

    const currentAffiliation = currentUserMemberships.find(
    ...
    if (user?.role === "agency_admin" && user?.agency_id === parseInt(id)) {
      ...
      if (isCurrentUserLastAdminOfThisAgency) {
        showMessage(
          "You cannot disconnect as you are the last agency administrator. Please promote another agent to admin first.",
          "error",
        );
        return;
      }
          ...
          try {
            ...
            if (response.status === 200) {
              ...
              updateUser({
                ...
                agency: null,
                role: "agent",
              }); // Clear agency info from user context
              fetchCurrentUserAgencyMemberships(); // Re-fetch to ensure state consistency
            } else {
              showMessage(
                "Failed to disconnect from agency. Please try again.",
                "error",
              );
            }
          } catch (error) {
            console.error(
              "Error disconnecting from agency:",
              error.response?.data || error.message,
            );
            showMessage(
      ...
      onConfirm: async () => {
        try {
          ...
          if (response.status === 200) {
            ...
            showMessage(
              "Failed to cancel pending request. Please try again.",
              "error",
            );
          }
        } catch (error) {
          console.error(
            "Error canceling pending request:",
            error.response?.data || error.message,
          );
          showMessage(
            `Failed to cancel request: ${error.response?.data?.message || "Please try again."}`,
            "error",
          );
          fetchCurrentUserAgencyMemberships(); // Re-fetch to ensure state consistency
        }
  ...
  const handleRequestToJoin = async () => {
    if (!isAuthenticated || user?.role !== "agent") {
      showMessage(
        "You must be logged in as an agent to send a join request.",
        "info",
      );
      return;
    }
    ...
    if (hasAnyAffiliation) {
      showMessage(
        "You are already affiliated with an agency (connected or pending). An agent can only be affiliated with one agency at a time.",
        "info",
      );
      return;
    }
    ...
    showConfirm({
      ...
      onConfirm: async () => {
        try {
          ...
          showMessage(
            `Failed to send join request: ${error.response?.data?.message || "Please try again."}`,
            "error",
          );
          fetchCurrentUserAgencyMemberships(); // Re-fetch to ensure state consistency
        }
  ...
  const handleSaveEdit = async () => {
    showConfirm({
      title: "Save Changes",
      message:
        "Are you sure you want to save these changes to the agency details?",
      onConfirm: async () => {
        try {
          const token = localStorage.getItem("token");
          ...
          console.error(
            "Error updating agency details:",
            error.response?.data || error.message,
          );
          showMessage(
            `Failed to update agency details: ${error.response?.data?.message || "Please try again."}`,
            "error",
          );
        }
      },
  ...
  const duplicatedListings = useMemo(
    () =>
      isCarousel
        ? [...displayedListings, ...displayedListings, ...displayedListings]
        : displayedListings,
  ...
  useEffect(() => {
    if (
      isCarousel &&
      agencyListingsCarouselRef.current &&
  ...
  const isConnectedToThisAgency =
    currentUserAffiliationWithThisAgency?.request_status === "accepted";
  const isPendingWithThisAgency =
    currentUserAffiliationWithThisAgency?.request_status === "pending";
  const isRejectedByThisAgency =
    currentUserAffiliationWithThisAgency?.request_status === "rejected";
  ...
  const defaultLogo = `https://placehold.co/100x100/${darkMode ? "2D3748" : "E0F2F1"}/${darkMode ? "A0EC0" : "047857"}?text=${
    agency?.name
      ?.split(" ")
      .map((n) => n[0])
      .join("")
    ...
    >
      <div className="flex flex-col lg:flex-row gap-8 lg:max-w-7xl lg:mx-auto">
        ...
        >
          <div className="flex justify-between items-center flex-wrap mt-0 mb-1">
            ...
            >
              {loadingAgencyDetails ? (
                <div
                  className={`h-8 bg-gray-300 dark:bg-gray-700 rounded w-48 animate-pulse`}
                ></div>
              ) : (
                agency.name
              )}
          ...
          >
            <div className="flex items-center space-x-4">
              {loadingAgencyDetails ? (
                <div
                  className={`w-24 h-24 rounded-full animate-pulse ${darkMode ? "bg-gray-700" : "bg-gray-300"}`}
                ></div>
              ) : (
                <img
                  src={
                    editFormData.logoBase64
                      ? `data:image/png;base64,${editFormData.logoBase64}`
                      : agency.logo_url || defaultLogo
                  }
              ...
              <div className="flex flex-col">
                {loadingAgencyDetails ? (
                  <>
                    <div
                      className={`h-6 bg-gray-300 dark:bg-gray-700 rounded w-40 animate-pulse`}
                    ></div>
                    <div
                      className={`h-4 bg-gray-300 dark:bg-gray-700 rounded w-24 mt-2 animate-pulse`}
                    ></div>
                  </>
                ) : (
                  <>
            ...
            {loadingAgencyDetails ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {[...Array(6)].map((_, i) => (
                    <div key={i} className="space-y-2">
                      <div
                        className={`h-4 bg-gray-300 dark:bg-gray-700 rounded w-1/4 animate-pulse`}
                      ></div>
                      <div
                        className={`h-10 bg-gray-300 dark:bg-gray-700 rounded-xl w-full animate-pulse`}
                      ></div>
                    </div>
                  ))}
                  <div className="sm:col-span-2 space-y-2">
                    ...
                    ></div>
                    <div
                      className={`h-24 bg-gray-300 dark:bg-gray-700 rounded-xl w-full animate-pulse`}
                    ></div>
                  </div>
                  <div className="sm:col-span-2 space-y-2">
                    <div
              ...
              <div className="space-y-4 pt-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div className="sm:col-span-2">
                    <label htmlFor="logo" className={labelStyle}>
                      Agency Logo
                    </label>
                    <input
                      type="file"
              ...
              >
                ...
                {agency.website && (
                  <p className="flex items-center gap-2">
                    <Globe size={18} /> <strong>Website:</strong>{" "}
                    <a
                      href={agency.website}
                      target="_blank"
                ...
                {agency.description && (
                  <div className="sm:col-span-2 space-y-2 pt-4">
                    <h2
                      className={`text-xl font-bold ${darkMode ? "text-green-400" : "text-green-700"} flex items-center gap-2`}
                    >
            ...
            {loadingAgencyDetails ? (
              <div
                className={`h-10 bg-gray-300 dark:bg-gray-700 rounded-full w-48 animate-pulse mt-6`}
              ></div>
            ) : (
            ...
            {isAuthenticated &&
              user?.role !== "client" &&
              loadingCurrentUserMemberships && (
                <div
                  className={`mt-6 p-4 rounded-xl border ${darkMode ? "border-gray-700 bg-gray-700" : "border-gray-50"} flex items-center justify-center`}
                >
              ...
              !loadingCurrentUserMemberships && (
                <>
                  {isConnectedToThisAgency && (
                    <div
                      className={`mt-6 p-4 rounded-xl ${darkMode ? "bg-green-800/20 text-green-200 border-green-700" : "bg-green-100 text-green-800 border-green-200"} flex items-center justify-between gap-2`}
                    >
                      <div className="flex items-center gap-2">
                        <CheckCircle size={20} /> You are{" "}
                        {user?.role === "agency_admin"
                          ? "an administrator"
                          : "an agent"}{" "}
                        affiliated with this agency.
                      </div>
                      <button
                        ...
                        title={
                          user?.role === "agency_admin" &&
                          isCurrentUserLastAdminOfThisAgency
                            ? "Cannot disconnect: You are the last agency administrator."
                            : user?.role === "agency_admin"
                              ? "Step down as admin and disconnect"
                              : "Disconnect"
                        }
                        disabled={
                          user?.role === "agency_admin" &&
                      ...
                      >
                        <UserX size={16} className="inline-block mr-1" />{" "}
                        Disconnect
                      </button>
                    </div>
                  )}
                  ...
                  {isPendingWithThisAgency && (
                    <div
                      className={`mt-6 p-4 rounded-xl ${darkMode ? "bg-yellow-800/20 text-yellow-200 border-yellow-700" : "bg-yellow-100 text-yellow-800 border-yellow-200"} flex items-center justify-between gap-2`}
                    >
                      <div className="flex items-center gap-2">
                        <Hourglass size={20} className="animate-pulse" /> Your
                        request to join this agency is pending.
                      </div>
                      <button
                  ...
                  {isRejectedByThisAgency && (
                    <div
                      className={`mt-6 p-4 rounded-xl ${darkMode ? "bg-red-800/20 text-red-200 border-red-700" : "bg-red-100 text-red-800 border-red-200"} flex items-center justify-between gap-2`}
                    >
                      <div className="flex items-center gap-2">
                        <UserX size={20} /> Your request to join this agency was
                        rejected.
                      </div>
                      <button
            ...
            >
              ...
              {agencyMembers.length > 0 ? (
                <ul className="space-y-3">
                  {agencyMembers.slice(0, 3).map(
                    (
                      member, // Show max 3 members
                    ) => (
                      <li
                        key={member.user_id}
                        className={`flex items-center justify-between p-3 rounded-lg shadow-sm ${darkMode ? "bg-gray-700" : "bg-gray-100"}`}
                      >
                        <div className="flex items-center space-x-3">
                          <img
                            src={
                              member.profile_picture_url ||
                              `https://placehold.co/50x50/${darkMode ? "4A5568" : "CBD5E0"}/${darkMode ? "E2E8F0" : "4A5568"}?text=${member.full_name
                                .split(" ")
                                .map((n) => n[0])
                                .join("")
                                .toUpperCase()}`
                          ...
                          <div>
                            ...
                            <div className="flex items-center gap-2 text-xs">
                              <span
                                className={`px-2 py-0.5 rounded-full ${member.agency_role === "admin" ? "bg-purple-500 text-white" : "bg-blue-500 text-white"}`}
                              >
                              ...
                              <span
                                className={`px-2 py-0.5 rounded-full ${member.member_status === "vip" ? "bg-yellow-500 text-white" : "bg-gray-500 text-white"}`}
                              >
                                ...
                                >
                                  ...
                                  {member.member_status !== "vip" && (
                                    <button
                                      ...
                                      role="menuitem"
                                    >
                                      <Star size={16} className="mr-2" /> Set as
                                      VIP
                                    </button>
                                  )}
          ...
          {(isCurrentUserAgencyAdmin || user?.role === "admin") && (
            ...
            >
              ...
              {pendingRequests.length > 0 ? (
                <ul className="space-y-3">
                  {pendingRequests.slice(0, 3).map(
                    (
                      request, // Show max 3 pending requests
                    ) => (
                      <li
                      ...
                      >
                        <div className="flex items-center space-x-3">
                          <img
                            src={
                              request.agent_profile_picture_url ||
                              `https://placehold.co/50x50/${darkMode ? "4A5568" : "CBD5E0"}/${darkMode ? "E2E8F0" : "4A5568"}?text=${request.agent_name
                                .split(" ")
                                .map((n) => n[0])
                                .join("")
                                .toUpperCase()}`
        ...
        >
            ...
            {loadingAgencyDetails ? (
              <div className="flex flex-wrap justify-center gap-3 pt-2">
                {[...Array(3)].map((_, i) => (
                  <div
                    key={i}
                    className={`h-16 w-24 rounded-xl animate-pulse ${darkMode ? "bg-gray-700" : "bg-gray-300"}`}
                  ></div>
                ))}
              </div>
            ) : (
          ...
          >
            ...
            {loadingAgencyDetails ? (
              <div
                className={`block w-full h-48 rounded-xl animate-pulse ${darkMode ? "bg-gray-700" : "bg-gray-300"}`}
              ></div>
            ) : (
              <a
      ...
      {agencyListings.length > 0 && (
        <motion.div
          ...
          onTouchStart={
            isCarousel && !isMobileView ? handleTouchStart : undefined
          }
          onTouchEnd={isCarousel && !isMobileView ? handleTouchEnd : undefined}
        >
          <h2
            className={`text-xl md:text-2xl font-bold text-center py-0 mb-4 flex items-center justify-center gap-1 md:gap-3 ${
          ...
          <div
            className={`${isMobileView ? "grid grid-cols-2 gap-4 p-4" : "flex pb-4 -mb-4 overflow-x-scroll no-scrollbar"}`}
          >
            {(isMobileView ? displayedListings : duplicatedListings).map(
              (listing, index) => (
                <div

...
const ClientInquiries = () => {
      ...
      setGroupedConversations(prev => {
        ...
        const updatedConversations = prev.map(conv => {
          if (conv.id === newMessage.conversationId) {
            ...
            const updatedConv = {
              ...conv,
              messages: [...conv.messages, { ...newMessage, sender: newMessage.senderId === conv.agent_id ? 'Agent' : 'Client' }],
              lastMessage: newMessage.message,
              lastMessageTimestamp: newMessage.timestamp,
              lastMessageSenderId: newMessage.senderId,
              // Increment unread count only if the message is from the agent AND client is NOT the sender
              unreadCount: (newMessage.senderId === conv.agent_id && newMessage.senderId !== clientUserId) ? conv.unreadCount + 1 : conv.unreadCount,
            };
        ...
        return updatedConversations.sort((a, b) => new Date(b.lastMessageTimestamp) - new Date(a.lastMessageTimestamp));
      });
    };
      ...
      if (readerId === clientUserId && role === 'client') {
        setGroupedConversations(prev => prev.map(conv => {
          if (conv.id === conversationId) {
            return { 
                ...conv, 
                messages: conv.messages.map(msg => msg.sender_id === conv.agent_id ? { ...msg, read: true } : msg),
        ...
        setGroupedConversations(prev => prev.map(conv => {
          if (conv.id === conversationId) {
            return { 
                ...conv, 
                messages: conv.messages.map(msg => msg.sender_id === clientUserId ? { ...msg, read: true } : msg)
            };
          }
          return conv;
        }));
  ...
  }, [groupedConversations, clientUserId, fetchInquiries, openedConversationId, selectedConversationId, selectedConversation]);
  ...
  const handleViewConversation = useCallback(async (conversation) => {
    ...
    if (conversation.unreadCount > 0) {
      const token = localStorage.getItem('token');
      try {
        await fetch(`${API_BASE_URL}/inquiries/client/mark-read/${conversation.id}`, { method: 'PUT', headers: { 'Authorization': `Bearer ${token}` } });
        socket.emit('message_read', { conversationId: conversation.id, userId: clientUserId, role: 'client' });
  ...
  const handleSendMessageToConversation = useCallback(async (conversationId, messageText, guestDetails = {}) => {
    const token = localStorage.getItem('token');
    ...
    if (!conversationId) {
        ...
        try {
            ...
            showMessage('Failed to send initial inquiry due to network error.', 'error');
        }
    } else {
        ...
        try {
            await fetch(`${API_BASE_URL}/inquiries/client/mark-responded/${conversationId}`, {
                method: 'PUT',
                headers: { 'Authorization': `Bearer ${token}` }
            });
  ...
  return (
    <div className={`${darkMode ? "bg-gray-900" : "bg-gray-50"} pt-0 -mt-6 px-4 md:px-0 min-h-screen flex flex-col`}>
      ...
      <motion.div key={isMobile ? 'mobile' : 'desktop'} animate={{ marginLeft: contentShift }} transition={{ duration: 0.3 }} className="pt-6 px-4 md:px-8 flex-1 overflow-auto min-w-0">
        <div className="flex items-center justify-center mb-4 md:hidden"><h1 className={`text-2xl font-extrabold ${darkMode ? "text-green-400" : "text-green-700"}`}>Inquiries</h1></div>
        <div className="hidden md:block mb-6"><h1 className={`text-3xl font-extrabold text-center mb-6 ${darkMode ? "text-green-400" : "text-green-700"}`}>Inquiries</h1></div>

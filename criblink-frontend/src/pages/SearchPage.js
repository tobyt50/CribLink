...
function SearchPage() {
  ...
  const fetchResults = useCallback(async () => {
    ...
    try {
      ...
      showMessage(error.response?.data?.message || 'Failed to fetch search results.', 'error');
      setResults([]);
      setTotalResults(0);
    } finally {
      setLoading(false);
  ...
  return (
    <div className={`pt-0 -mt-6 pb-10 px-4 md:px-10 min-h-screen ${darkMode ? "bg-gray-900" : "bg-gray-50"}`}>
      <motion.div
      ...
      >
        <form onSubmit={handleSearchSubmit} className="relative flex-grow">
          ...
          />
          <button type="submit" className={`absolute right-3 top-1/2 -translate-y-1/2 p-2 rounded-2xl shadow-md transition-all duration-200 ${darkMode ? "bg-green-700 text-white hover:bg-green-600" : "bg-green-500 text-white hover:bg-green-600"}`} title="Search">
            <Search size={20} />
          </button>
        </form>
        <button type="button" onClick={() => setShowFilters(!showFilters)} className={`p-2 rounded-xl text-white shadow-md h-10 w-10 flex items-center justify-center flex-shrink-0 focus:outline-none focus:border-transparent focus:ring-1 focus:ring-offset-0 ${darkMode ? "bg-green-700 hover:bg-green-600 focus:ring-green-400" : "bg-green-500 hover:bg-green-600 focus:ring-green-600"}`} title="Open Filters">
      ...
      <AnimatePresence>
        {showFilters && (
          ...
          >
            ...
            <SearchFilters filters={filters} setFilters={setFilters} onSearch={handleSearchSubmit} />
          </motion.div>
        )}
      </AnimatePresence>

      {(totalResults > 0) && !loading && (
        <motion.div className={`text-center mb-4 text-lg font-semibold ${darkMode ? 'text-gray-400' : 'text-gray-500'}`} initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>

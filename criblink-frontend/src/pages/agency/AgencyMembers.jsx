      ...
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {[...Array(6)].map(
          (
            ...
            <div
              key={i}
              className={`p-4 rounded-xl shadow-md h-56 ${darkMode ? "bg-gray-700" : "bg-gray-100"}`}
            >
              ...
              <div className="flex justify-end mt-4 space-x-2">
                ...
                ></div>
                <div
                  className={`h-8 w-8 rounded-xl ${darkMode ? "bg-gray-600" : "bg-gray-300"}`}
                ></div>
              </div>
            </div>
          ),
...
const AgencyMembers = () => {
  ...
  ); // Changed to defaultListingsView
  const { darkMode } = useTheme();
  ...
  const fetchMembersAndRequests = useCallback(async () => {
    ...
    try {
      ...
      const membersRes = await axios.get(
        `${API_BASE_URL}/agencies/${agencyId}/agents`,
        {
      ...
      const requestsRes = await axios.get(
        `${API_BASE_URL}/agencies/${agencyId}/pending-requests`,
        {
          headers: { Authorization: `Bearer ${token}` },
        },
      );
      ...
      showMessage(
        "Failed to fetch agency members or requests. Please try again.",
        "error",
      );
    } finally {
    ...
    if (!showPendingRequests) {
      ...
      if (memberStatusFilter !== "all") {
        currentMembers = currentMembers.filter((member) => {
          return (
            (member.member_status || "regular").toLowerCase() ===
            memberStatusFilter
          );
        });
      }
  ...
  const handleExportCsv = (scope) => {
    const dataToExport = showPendingRequests
      ? scope === "current"
    ...
    link.download = showPendingRequests
      ? "pending_member_requests.csv"
      : "agency_members.csv";
  ...
  const contentShift =
    user?.role === "agency_admin" && !isMobile ? (isCollapsed ? 80 : 256) : 0;
  ...
  const handleRemoveMember = async (memberIdToRemove) => {
    ...
    if (
      user &&
      user.user_id === memberIdToRemove &&
      user.role === "agency_admin"
    ) {
      const adminMembers = members.filter(
        (member) => member.agency_role === "agency_admin",
      );
      if (
        adminMembers.length === 1 &&
        adminMembers[0].user_id === user.user_id
      ) {
        showMessage(
          "You cannot remove yourself as you are the last agency administrator. Please assign another member as an administrator before removing yourself.",
          "error",
        );
        return;
      }
    ...
    showConfirm({
      ...
      onConfirm: async () => {
        try {
          const token = localStorage.getItem("token");
          await axios.delete(
            `${API_BASE_URL}/agencies/${agencyId}/members/${memberIdToRemove}`,
            {
              headers: { Authorization: `Bearer ${token}` },
            },
          );
          ...
          showMessage(
            `Failed to remove member: ${err.response?.data?.message || "Please try again."}`,
            "error",
          );
        }
      },
    ...
    showConfirm({
      ...
      onConfirm: async () => {
        try {
          ...
          await axios.put(
            `${API_BASE_URL}/agencies/${agencyId}/members/${memberIdToPromote}/promote-to-admin`,
            {},
            {
              headers: { Authorization: `Bearer ${token}` },
            },
          ...
          showMessage(
            "Member promoted to administrator successfully!",
            "success",
          );
          ...
          showMessage(
            `Failed to promote member: ${err.response?.data?.message || "Please try again."}`,
            "error",
          );
        }
      },
  ...
  const handleDemoteMember = async (memberIdToDemote) => {
    ...
    if (
      ...
      if (
        ...
        showMessage(
          "You cannot demote yourself as you are the last agency administrator. Please assign another member as an administrator before stepping down.",
          "error",
        );
        return;
      }
    ...
    showConfirm({
      ...
      onConfirm: async () => {
        try {
          ...
          await axios.put(
            `${API_BASE_URL}/agencies/${agencyId}/members/${memberIdToDemote}/demote-to-agent`,
            {},
            {
              headers: { Authorization: `Bearer ${token}` },
            },
          ...
          showMessage(
            `Failed to demote member: ${err.response?.data?.message || "Please try again."}`,
            "error",
          );
        }
      },
    ...
    showConfirm({
      ...
      onConfirm: async () => {
        try {
          const token = localStorage.getItem("token");
          await axios.put(
            `${API_BASE_URL}/agencies/${agencyId}/members/${memberId}/status`,
            { status: newStatus },
            {
              headers: { Authorization: `Bearer ${token}` },
            },
          ...
          setMembers((prev) =>
            prev.map((m) =>
              m.user_id === memberId ? { ...m, member_status: newStatus } : m,
            ),
          );
          ...
          showMessage(
            `Member status updated to ${capitalizeFirstLetter(newStatus)}.`,
            "success",
          );
        } catch (err) {
          console.error("Failed to update member status:", err);
          showMessage(
            `Failed to update member status: ${err.response?.data?.message || "Please try again."}`,
            "error",
          );
        }
      },
  ...
  const totalItems = showPendingRequests
        ...
        >
          ...
          <div className="hidden md:grid grid-cols-3 items-center gap-4 mb-6 max-w-[1344px] mx-auto">
            ...
            <div className="flex justify-start w-full relative">
              <div className="w-full relative" ref={filterAreaRef}>
                {" "}
                {/* Expanded ref scope here */}
                <input
                  type="text"
            ...
            ${darkMode ? "bg-gray-800 border border-gray-700" : "bg-white border border-gray-200"}`}
                    >
                      <div>
                        <label
                          htmlFor="member-role-filter-desktop"
                      ...
                      <div>
                        <label
                          htmlFor="member-status-filter-desktop"
                          className={`block text-sm font-medium mb-2 ${darkMode ? "text-gray-300" : "text-gray-700"}`}
                        >
                          Rank
                        </label>
          ...
          <div className="md:hidden flex flex-col gap-4 mb-6">
            <div className="flex items-center justify-between gap-4">
              ...
              <div className="flex-1 relative" ref={filterAreaRef}>
                {" "}
                {/* Wrapped search and filter in one ref */}
                <input
                  type="text"
                  placeholder="Search members by name or email..."
                  value={searchTerm}
                ...
                <AnimatePresence>
                  {showSearchBarFilters && (
                    ...
                    >
                      <div>
                        <label
                          htmlFor="member-role-filter-mobile"
                          className={`block text-sm font-medium mb-2 ${darkMode ? "text-gray-300" : "text-gray-700"}`}
                        >
                          Member Role
                        </label>
                      ...
                      <div>
                        <label
                          htmlFor="member-status-filter-mobile"
                          className={`block text-sm font-medium mb-2 ${darkMode ? "text-gray-300" : "text-gray-700"}`}
                        >
                          Rank
                        </label>
                  ...
                  <thead>
                    ...
                    >
                      <th
                        ...
                        className="cursor-pointer text-left py-2 px-2 whitespace-nowrap min-w-[120px]"
                      >
                        Name {renderSortIcon("agent_name")}
                      </th>
                      <th
                        onClick={() => handleSortClick("agent_email")}
                        className="cursor-pointer text-left py-2 px-2 whitespace-nowrap min-w-[150px]"
                      >
                        Email {renderSortIcon("agent_email")}
                      </th>
                      <th className="text-left py-2 px-2 whitespace-nowrap min-w-[100px]">
                        Phone
                      </th>{" "}
                      {/* New Phone Header */}
                      <th
                        onClick={() => handleSortClick("requested_at")}
                        className="cursor-pointer text-left py-2 px-2 whitespace-nowrap min-w-[100px]"
                      >
                        Requested At {renderSortIcon("requested_at")}
                      </th>
                      <th className="text-left py-2 px-2 whitespace-nowrap min-w-[120px]">
                        Actions
                      </th>
                    </tr>
                  </thead>
                  ...
                  >
                    {paginatedData.map((request) => (
                      <tr
                        ...
                        onClick={() =>
                          handleViewProfile(request.agent_id, "pending")
                        } // Pass 'pending' as role for consistent handling
                      >
                        <td
                        ...
                        <td
                          className="px-2 py-2 break-words"
                          title={request.agent_email}
                        >
                          {request.agent_email}
                        </td>
                        <td
                          className="px-2 py-2 break-words"
                          title={request.agent_phone}
                        >
                          {request.agent_phone || "N/A"}
                        </td>{" "}
                        {/* New Phone Data */}
                        <td
                          className="px-2 py-2 flex gap-1 flex-wrap"
              ...
              >
                ...
                <thead>
                  ...
                  >
                    ...
                    {/* New Phone Header */}
                    <th
                      onClick={() => handleSortClick("agency_role")}
                      className="cursor-pointer text-left py-2 px-2 whitespace-nowrap min-w-[80px]"
                    >
                      Role {renderSortIcon("agency_role")}
                    </th>
                    <th
                      onClick={() => handleSortClick("user_status")}
                      className="cursor-pointer text-left py-2 px-2 whitespace-nowrap min-w-[80px]"
                    >
                      Status {renderSortIcon("user_status")}
                    </th>
                    <th
                      onClick={() => handleSortClick("member_status")}
                      className="cursor-pointer text-left py-2 px-2 whitespace-nowrap min-w-[80px]"
                    >
                      Rank {renderSortIcon("member_status")}
                    </th>
                    <th className="text-left py-2 px-2 whitespace-nowrap min-w-[150px]">
                      Actions
                    </th>
                  </tr>
                </thead>
                ...
                >
                  {paginatedData.map((member) => (
                    <tr
                      ...
                      className={`border-t cursor-pointer ${darkMode ? "border-gray-700 hover:bg-gray-700" : "border-gray-200 hover:bg-gray-50"}`}
                      onClick={() =>
                        handleViewProfile(member.user_id, member.agency_role)
                      }
                    >
                      <td
                        className="px-2 py-2 break-words"
                      ...
                      {/* New Phone Data */}
                      <td
                        className={`px-2 py-2 font-semibold ${darkMode ? "text-gray-300" : "text-gray-600"} break-words`}
                        title={member.agency_role || "agent"}
                      >
                      ...
                      <td
                        className={`px-2 py-2 font-semibold ${member.user_status === "banned" ? "text-red-600" : member.user_status === "deactivated" ? "text-yellow-600" : "text-green-600"} break-words`}
                        title={member.user_status || "active"}
                      >
                        {(member.user_status || "active")
                          .charAt(0)
                          .toUpperCase() +
                          (member.user_status || "active").slice(1)}
                      </td>
                      ...
                      <td
                        className="px-2 py-2 flex gap-1 items-center flex-wrap"
                        onClick={(e) => e.stopPropagation()}
                      >
                        {" "}
                        {/* Stop propagation for action buttons */}

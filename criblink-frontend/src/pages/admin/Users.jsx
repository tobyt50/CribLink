      ...
      <button
        ...
        className={`flex items-center justify-between w-full py-1 px-4 border rounded-xl shadow-sm focus:outline-none focus:border-transparent focus:ring-1 focus:ring-offset-0 transition-all duration-200 h-10 ${
          darkMode
            ...
            : "bg-white border-gray-300 text-gray-500 hover:border-green-500 focus:ring-green-600"
        }`}
      >
...
const UsersSkeleton = ({ darkMode, viewMode }) => (
  <div className={`animate-pulse space-y-4`}>
    {viewMode === "graphical" ? (
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {[...Array(9)].map((_, i) => (
          <div
            key={i}
            className={`rounded-xl p-4 shadow-sm h-48 ${darkMode ? "bg-gray-700" : "bg-gray-100"}`}
          >
            <div className="flex flex-col items-center justify-center h-full space-y-3">
              <div
                className={`w-20 h-20 rounded-full ${darkMode ? "bg-gray-600" : "bg-gray-300"}`}
              ></div>
              <div
    ...
    <div className="flex justify-center items-center space-x-4 mt-4">
      <div
        className={`h-8 w-16 rounded-lg ${darkMode ? "bg-gray-700" : "bg-gray-200"}`}
      ></div>
      <div
...
const Users = () => {
  ...
  const renderSortIcon = (key) => {
    ...
    if (sortKey === key) {
      return sortDirection === "asc" ? (
        <ArrowUpIcon className={`w-4 h-4 ml-1 ${iconColor}`} />
      ) : (
        <ArrowDownIcon className={`w-4 h-4 ml-1 ${iconColor}`} />
      );
    }
    return (
      <ArrowDownIcon className={`w-4 h-4 ml-1 text-gray-400 opacity-50`} />
    );
  };
    ...
    try {
      if (action.startsWith("role:")) {
        ...
        const actionText =
          newRole === "admin"
            ...
            : newRole === "agent"
              ...
              : newRole === "client"
                ? "change to Client"
                : "change to Agency Admin";
        showConfirm({
          title: `Confirm Role Change`,
          message: `Are you sure you want to ${actionText} for ${user.full_name}?`,
          onConfirm: async () => {
            await axiosInstance.put(`/admin/users/${user.user_id}/role`, {
              newRole,
            });
        ...
        showConfirm({
          ...
          onConfirm: async () => {
            await axiosInstance.put(`/admin/users/${user.user_id}/status`, {
              status: newStatus,
            });
      ...
      showMessage(
        error.response?.data?.message || "Failed to apply action.",
        "error",
      );
    }
  };
  ...
  const handleSubscriptionChange = async (user, subscriptionType) => {
    try {
      await axiosInstance.put(`/admin/users/${user.user_id}/subscription`, {
  ...
  const handleExportCsv = async (scope) => {
    if (
      (scope === "current" && users.length === 0) ||
      (scope === "all" && totalUsers === 0)
    ) {
                  ...
                  <AnimatePresence>
                    {showSearchBarFilters && (
                      <motion.div
                        ...
                        exit={{ opacity: 0, y: 10 }}
                        transition={{ duration: 0.2 }}
                        className={`absolute left-0 right-0 top-full mt-2 p-4 rounded-xl shadow-lg z-50 space-y-4 ${darkMode ? "bg-gray-800 border border-gray-700" : "bg-white border border-gray-200"}`}
                      >
                        <div>
                          <label
                        ...
                        <div>
                          ...
                          <Dropdown
                            ...
                            onChange={handleSubscriptionChangeForFilter}
                            className="w-full"
                          />
                        </div>
                      </motion.div>
                  ...
                  <AnimatePresence>
                    {showSearchBarFilters && (
                      <motion.div
                        ...
                        exit={{ opacity: 0, y: 10 }}
                        transition={{ duration: 0.2 }}
                        className={`absolute left-0 right-0 top-full mt-2 p-4 rounded-xl shadow-lg z-50 space-y-4 w-96 ${darkMode ? "bg-gray-800 border border-gray-700" : "bg-white border border-gray-200"}`}
                      >
                        <div>
                          <label
                            className={`block text-sm font-medium mb-2 ${darkMode ? "text-gray-300" : "text-gray-700"}`}
                ...
                >
                  {users.map((user) => (
                    ...
                    >
                      ...
                      <td
                        className={`py-2 px-2 truncate font-semibold ${user.status === "banned" ? "text-red-600" : user.status === "deactivated" ? "text-yellow-600" : "text-green-600"}`}
                      >
                        {user.status
                      ...
                      >
                        ...
                        ((user.role || user.user_role) === "agent" ||
                          (user.role || user.user_role) === "agency_admin") ? (
                          <Dropdown
                            placeholder="None"
                            options={subscriptionOptionsForTable}
                            value={user.subscription_type ?? ""}
                            onChange={(value) =>
                              handleSubscriptionChange(user, value)
                            }
                            className="w-full"
                          />
                        ) : (
                        ...
                        <div className="flex flex-col gap-1 items-start w-full min-w-[120px]">
                          <Dropdown
                            ...
                            options={[
                              ...
                              ...(user.role !== "admin"
                                ? [{ value: "role:admin", label: "To Admin" }]
                                : []),
                              ...(user.role !== "agent"
                                ? [{ value: "role:agent", label: "To Agent" }]
                                : []),
                              ...(user.role !== "client"
                                ? [{ value: "role:client", label: "To Client" }]
                                : []),
                              ...(user.role !== "agency_admin"
                                ? [
                                    {
                                      value: "role:agency_admin",
                                      label: "To Agency Admin",
                                    },
                                  ]
                                : []),

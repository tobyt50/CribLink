...
const ListingTableRowSkeleton = ({ darkMode }) => (
  ...
  >
    ...
    <td className="py-2 px-2">
      <div
        className={`h-4 w-28 rounded ${darkMode ? "bg-gray-600" : "bg-gray-300"}`}
      ></div>
    </td>
    <td className="py-2 px-2">
      <div
    ...
    <td className="py-2 px-2">
      <div
        className={`h-4 w-16 rounded ${darkMode ? "bg-gray-600" : "bg-gray-300"}`}
      ></div>
    </td>
    <td className="py-2 px-2">
      <div
    ...
    <td className="py-2 px-2">
      <div
        className={`h-4 w-12 rounded ${darkMode ? "bg-gray-600" : "bg-gray-300"}`}
      ></div>
    </td>
    <td className="py-2 px-2">
      <div
...
const Listings = () => {
  ...
  const fetchListings = useCallback(async () => {
    ...
    try {
      ...
      if (
        ...
        purchaseCategoryFilter.toLowerCase() !== "all"
      ) {
      ...
      if (statusFilter && statusFilter.toLowerCase() !== "all statuses") {
        // Send 'all' as a filter too if needed by backend
    ...
    navigate,
    showMessage,
  ]); // All filters, pagination, and user-related dependencies
  ...
  const fetchAgencyDetails = useCallback(async () => {
    ...
    try {
      ...
      const config = token
        ...
        : {};
      const response = await axios.get(
        `${API_BASE_URL}/agencies/${agencyIdFilter}`,
        config,
      );
  ...
  const handleApproveListing = async (listingId) => {
    showConfirm({
      ...
      message:
        "Are you sure you want to approve this listing and make it available?",
      onConfirm: async () => {
        const token = localStorage.getItem("token");
        if (!token) {
          showMessage(
  ...
  const handleMarkAsFailed = async (listingId) => {
    showConfirm({
      ...
      message:
        "Are you sure you want to mark this listing as 'Available' (undo offer)?",
      onConfirm: async () => {
        const token = localStorage.getItem("token");
        if (!token) {
          showMessage(
        ...
        try {
          ...
          console.error(
            "Error marking listing as failed:",
            error.response?.data || error.message,
          );
          showMessage(
            "Failed to mark listing as available. Please try again.",
            "error",
          );
        }
      },
  ...
  const handleExportCsv = async (scope) => {
    const dataToExport =
      scope === "current" ? filteredAndSortedListings : listings; // Use filteredAndSortedListings for current view
  ...
  const handleStatusChange = useCallback((value) => {
    // Changed to accept value directly from Dropdown
  ...
  const canPerformActions = useMemo(() => {
    ...
    if (agencyIdFilter) {
      ...
      return (
        ...
        parseInt(userAgencyId) === parseInt(agencyIdFilter)
      );
    }
  ...
  return (
    ...
    ${darkMode ? "bg-gray-800 text-gray-300" : "bg-white text-gray-700"}`}
      ...
      >
        ...
        <h1
          className={`text-2xl md:text-3xl font-extrabold text-center mb-4 md:mb-6 ${darkMode ? "text-green-400" : "text-green-700"}`}
        >
                    ...
                    <AnimatePresence>
                      {showSearchBarFilters && (
                        ...
                        >
                          <div>
                            <label
                              htmlFor="purchase-category-filter-mobile"
                          ...
                          <div>
                            <label
                              htmlFor="min-price-filter-mobile"
                              className={`block text-sm font-medium mb-2 ${darkMode ? "text-gray-300" : "text-gray-700"}`}
                            >
                              Min Price
                            </label>
                            <div className="relative">
                              <span
                                className={`absolute left-3 top-1/2 -translate-y-1/2 text-lg font-bold ${darkMode ? "text-gray-400" : "text-gray-400"}`}
                              >
                                ₦
                              </span>
                              <input
                          ...
                          <div>
                            <label
                              htmlFor="max-price-filter-mobile"
                              className={`block text-sm font-medium mb-2 ${darkMode ? "text-gray-300" : "text-gray-700"}`}
                            >
                              Max Price
                            </label>
                  ...
                  {shouldShowAddExportButtons && (
                    <>
                      <button
                        ...
                        onClick={() =>
                          navigate(`${getRoleBasePath()}/add-listing`)
                        } // Role-specific path
                      ...
                      >
                        ...
                        {isExportDropdownOpen && (
                          <div
                            className={`absolute right-0 z-10 mt-2 w-40 origin-top-right rounded-xl shadow-lg ring-1 ring-black ring-opacity-5 ${darkMode ? "bg-gray-800 text-gray-200 border-gray-700" : "bg-white text-gray-900"}`}
                          >
                            <div className="py-1">
                              <button
            ...
            {!isMobile && (
              <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
                ...
                <div className="flex items-center gap-4 w-full">
                  {/* Search Bar with integrated Filter Icon */}
                  <div
                    className="w-full relative max-w-[28rem]"
                    ref={filterAreaRef}
                  >
                    <input
                      type="text"
                    ...
                    <AnimatePresence>
                      {showSearchBarFilters && (
                        ...
                        >
                          <div>
                            <label
                              htmlFor="purchase-category-filter-desktop"
                              className={`block text-sm font-medium mb-2 ${darkMode ? "text-gray-300" : "text-gray-700"}`}
                            >
                              Purchase Category
                            </label>
                          ...
                          <div>
                            <label
                              htmlFor="min-price-filter-desktop"
                              className={`block text-sm font-medium mb-2 ${darkMode ? "text-gray-300" : "text-gray-700"}`}
                            >
                              Min Price
                            </label>
                          ...
                          <div>
                            <label
                              htmlFor="max-price-filter-desktop"
                              className={`block text-sm font-medium mb-2 ${darkMode ? "text-gray-300" : "text-gray-700"}`}
                            >
                              Max Price
                            </label>
              ...
              viewMode === "graphical" ? (
                ...
                <div className="overflow-x-auto">
                  ...
                  >
                    <thead>
                      ...
                      >
                        {[
                          ...
                          "bedrooms",
                          "bathrooms",
                          showActionsColumn ? "actions" : null,
                        ]
                          .filter(Boolean)
                          .map((key) => (
                            <th
                    ...
                    >
                      {[...Array(limit)].map((_, i) => (
                        <ListingTableRowSkeleton key={i} darkMode={darkMode} />
                      ))}
                    </tbody>
                  </table>
                </div>
                ...
                >
                                          ...
                                          : key === "status"
                                            ...
                                            : key === "date_listed"
                                              ...
                                              : key === "purchase_category"
                                                ...
                                                : key === "bedrooms"
                                                  ? "70px"
                                                  : "70px", // Default for bathrooms if actions is not present
                            }}
                          >
                            <div className="flex items-center gap-1">
                              <span className="truncate">
                  ...
                  >
                    {filteredAndSortedListings.map((listing) => (
                      ...
                      >
                        <td
                          className="py-2 px-2 max-w-[90px] truncate"
                          title={
                            listing.property_id &&
                            listing.property_id.length > 10
                              ? listing.property_id
                              : ""
                          }
                        >
                        ...
                        <td
                          className="py-2 px-2 max-w-[120px] truncate"
                          title={
                        ...
                        <td
                          className="py-2 px-2 max-w-[120px] truncate"
                          title={
                            listing.location && listing.location.length > 15
                              ? listing.location
                              : ""
                          }
                        >
                        ...
                        <td
                          ...
                          title={
                            ...
                            listing.property_type.length > 10
                              ? listing.property_type
                              : ""
                          }
                        >
                        ...
                        <td
                          ...
                          title={
                            ...
                            listing.purchase_category.length > 12
                              ? listing.purchase_category
                              : ""
                          }
                        >
                        ...
                        <td
                          className="py-2 px-2 max-w-[70px] truncate"
                          title={
                            ...
                            listing.status.toLowerCase() === "pending" ? (
                              <div className="flex items-center gap-2">
                                <button
                                  ...
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    handleApproveListing(listing.property_id);
                                  }}
                                  title="Approve Listing"
                                >
                                ...
                                <button
                                  ...
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    handleRejectListing(listing.property_id);
                                  }}
                                  title="Reject Listing"
                                >
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                <button
                                  ...
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    handleDeleteListing(listing.property_id);
                                  }}
                                  title="Delete Listing"
                                >
                              ...
                              listing.status.toLowerCase() === "under offer" ? (
                              <div className="flex items-center gap-2">
                                <button
                                  className="text-green-600 hover:text-green-800 p-1"
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    handleMarkAsSold(listing.property_id);
                                  }}
                                  title="Mark as Sold"
                                >
                                ...
                                <button
                                  ...
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    handleMarkAsFailed(listing.property_id);
                                  }}
                                  title="Mark as Failed (Return to Available)"
                                >
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                <button
                                  ...
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    navigate(
                                      `${getRoleBasePath()}/edit-listing/${listing.property_id}`,
                                    );
                                  }}
                                  title="Edit Listing"
                                >

    ...
    <div className="space-y-8 animate-pulse">
        <div className={`pb-6 mb-6 border-b ${darkMode ? "border-gray-700" : "border-gray-200"}`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {[...Array(4)].map((_, i) => (
                    <div key={i} className={`h-24 rounded-xl ${darkMode ? "bg-gray-700" : "bg-gray-300"}`}></div>
                ))}
            </div>
        </div>
...
function GeneralSystem() {
    ...
    const handleUpdate = async () => {
        ...
        try {
            ...
            // });
            showMessage('System settings updated successfully! (Simulated)', 'success');
        } catch (error) {
    ...
    return (
        ...
        >
            <div className={`pb-6 mb-6 border-b ${darkMode ? "border-gray-700" : "border-gray-200"}`}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label className={labelStyles} htmlFor="site_name">Site Name</label>
                        <input type="text" id="site_name" name="site_name" value={settings.site_name} onChange={handleChange} className={inputFieldStyles} placeholder="e.g., RealtyHub"/>
                    </div>
                     <div>
                        <label className={labelStyles} htmlFor="contact_email">Public Contact Email</label>
                        <input type="email" id="contact_email" name="contact_email" value={settings.contact_email} onChange={handleChange} className={inputFieldStyles} placeholder="e.g., support@example.com"/>
                    </div>
                     <div>
                        <label className={labelStyles} htmlFor="default_currency">Default Currency</label>
                        <select id="default_currency" name="default_currency" value={settings.default_currency} onChange={handleChange} className={inputFieldStyles}>
                    ...
                    <div>
                        <label className={labelStyles} htmlFor="mapbox_api_key">Mapbox API Key</label>
                        <input type="text" id="mapbox_api_key" name="mapbox_api_key" value={settings.mapbox_api_key} onChange={handleChange} className={inputFieldStyles} placeholder="pk.ey..."/>
                    </div>
                </div>
            </div>

...
import { Sun, Moon, Monitor, LayoutGrid, LayoutList, ChevronDownIcon, Globe, Home } from 'lucide-react';
    ...
    <div className="space-y-8 animate-pulse">
        <div className={`pb-6 mb-6 border-b ${darkMode ? "border-gray-700" : "border-gray-200"}`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {[...Array(4)].map((_, i) => <div key={i} className={`h-24 rounded-xl ${darkMode ? "bg-gray-700" : "bg-gray-300"}`}></div>)}
            </div>
        </div>
        <div className={`pb-6 mb-6`}>
            <div className={`h-6 w-1/3 rounded ${darkMode ? "bg-gray-700" : "bg-gray-300"} mb-5`}></div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {[...Array(3)].map((_, i) => <div key={i} className={`h-24 rounded-xl ${darkMode ? "bg-gray-700" : "bg-gray-300"}`}></div>)}
            </div>
        </div>
    </div>
);
...
const Preferences = () => {
    ...
    const [userSettings, setUserSettings] = useState({ language: 'en', timezone: 'UTC+1', currency: 'NGN', default_landing_page: '/' });
    ...
    const handleSettingUpdate = async (name, value) => {
        ...
        try {
            await axiosInstance.put('/users/update', { [name]: value }, { headers: { Authorization: `Bearer ${token}` } });
    ...
    const timezoneOptions = [ { value: 'UTC+01', label: 'West Central Africa (UTC+1)' } ];
    const currencyOptions = [ { value: 'NGN', label: '₦ Nigerian Naira' }, { value: 'USD', label: '$ US Dollar' }, { value: 'EUR', label: '€ Euro' } ];
    const landingPageOptions = useCallback(() => {
        const base = [{ value: '/', label: 'Home', icon: <Home size={20}/> }];
        if (user?.role === 'admin') base.push({ value: '/admin/dashboard', label: 'Admin Dashboard' });
        if (user?.role === 'agent' || user?.role === 'agency_admin') base.push({ value: '/agent/dashboard', label: 'Agent Dashboard' });
        if (user?.role === 'client') base.push({ value: '/client/dashboard', label: 'Client Dashboard' });
        return base;
    }, [user?.role]);
    ...
    return (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-8">
            <div className={`pb-6 mb-6 border-b ${darkMode ? "border-gray-700" : "border-gray-200"}`}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className={`p-6 rounded-xl border ${darkMode ? "border-gray-700 bg-gray-800" : "border-gray-200 bg-white"}`}>
                        ...
                        <Dropdown options={themeOptions} value={themePreference} onChange={handleThemeChange} />
                    </div>
                    <div className={`p-6 rounded-xl border ${darkMode ? "border-gray-700 bg-gray-800" : "border-gray-200 bg-white"}`}>
                        <label className="block text-lg font-semibold mb-3">Default Listings View</label>
                        <Dropdown options={listViewOptions} value={defaultListView} onChange={handleDefaultListViewChange} />
                    </div>
                     <div className={`p-6 rounded-xl border ${darkMode ? "border-gray-700 bg-gray-800" : "border-gray-200 bg-white"}`}>
                        <label className="block text-lg font-semibold mb-3">Language</label>
                        <Dropdown options={languageOptions} value={userSettings.language} onChange={(v) => handleSettingUpdate('language', v)} />
                    </div>
                    <div className={`p-6 rounded-xl border ${darkMode ? "border-gray-700 bg-gray-800" : "border-gray-200 bg-white"}`}>
                        <label className="block text-lg font-semibold mb-3">Timezone</label>
                        <Dropdown options={timezoneOptions} value={userSettings.timezone} onChange={(v) => handleSettingUpdate('timezone', v)} />
                    </div>
                </div>
            </div>
            ...
            <div className={`pb-6 mb-6`}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className={`p-6 rounded-xl border ${darkMode ? "border-gray-700 bg-gray-800" : "border-gray-200 bg-white"}`}>
                        <label className="block text-lg font-semibold mb-3">Default Landing Page</label>
                        <Dropdown options={landingPageOptions()} value={userSettings.default_landing_page} onChange={(v) => handleSettingUpdate('default_landing_page', v)} />
                    </div>
                    <div className={`p-6 rounded-xl border ${darkMode ? "border-gray-700 bg-gray-800" : "border-gray-200 bg-white"}`}>
                        <label className="block text-lg font-semibold mb-3">Default Currency</label>
                        <Dropdown options={currencyOptions} value={userSettings.currency} onChange={(v) => handleSettingUpdate('currency', v)} />
                    </div>
                    <Switch

...
const InquiryTableRowSkeleton = ({ darkMode }) => (
  ...
  >
    ...
    <td className="py-2 px-2 text-center">
      <div
        className={`h-8 w-8 rounded-full mx-auto ${darkMode ? "bg-gray-600" : "bg-gray-300"}`}
      ></div>
    </td>
  </tr>
);
...
const AgentInquiries = () => {
  ...
  const fetchInquiries = useCallback(async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const res = await fetch(`${API_BASE_URL}/inquiries/agent?${params}`, {
  ...
  useEffect(() => {
    ...
    const handleNewMessage = async (newMessage) => {
      setGroupedConversations((prev) => {
        ...
        const updatedConversations = prev.map((conv) => {
          if (conv.id === newMessage.conversationId) {
            ...
            const updatedConv = {
              ...
              unreadCount:
                newMessage.senderId === conv.client_id && !messageToAdd.read
                  ? conv.unreadCount + 1
                  : conv.unreadCount,
              // Update is_agent_responded only if the agent sends a message
              is_agent_responded:
                newMessage.senderId === agentUserId
                  ? true
                  : conv.is_agent_responded,
            };
            return updatedConv;
          }
          return conv;
        ...
        if (
          !conversationExists &&
          (newMessage.agentId === agentUserId || !newMessage.agentId)
        ) {
          fetchInquiries();
          return prev;
        }
        ...
        if (
          ...
          sortedConversations.some((c) => c.id === selectedConversationId)
        ) {
          ...
          prev.map((conv) => {
            if (conv.id === conversationId) {
              return {
                ...conv,
                messages: conv.messages.map((msg) =>
                  msg.sender_id === agentUserId ? { ...msg, read: true } : msg,
                ),
              };
            }
            return conv;
  ...
  const handleViewConversation = useCallback(
    async (conversation) => {
      ...
      if (conversation.unreadCount > 0 && !conversation.isReassignedFromMe) {
        ...
        try {
          await fetch(
            `${API_BASE_URL}/inquiries/agent/mark-read/${conversation.id}`,
            {
              method: "PUT",
              headers: { Authorization: `Bearer ${token}` },
            },
      ...
      try {
        const token = localStorage.getItem("token");
        await fetch(
          `${API_BASE_URL}/inquiries/agent/mark-opened/${conversation.id}`,
          {
            method: "PUT",
            headers: { Authorization: `Bearer ${token}` },
          },
  ...
  const handleArchiveConversation = async (
    ...
    showConfirm({
      ...
      message: isReassignedFromMe
        ? "Are you sure you want to remove this reassigned chat from your active list? It will still be accessible in the Archive."
        : `Are you sure you want to archive this conversation? It will be moved to your Archive and you can restore it later.`,
      onConfirm: async () => {
        const token = localStorage.getItem("token");
        try {
          // This endpoint will set hidden_from_agent = TRUE for the current agent
          const res = await fetch(
            `${API_BASE_URL}/inquiries/${conversationId}/archive-for-agent`,
            {
              method: "PUT",
              headers: { Authorization: `Bearer ${token}` },
            },
          ...
          showMessage(
            isReassignedFromMe
              ? "Failed to remove chat from list."
        ...
        >
          <div className="flex flex-col md:flex-row gap-4 items-center">
            <input
              type="text"
              placeholder="Search..."
              className={`w-full md:w-1/3 py-2 px-4 border rounded-xl h-10 focus:outline-none focus:ring-1 ${darkMode ? "bg-gray-700 border-gray-600 text-white focus:ring-green-400" : "bg-white border-gray-300 focus:ring-green-600"}`}
              value={search}
              onChange={(e) => {
            ...
            <div className="space-y-4">
              {groupedConversations.length > 0 ? (
                groupedConversations.map((conv) => {
                  ...
                  const hasUnreadMessagesForAgent = conv.messages.some(
                  ...
                  const displayStatus = conv.is_agent_responded
                    ? "Responded"
                    : "New Message";
                    ...
                    >
                      <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center">
                          <img
                            src={
                              ...
                              `https://placehold.co/40x40/${darkMode ? "374151" : "E0F7FA"}/${darkMode ? "D1D5DB" : "004D40"}?text=${getInitial(conv.clientName)}`
                            }
                            alt="Client Profile"
                            className="w-12 h-12 rounded-full mr-4 object-cover cursor-pointer" // Increased size and margin
                            onError={(e) => {
                              e.target.onerror = null;
                              e.target.src = `https://placehold.co/40x40/${darkMode ? "374151" : "E0F7FA"}/${darkMode ? "D1D5DB" : "004D40"}?text=${getInitial(conv.clientName)}`;
                            }}
                            onClick={(e) => {
                              e.stopPropagation();
                          ...
                          >
                            {" "}
                            {/* Adjusted client name color for mobile */}
                            ...
                            {conv.client_id ? (
                              <span
                                ...
                                onClick={(e) => {
                                  e.stopPropagation();
                                  navigate(
                                    `/agent/client-profile/${conv.client_id}`,
                                  );
                                }}
                              >
                                {conv.clientName}
                        ...
                        <div className="absolute top-2 right-2">
                          {isReassignedFromMe ? (
                            ...
                            >
                              {" "}
                              {/* Changed to red for new, green for responded */}
                              {displayStatus}
                            </span>
                          )}
                        </div>
                      ...
                      <div>
                        ...
                        >
                          ...
                          <span
                            ...
                            onClick={(e) => {
                              e.stopPropagation();
                              if (conv.agent_id)
                                navigate(`/agent-profile/${conv.agent_id}`);
                            }}
                          >
                        ...
                        {isReassignedFromMe &&
                          ...
                          conv.reassigned_at && (
                            ...
                            >
                              ...
                              Reassigned to {conv.agent_name} by{" "}
                              ...
                              {conv.reassigned_by_admin_id ? (
                                <span
                                  ...
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    navigate(
                                      `/agency-admin-profile/${conv.reassigned_by_admin_id}`,
                                    );
                                  }}
                                >
                                  {conv.reassigned_by_admin_name}
                        ...
                        >
                          ...
                          Last Message:{" "}
                          <span
                            className={`${darkMode ? "text-green-400" : "text-green-600"} ${isBold && !isReassignedFromMe ? "font-semibold" : ""}`}
                          >
                ...
                >
                  {groupedConversations.length > 0 ? (
                    groupedConversations.map((conv) => {
                      ...
                      // Determine display status: "New Message" if unread messages OR if agent hasn't responded yet
                      // "Responded" only if agent has sent a message AND all client messages are read
                      let displayStatus = conv.is_agent_responded
                        ? "Responded"
                        : "New Message";
                      ...
                      return (
                        <tr
                          key={conv.id}
                          className={`border-t cursor-pointer ${darkMode ? "border-gray-700 hover:bg-gray-700" : "border-gray-200 hover:bg-gray-50"} ${isBold && !isReassignedFromMe ? "font-bold" : "font-normal"} ${isReassignedFromMe ? "opacity-60 bg-yellow-100 dark:bg-yellow-900" : ""}`}
                          onClick={() => handleViewConversation(conv)}
                        >
                          <td
                            className="py-2 px-2 truncate"
                            title={conv.clientName}
                          >
                            <div className="flex items-center">
                              <img
                                src={
                              ...
                              >
                                {" "}
                                {/* Adjusted client name color for desktop */}
                                {conv.client_id ? (
                                  <span
                                    className="cursor-pointer hover:underline"
                                    onClick={(e) => {
                          ...
                          <td className="py-2 px-2 truncate">
                            ...
                            {isReassignedFromMe &&
                              ...
                              conv.reassigned_at && (
                                ...
                                >
                                  (from {conv.original_agent_name} by{" "}
                                  {/* Admin Name clickable */}
                                  {conv.reassigned_by_admin_id ? (
                                    <span
                                      className="cursor-pointer hover:underline"
                          ...
                          <td
                            className={`py-2 px-2 truncate font-semibold ${displayStatus === "New Message" ? "text-red-600" : displayStatus === "Reassigned" ? (darkMode ? "text-yellow-400" : "text-yellow-700") : darkMode ? "text-green-400" : "text-green-700"}`}
                          >
                            {displayStatus}
                          </td>
                          <td className="py-2 px-2 text-center">

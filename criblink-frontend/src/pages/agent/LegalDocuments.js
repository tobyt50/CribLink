...
import {
  TableCellsIcon, Squares2X2Icon, ArrowUpIcon, ArrowDownIcon,
  TrashIcon, PencilIcon, CheckCircleIcon, XCircleIcon
} from '@heroicons/react/24/outline';
import { ChevronDownIcon } from '@heroicons/react/24/outline';
import API_BASE_URL from '../../config';
import { Menu, X, Search, SlidersHorizontal, Plus, FileText, LayoutList } from 'lucide-react';
import { useTheme } from '../../layouts/AppShell';
import { useMessage } from '../../context/MessageContext';
import { useConfirmDialog } from '../../context/ConfirmDialogContext';
import { useSidebarState } from '../../hooks/useSidebarState'; // Import the hook

// Reusable Dropdown component (copied from Listings.js for consistency)
...
const LegalDocuments = () => {
    ...
    const [activeSection, setActiveSection] = useState('documents'); // Set active section for sidebar

    const [isExportDropdownOpen, setIsExportDropdownOpen] = useState(false);
    const exportDropdownRef = useRef(null);

    const [isFilterModalOpen, setIsFilterModalOpen] = useState(false); // For mobile filter modal
    const [isDesktopFilterModalOpen, setIsDesktopFilterModalOpen] = useState(false); // For desktop filter modal

    const location = useLocation(); // To potentially read state from navigation
    ...
    const fetchDocuments = useCallback(async () => {
        ...
        try {
            // Assuming your backend endpoint for legal documents is /docs
            const response = await axiosInstance.get(`${API_BASE_URL}/docs?${params.toString()}`, { headers });
            setDocuments(response.data.documents || []);
            setFilteredDocuments(response.data.documents || []); // Initial filter/sort
    ...
    }, [documentTypeFilter, searchTerm, statusFilter, page, limit, showMessage]);
    ...
    const performDeleteDocument = async (documentId, publicId) => {
        ...
        try {
            // Assuming a DELETE endpoint for documents that also handles Cloudinary deletion
            await axiosInstance.delete(`${API_BASE_URL}/docs/${documentId}`, {
                headers: {
                    'Authorization': `Bearer ${token}`
                },
                data: { publicId: publicId } // Send publicId for Cloudinary deletion on backend
            });
    ...
    const handleExportCsv = async (scope) => {
        ...
        const headers = [
            'document_id', 'title', 'client_name', 'property_id', 'document_type', 'status', 'upload_date', 'completion_date', 'document_url'
        ];
    ...
    const handleSortClick = (key) => {
        const sortableColumns = ['document_id', 'title', 'client_name', 'document_type', 'status', 'upload_date', 'completion_date'];
    ...
    // Options for dropdowns (re-defined here for self-containment, but could be imported)
                    ...
                    >
                        {isMobile && (
                            <div className="flex justify-between items-center mb-4">
                                ...
                                {/* No graphical view for documents, so no view mode buttons */}
                            </div>
                        )}
                                    ...
                                    <thead>
                                        <tr className={`${darkMode ? "text-gray-400" : "text-gray-500"}`}>
                                            {[
                                                ...
                                                'document_type', 'status', 'upload_date', 'completion_date', 'actions'
                                            ].map((key) => (
                                                <th
                                                    key={key}
                                    ...
                                    <tbody className={`${darkMode ? "divide-gray-700" : "divide-gray-200"} divide-y`}>
                                        {filteredDocuments.map((doc) => (
                                            <tr key={doc.document_id} className={`border-t cursor-default max-w-full break-words ${darkMode ? "border-gray-700 hover:bg-gray-700" : "border-gray-200 hover:bg-gray-50"}`}>
                                                <td className="py-2 px-2 max-w-[90px] truncate" title={doc.document_id && doc.document_id.length > 10 ? doc.document_id : ''}>{doc.document_id}</td>
                                                <td className="py-2 px-2 max-w-[150px] truncate" title={doc.title && doc.title.length > 20 ? doc.title : ''}>{doc.title}</td>
                                                <td className="py-2 px-2 max-w-[120px] truncate" title={doc.client_name || ''}>{doc.client_name || 'N/A'}</td>
                                                <td className="py-2 px-2 max-w-[90px] truncate" title={doc.property_id || ''}>{doc.property_id || 'N/A'}</td>
                                                <td className="py-2 px-2 max-w-[120px] truncate" title={doc.document_type || ''}>{doc.document_type}</td>

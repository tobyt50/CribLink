...
const AgentDashboardSkeleton = ({ darkMode }) => (
  <div className={`animate-pulse space-y-6`}>
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-10">
      ...
      <Card className="lg:col-span-2">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 w-full">
          {[...Array(4)].map((_, j) => (<div key={j} className={`h-16 rounded-xl ${darkMode ? "bg-gray-700" : "bg-gray-200"}`}></div>))}
        </div>
      </Card>
      <Card className="lg:col-span-1">
        ...
        <div className={`h-4 w-full rounded ${darkMode ? "bg-gray-700" : "bg-gray-200"} mb-4`}></div>
        <div className={`h-10 w-full rounded-lg ${darkMode ? "bg-gray-700" : "bg-gray-200"}`}></div>
      </Card>
    </div>
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card>
            <div className={`h-8 w-1/3 rounded ${darkMode ? "bg-gray-700" : "bg-gray-200"} mb-4`}></div>
            <div className="grid grid-cols-2 gap-4 w-full">
              {[...Array(2)].map((_, j) => (<div key={j} className={`h-16 rounded-xl ${darkMode ? "bg-gray-700" : "bg-gray-200"}`}></div>))}
            </div>
        </Card>
        <Card>
            <div className={`h-8 w-1/3 rounded ${darkMode ? "bg-gray-700" : "bg-gray-200"} mb-4`}></div>
            <div className={`h-32 w-full rounded-lg ${darkMode ? "bg-gray-700" : "bg-gray-200"}`}></div>
        </Card>
    </div>
    ...
    <Card>
      ...
      <ul className="space-y-3">
        {[...Array(5)].map((_, i) => (<li key={i} className={`h-5 w-full rounded-full ${darkMode ? "bg-gray-700" : "bg-gray-200"}`}></li>))}
      </ul>
    </Card>
  </div>
);
    ...
    const fetchDashboardData = async () => {
      ...
      try {
        ...
        const activityData = activityRes.data.activities.map((a) => {
          let IconComponent = User, tag = 'User', color = 'gray';
          const msg = a.message?.toLowerCase() || '';
          if (msg.includes('inquiry')) { IconComponent = MessageSquare; tag = 'Inquiry'; color = 'blue'; }
          else if (msg.includes('listing') || msg.includes('property')) { IconComponent = Home; tag = 'Listing'; color = 'green'; }
          return {
            icon: <IconComponent size={16} />, tag, color, message: a.message,
  ...
  return (
    <div className={`${darkMode ? "bg-gray-900" : "bg-gray-50"} -mt-12 px-4 md:px-0 min-h-screen flex flex-col`}>
      ...
      <motion.div key={isMobile ? 'mobile' : 'desktop'} animate={{ marginLeft: contentShift }} transition={{ duration: 0.3, ease: 'easeInOut' }} initial={false} className="pt-6 px-4 md:px-8 flex-1 overflow-auto min-w-0" style={{ minWidth: `calc(100% - ${contentShift}px)` }}>
        ...
        <div className="hidden md:block mb-6"><h1 className={`text-3xl font-extrabold text-center mb-6 ${darkMode ? "text-green-400" : "text-green-700"}`}>Dashboard</h1></div>
        {loading || authLoading ? <AgentDashboardSkeleton darkMode={darkMode} /> : (
    ...
    <div className="flex items-center justify-between mb-3">
        <h3 className={`text-lg font-semibold ${darkMode ? "text-green-300" : "text-green-600"}`}>Your Plan</h3>
        <span className={`inline-flex items-center px-3 py-1 text-sm font-bold rounded-full ${darkMode ? 'bg-gray-700 text-gray-200' : 'bg-gray-100 text-gray-800'}`}>
            <Star className={`h-4 w-4 mr-2 ${getStarColor(tier)}`} /> {tierConfig.name}
        </span>
    </div>
    ...
    {tier !== 'enterprise' ? (
        ...
        <Link to="/subscriptions" className={`mt-4 w-full text-center py-2 px-4 rounded-lg font-semibold transition-transform duration-200 hover:scale-105 ${darkMode ? 'bg-blue-600 text-white hover:bg-blue-500' : 'bg-blue-600 text-white hover:bg-blue-700'}`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <Card>
                    <div className="flex items-center justify-between mb-4">
                        <h2 className={`text-xl font-semibold ${darkMode ? "text-green-400" : "text-green-700"}`}>Analytics</h2>
                        <BarChart2 size={24} className={`${darkMode ? "text-gray-400" : "text-gray-500"}`} />
                    </div>
                    {tier === 'basic' && <div className="text-center p-4 rounded-lg bg-gray-100 dark:bg-gray-800"><Shield size={32} className="mx-auto text-gray-400 mb-2" /><h4 className="font-semibold text-lg">Basic Analytics</h4><p className="text-sm text-gray-600 dark:text-gray-400 mb-4">You have access to basic listing view counts.</p><Link to="/subscriptions" className="font-bold text-green-600 dark:text-green-400 hover:underline">Upgrade to Pro for More Insights</Link></div>}
                    {tier === 'pro' && <div className="text-center p-4 rounded-lg bg-purple-50 dark:bg-gray-800 border border-purple-200 dark:border-purple-700"><TrendingUp size={32} className="mx-auto text-purple-500 mb-2" /><h4 className="font-semibold text-lg text-purple-800 dark:text-purple-300">Moderate Analytics</h4><p className="text-sm text-gray-600 dark:text-gray-400 mb-4">Track views, inquiries, and click-through rates.</p><Link to="/agent/analytics" className="font-bold text-purple-600 dark:text-purple-400 hover:underline">View Your Analytics</Link></div>}
                    {tier === 'enterprise' && <div className="text-center p-4 rounded-lg bg-yellow-50 dark:bg-gray-800 border border-yellow-200 dark:border-yellow-700"><Briefcase size={32} className="mx-auto text-yellow-500 mb-2" /><h4 className="font-semibold text-lg text-yellow-800 dark:text-yellow-300">Advanced Analytics</h4><p className="text-sm text-gray-600 dark:text-gray-400 mb-4">Access heatmaps, interest tracking, and exportable reports.</p><Link to="/agent/analytics" className="font-bold text-yellow-600 dark:text-yellow-400 hover:underline">Access Advanced Reports</Link></div>}
                </Card>
            </div>
            ...
            <Card>
                ...
                {activities.length > 5 && (<div className="mt-4 text-center"><button onClick={() => setShowAllActivities(prev => !prev)} className={`text-sm hover:underline ${darkMode ? "text-green-400" : "text-green-600"}`}>{showAllActivities ? 'Show Less' : 'Show More'}</button></div>)}
            </Card>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <Card>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <button onClick={goToAddListing} className={`flex items-center justify-center gap-2 p-3 rounded-lg shadow-md transition-all duration-200 ${darkMode ? "bg-green-700 hover:bg-green-600 text-white" : "bg-green-500 hover:bg-green-600 text-white"}`}><Home size={20} /> Add New Listing</button>
                        <button onClick={goToClients} className={`flex items-center justify-center gap-2 p-3 rounded-lg shadow-md transition-all duration-200 ${darkMode ? "bg-blue-700 hover:bg-blue-600 text-white" : "bg-blue-500 hover:bg-blue-600 text-white"}`}><Users size={20} /> Manage Clients</button>
                        <button onClick={goToSettings} className={`flex items-center justify-center gap-2 p-3 rounded-lg shadow-md transition-all duration-200 ${darkMode ? "bg-purple-700 hover:bg-purple-600 text-white" : "bg-purple-500 hover:bg-purple-600 text-white"}`}><Settings size={20} /> Settings</button>
                        <button onClick={() => showMessage('Reporting feature under development!', 'info')} className={`flex items-center justify-center gap-2 p-3 rounded-lg shadow-md transition-all duration-200 ${darkMode ? "bg-yellow-700 hover:bg-yellow-600 text-white" : "bg-yellow-500 hover:bg-yellow-600 text-white"}`}><BarChart2 size={20} /> Generate Report</button>
                    </div>
                </Card>
                <Card>
                    <h2 className={`text-xl font-semibold mb-4 ${darkMode ? "text-green-400" : "text-green-700"}`}>Financial Snapshot</h2>
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                        <StatCard label="Commission (YTD)" value="$X,XXX" icon={<DollarSign size={20} />} />
                        <StatCard label="Pending" value="$X,XXX" icon={<DollarSign size={20} />} />
                        <StatCard label="Marketing Spend" value="$X,XXX" icon={<DollarSign size={20} />} />
                    </div>
                    <p className={`mt-4 text-sm ${darkMode ? "text-gray-400" : "text-gray-600"}`}>*Financial data is illustrative. Integration coming soon.</p>
                </Card>
            </div>
            </motion.div>
        )}

...
const ContentBlockSkeleton = ({ darkMode, height = "h-40" }) => (
  <div
    className={`p-6 rounded-2xl shadow-xl animate-pulse ${darkMode ? "bg-gray-800" : "bg-white"} ${height}`}
  >
    <div
    ...
    ></div>
    <div
      className={`h-4 w-5/6 rounded ${darkMode ? "bg-gray-700" : "bg-gray-200"}`}
    ></div>
  </div>
);
...
const ClientInfoSkeleton = ({ darkMode }) => (
  <div
    className={`p-6 rounded-2xl shadow-xl animate-pulse ${darkMode ? "bg-gray-800" : "bg-white"}`}
  >
    {/* Header: Name and Bookmark */}
    <div className="flex justify-between items-center mb-6">
      <div
        className={`h-8 w-48 rounded ${darkMode ? "bg-gray-700" : "bg-gray-200"}`}
      ></div>
      <div
    ...
    <div className="flex flex-col sm:flex-row items-start space-y-4 sm:space-y-0 sm:space-x-4 mb-6">
      <div
        className={`w-32 h-32 rounded-full flex-shrink-0 ${darkMode ? "bg-gray-700" : "bg-gray-200"}`}
      ></div>
      <div className="flex-1 w-full space-y-3">
        ...
        ></div>
        <div
          className={`h-px w-full ${darkMode ? "bg-gray-700" : "bg-gray-200"} my-2`}
        ></div>
        <div
          className={`h-5 w-1/3 rounded ${darkMode ? "bg-gray-700" : "bg-gray-200"}`}
        ></div>
        <div
    ...
    <div
      className={`h-px w-full ${darkMode ? "bg-gray-700" : "bg-gray-200"} my-6`}
    ></div>
    <div
      className={`h-5 w-1/4 rounded ${darkMode ? "bg-gray-700" : "bg-gray-200"} mb-3`}
    ></div>
    <div
      className={`h-4 w-full rounded ${darkMode ? "bg-gray-700" : "bg-gray-200"} mb-2`}
    ></div>
    <div
      className={`h-4 w-5/6 rounded ${darkMode ? "bg-gray-700" : "bg-gray-200"} mb-6`}
    ></div>
    ...
    ></div>
    <div
      className={`h-5 w-1/3 rounded ${darkMode ? "bg-gray-700" : "bg-gray-200"} mb-3`}
    ></div>
    <div
      className={`h-4 w-full rounded ${darkMode ? "bg-gray-700" : "bg-gray-200"} mb-2`}
    ></div>
  ...
  >
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 md:gap-4">
      {[...Array(2)].map(
        (
          _,
          i, // Showing 2 is enough to represent the grid without taking too much vertical space
        ) => (
...
const ClientProfile = () => {
  ...
  const [canViewPropertyPreferences, setCanViewPropertyPreferences] =
    useState(false); // New state for property preferences visibility
  const [favouriteListings, setFavouriteListings] = useState([]);
  const [showFavourites, setShowFavourites] = useState(false);

  const [favouriteListingStartIndex, setFavouriteListingStartIndex] =
    useState(0);
  ...
  const contentShift =
    userRole === "agent" && !isMobile ? (isCollapsed ? 80 : 256) : 0;
  ...
  const fetchClientData = useCallback(async () => {
    ...
    try {
      ...
      const clientSharePropertyPreferences =
        clientRes.data.share_property_preferences_with_agents ?? false;
      setCanViewPropertyPreferences(clientSharePropertyPreferences);

      if (clientSharePropertyPreferences) {
        try {
          const preferencesRes = await axiosInstance.get(
            `${API_BASE_URL}/clients/${clientId}/preferences`,
            { headers },
          );
          setClientPreferences({
            preferred_property_type:
              preferencesRes.data.preferred_property_type || "Any",
          ...
          console.error(
            "Failed to fetch client property preferences:",
            prefErr,
          );
          if (prefErr.response && prefErr.response.status === 403) {
            showMessage(
              "Client has restricted access to their property preferences.",
              "info",
            );
          } else if (clientSharePropertyPreferences) {
            showMessage(
              "Failed to load client's property preferences.",
              "error",
            );
          }
          setClientPreferences({
      ...
      const clientShareFavourites =
        clientRes.data.share_favourites_with_agents ?? false;
      setShowFavourites(clientShareFavourites);
      console.log(
        "Client share_favourites_with_agents:",
        clientShareFavourites,
      );

      if (clientShareFavourites) {
        try {
          const favouritesRes = await axiosInstance.get(
          ...
          if (favErr.response && favErr.response.status === 403) {
            showMessage(
              "Client has restricted access to their favourite listings.",
              "info",
            );
      ...
      if (err.response) {
        ...
        showMessage(
          `Failed to load client profile: ${err.response.data.message || "Server error"}`,
          "error",
        );
      } else {
        showMessage(
    ...
    if (userRole === "agent" && agentId && clientId) {
      try {
        ...
        const recommendedRes = await axiosInstance.get(
          `${API_BASE_URL}/clients/${clientId}/recommendations`,
          {
        ...
        console.log(
          "Agent recommended listings fetched:",
  ...
  const fetchConversationForClient = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(
        `${API_BASE_URL}/inquiries/agent/${agentId}/client/${clientId}/conversation`,
        {
          headers: { Authorization: `Bearer ${token}` },
        },
      );
  ...
  const handleOpenChat = useCallback(async () => {
    ...
    if (conversationToOpen.unreadCount > 0) {
      ...
      try {
        await fetch(
          `${API_BASE_URL}/inquiries/agent/mark-read/${conversationToOpen.id}`,
          {
            // Use conversationToOpen.id
            method: "PUT",
            headers: { Authorization: `Bearer ${token}` },
          },
        );
    ...
    try {
      const token = localStorage.getItem("token");
      await fetch(
        `${API_BASE_URL}/inquiries/agent/mark-opened/${conversationToOpen.id}`,
        {
          method: "PUT",
          headers: { Authorization: `Bearer ${token}` },
        },
  ...
  const handleDeleteInquiry = useCallback(async () => {
    ...
    showConfirm({
      ...
      onConfirm: async () => {
        const token = localStorage.getItem("token");
        const res = await fetch(
          `${API_BASE_URL}/inquiries/agent/delete-conversation/${conversationForModal.id}`,
  ...
  const handleSendMessageToConversation = useCallback(
    async (conversationId, messageText) => {
      ...
      try {
        const token = localStorage.getItem("token");
        await fetch(
          `${API_BASE_URL}/inquiries/agent/mark-responded/${conversationId}`,
          {
            method: "PUT",
            headers: { Authorization: `Bearer ${token}` },
          },
    ...
    if (agentId && clientId && userRole === "agent") {
      ...
      try {
        const response = await axiosInstance.get(
          `${API_BASE_URL}/favourites/clients/status/${clientId}`,
          {
            headers: { Authorization: `Bearer ${token}` },
          },
        );
  ...
  const handleToggleClientStatus = useCallback(async () => {
    if (!client || !agentId || userRole !== "agent") {
      showMessage(
        "You must be logged in as an agent to change client status.",
        "info",
      );
      return;
    }
      ...
      onConfirm: async () => {
        try {
          ...
          await axiosInstance.put(
            `${API_BASE_URL}/clients/agent/${agentId}/clients/${clientId}/vip`,
            { status: newStatus },
            { headers: { Authorization: `Bearer ${token}` } },
          );
          ...
          showMessage(
            `Client status updated to ${newStatus.toUpperCase()}!`,
            "success",
          );
        } catch (error) {
          console.error(
  ...
  const handleSaveNote = async () => {
    ...
    try {
      ...
      await axiosInstance.put(
        `${API_BASE_URL}/clients/agent/${agentId}/clients/${clientId}/note`,
  ...
  useEffect(() => {
    ...
    const handleNewMessage = async (newMessage) => {
      ...
      const expectedClientId = Number(
        newMessage.clientId ||
          ...
          conversationForModal.client_id,
      );
      ...
      if (isFromClient && openedConversationId === conversationForModal.id) {
        ...
        if (token && agentId) {
          fetch(
            `${API_BASE_URL}/inquiries/agent/mark-read/${conversationForModal.id}`,
            {
              method: "PUT",
              headers: {
                "Content-Type": "application/json",
    ...
    agentId,
    showMessage,
    fetchConversationForClient,
  ]);

  const handlePrevFavourite = () => {
    setFavouriteListingStartIndex((prevIndex) =>
      Math.max(0, prevIndex - listingsPerPage),
    );
  };
  ...
  const handleNextFavourite = () => {
    setFavouriteListingStartIndex((prevIndex) =>
      Math.min(
        favouriteListings.length - listingsPerPage,
        prevIndex + listingsPerPage,
      ),
    );
  };
  ...
  const handleNextRecommended = () => {
    setRecommendedListingStartIndex((prevIndex) =>
      Math.min(
        ...
        prevIndex + recommendedListingsPerPage,
      ),
    );
  };
  ...
  if (loading) {
    return (
      ...
      <div
        className={`${darkMode ? "bg-gray-900" : "bg-gray-50"} -mt-12 min-h-screen flex flex-col`}
      >
        {/* Sidebar placeholder */}
        {userRole === "agent" && (
          <div
            className={`fixed top-0 left-0 h-full ${isCollapsed ? "w-20" : "w-64"} ${darkMode ? "bg-gray-800" : "bg-white"} shadow-lg transition-all duration-300`}
          >
            <div
              className={`h-full animate-pulse flex flex-col p-4 ${darkMode ? "bg-gray-800" : "bg-white"}`}
            >
              <div
                className={`h-8 w-3/4 rounded ${darkMode ? "bg-gray-700" : "bg-gray-200"} mb-6`}
              ></div>
              {[...Array(6)].map((_, i) => (
                <div
                  key={i}
                  className={`h-10 w-full rounded ${darkMode ? "bg-gray-700" : "bg-gray-200"} mb-3`}
                ></div>
              ))}
            </div>
          </div>
        ...
        >
          <div
            className={`h-10 w-1/2 md:w-1/4 rounded mx-auto mb-6 animate-pulse ${darkMode ? "bg-gray-800" : "bg-gray-200"}`}
          ></div>
          ...
          <div className="flex flex-col lg:flex-row gap-8 lg:max-w-7xl lg:mx-auto">
            <div className="w-full lg:w-3/5 space-y-8">
              ...
              {isMobile && userRole === "agent" && (
                <ContentBlockSkeleton darkMode={darkMode} height="h-32" />
              )}
          ...
          <div className="w-full lg:w-3/5 space-y-8">
            ...
            >
              ...
              <div className="flex justify-between items-center flex-wrap mb-4">
                <h2
                  className={`text-xl md:text-2xl font-extrabold ${darkMode ? "text-green-400" : "text-green-800"} flex items-center`}
                >
                  {client.full_name}
              ...
              <div className="flex items-start space-x-4 mb-6">
                ...
                <img
                  src={
                    ...
                    `https://placehold.co/120x120/${darkMode ? "374151" : "E0F7FA"}/${darkMode ? "D1D5DB" : "004D40"}?text=${getInitial(client.full_name)}`
                  }
                  alt="Client Profile"
                  className="w-32 h-32 rounded-full object-cover border-2 border-green-500 shadow-md"
                  onError={(e) => {
                    e.target.onerror = null;
                    e.target.src = `https://placehold.co/120x120/${darkMode ? "374151" : "E0F7FA"}/${darkMode ? "D1D5DB" : "004D40"}?text=${getInitial(client.full_name)}`;
                  }}
                />
              ...
              >
                ...
                {editingNote ? (
                  ...
                  <div className="relative text-sm text-gray-500 dark:text-gray-400">
                    <span className="italic break-words block leading-tight">
              ...
              {canViewPropertyPreferences ? (
                ...
                >
                  ...
                  >
                    ...
                    {clientPreferences.preferred_property_type || "Any"}
                  </p>
                  <p
                    className={`flex items-center gap-2 text-base ${darkMode ? "text-gray-300" : "text-gray-700"}`}
                  >
                    📍 <strong>Location:</strong>{" "}
                    {clientPreferences.preferred_location || "Any"}
                  </p>
                  <p
                    className={`flex items-center gap-2 text-base ${darkMode ? "text-gray-300" : "text-gray-700"}`}
                  >
                    💰 <strong>Price Range:</strong>{" "}
                    {clientPreferences.min_price
                      ? `₦${clientPreferences.min_price.toLocaleString()}`
                      : "Any"}{" "}
                    -{" "}
                    {clientPreferences.max_price
                      ? `₦${clientPreferences.max_price.toLocaleString()}`
                      : "Any"}
                  </p>
                  <p
                    className={`flex items-center gap-2 text-base ${darkMode ? "text-gray-300" : "text-gray-700"}`}
                  >
                    🛏️ <strong>Bedrooms:</strong>{" "}
                    {clientPreferences.bedrooms > 0
                      ? clientPreferences.bedrooms
                      : "Any"}
                  </p>
                  <p
                    className={`flex items-center gap-2 text-base ${darkMode ? "text-gray-300" : "text-gray-700"}`}
                  >
                    ...
                    {clientPreferences.bathrooms > 0
                      ? clientPreferences.bathrooms
                      : "Any"}
                  </p>
                </div>
              ) : (
                ...
                >
                  ...
                  <p
                    className={`text-sm ${darkMode ? "text-gray-400" : "text-gray-600"}`}
                  >
                    Client has chosen not to share their property preferences
                    with agents.
                  </p>
                </div>
              )}
              ...
              {showFavourites ? (
                favouriteListings.length > 0 ? (
                  ...
                  <p
                    className={`text-sm ${darkMode ? "text-gray-400" : "text-gray-600"}`}
                  >
                    Client has no favourited listings yet.
                  </p>
                )
              ) : (
                <p
                  className={`text-sm ${darkMode ? "text-gray-400" : "text-gray-600"}`}
                >
                  Client has chosen not to share their favourited listings with
                  agents.
                </p>
              )}
            </motion.div>
          ...
          <div className="w-full lg:w-2/5 space-y-8">
            ...
            >
              ...
              recommendedListings.length > 0 ? (
                ...
                >
                  {!Array.isArray(recommendedListings)
                    ? "Loading recommended listings or error occurred."
                    : "No listings recommended for this client yet."}
                </p>
              ) : (
                <p
                  className={`text-sm ${darkMode ? "text-gray-400" : "text-gray-600"}`}
                >
                  This section is for agents to manage recommended listings.
                </p>
              )}
            </motion.div>
          </div>
